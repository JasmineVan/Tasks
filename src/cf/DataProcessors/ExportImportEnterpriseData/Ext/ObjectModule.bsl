...
Function ExchangeComponents(ExchangeDirection, ExchangeFornatVersionOnImport = "") 
	...
	CurrFormatVersion = ?(ExchangeDirection = "Send", FormatVersion, ExchangeFornatVersionOnImport);
	...
	EndIf;
	ExchangeComponents.EventLogMessageKey = NStr("ru = 'Перенос данных через буфер обмена'; en = 'Clipboard data transfer'; pl = 'Transfer danych przez schowek';de = 'Datentransfer über die Zwischenablage';ro = 'Transferul de date prin clipboard';tr = 'Pano üzerinden veri aktarma'; es_ES = 'Traslado de datos a través de portapapeles'", Common.DefaultLanguageCode());
	...
	If ExchangeManagerInternal Then
		...
		If ExchangeComponents.ExchangeManager = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не поддерживается версия формата обмена: <%1>.'; en = 'The exchange format version is not supported: %1.'; pl = 'Nie jest obsługiwana wersja formatu wymiany: <%1>.';de = 'Die Version des Austauschformats wird nicht unterstützt: <%1>.';ro = 'Nu este susținută versiunea formatului de schimb: <%1>.';tr = 'Değişim biçimi sürümü desteklenmiyor:<%1>.'; es_ES = 'No se admite la versión del formato de cambio: <%1>.'"), CurrFormatVersion);
		EndIf;
	EndIf;
...
Function ExportToXMLResult(ParametersStructure, AddressToPlaceResult = Undefined) Export
	...
	If ExportResult.HasErrors Then
		MessageText = NStr("ru = 'В ходе выполнения операции возникли ошибки'; en = 'Operation execution errors'; pl = 'W trakcie wykonywania operacji wystąpiły błędy';de = 'Während des Vorgangs sind Fehler aufgetreten';ro = 'În cursul executării operației s-au produs erori';tr = 'Işlem esnasında hatalar oluştu'; es_ES = 'Al ejecutar la operación se han producido errores'") + ": "
			+ Chars.LF + ExportResult.ErrorText
			+ Chars.LF + NStr("ru = 'Данные могут быть выгружены не полностью.'; en = 'Some data might not be exported.'; pl = 'Dane mogą nie być wczytane w pełni.';de = 'Daten werden möglicherweise nicht vollständig exportiert.';ro = 'Datele pot fi descărcate incomplet.';tr = 'Veriler tamamen dışa aktarılmamış olabilirler.'; es_ES = 'Los datos pueden ser subidos no completamente.'");
		Common.MessageToUser(MessageText);
	ElsIf Not ExportResult.HasExportedObjects Then
		MessageText = NStr("ru = 'Не найдено ни одного объекта к выгрузке.'; en = 'There are no objects to export.'; pl = 'Nie znaleziono ani jednego obiektu do wczytania.';de = 'Es wurden keine Objekte zum Exportieren gefunden.';ro = 'Nu a fost găsit nici un obiect pentru descărcare.';tr = 'Dışa aktarılacak hiç bir veri bulunamadı.'; es_ES = 'No se ha encontrado ningún objeto para subir.'");
		Common.MessageToUser(MessageText);
	EndIf;
...
Function ImportDataFromXML(XMLReader)
	...
	If XMLReader.NodeType <> XMLNodeType.StartElement
		...
		ImportResult.ErrorText = NStr("ru='Ошибка чтения сообщения загрузки. Неверный формат сообщения.'; en = 'Cannot read the import message. Invalid message format.'; pl = 'Błąd odczytu wiadomości do pobrania. Nieprawidłowy format wiadomości.';de = 'Fehler beim Lesen der Upload-Meldung. Das Nachrichtenformat ist falsch.';ro = 'Eroare de citire a mesajului de încărcare. Format de mesaj incorect.';tr = 'Yükleme iletisi okuma hatası. Yanlış ileti biçimi.'; es_ES = 'Error de leer el mensaje de descarga. Formato de mensaje incorrecto.'");
...
Procedure FillExportRules() Export
	...
	TreeNodeCatalogs.Description = NStr("ru='Справочники'; en = 'Catalogs'; pl = 'Katalogi';de = 'Stammdaten';ro = 'Cataloage';tr = 'Ana kayıtlar'; es_ES = 'Catálogos'");
	
	TreeNodeDocuments = ExportRuleTable.Rows.Add();
	TreeNodeDocuments.IsFolder = True;
	TreeNodeDocuments.Description = NStr("ru='Документы'; en = 'Documents'; pl = 'Dokumenty';de = 'Dokumente';ro = 'Documente';tr = 'Belgeler'; es_ES = 'Documentos'");

	CCTTreeNode = ExportRuleTable.Rows.Add();
	CCTTreeNode.IsFolder = True;
	CCTTreeNode.Description = NStr("ru='Планы видов характеристик'; en = 'Charts of characteristic types'; pl = 'Plany rodzajów charakterystyk';de = 'Diagramme von charakteristischen Typen';ro = 'Diagrame de tipuri caracteristice';tr = 'Karakteristik tiplerin çizelgeleri'; es_ES = 'Diagramas de los tipos de características'");
	...
	For Each DPRRow In ExchangeComponents.DataProcessingRules Do
		...
		If Metadata.Catalogs.Contains(CurrMetadata) Then
			...
			Presentation = NStr("ru='Справочник %1'; en = '""%1"" catalog'; pl = 'Skorowidz %1';de = 'Verzeichnis %1';ro = 'Clasificatorul %1';tr = 'Katalog %1'; es_ES = 'Catálogo %1'");
		ElsIf Metadata.Documents.Contains(CurrMetadata) Then
			...
			Presentation = NStr("ru='Документ %1'; en = '""%1"" document'; pl = 'Dokument %1';de = 'Dokument %1';ro = 'Documentul %1';tr = 'Belge %1'; es_ES = 'Documento %1'");
		ElsIf  Metadata.ChartsOfCharacteristicTypes.Contains(CurrMetadata) Then
			...
			Presentation = NStr("ru='План видов характеристик %1'; en = '""%1"" chart of characteristic types'; pl = 'Plan rodzajów cech %1';de = 'Plan der Merkmals-Arten %1';ro = 'Planul tipurilor de caracteristici %1';tr = 'Özellik türlerin %1 planı'; es_ES = 'Plan de tipos de características %1'");
		Else
...
Procedure AddTabularSectionCompositionAdditionalSets(SourceItems, AddingOptions)
	...
	If ObjectMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru='Некорректное имя метаданных ""%1""'; en = 'Invalid metadata name: ""%1"".'; pl = 'Nieprawidłowa nazwa metadanych ""%1""';de = 'Falscher Metadatenname ""%1""';ro = 'Nume de metadate incorect ""%1""';tr = 'Metaverilerin ""%1"" adı yanlıştır'; es_ES = 'Nombre incorrecto de metadatos ""%1""'"),
				NameOfTableToAdd);
	EndIf;
