...
Function ExternalDataProcessorInfo() Export
	...
	If SubsystemExists("StandardSubsystems.AdditionalReportsAndDataProcessors") Then
		...
		NewCommand.Presentation = NStr("ru = 'Групповое изменение реквизитов'; en = 'Bulk attribute edit'; pl = 'Zbiorcza edycja atrybutów';de = 'Massenattributbearbeitung';ro = 'Editarea atributelor în masă';tr = 'Toplu özellik düzenleme'; es_ES = 'Edición del atributo grueso'");
...
Function CommonObjectsAttributes(ObjectsTypes) Export
	...
	For Index = 1 To MetadataObjects.Count() - 1 Do
		TabularSections = SetIntersection(TabularSections, ItemsList(MetadataObjects[Index].TabularSections));
	EndDo;
...
Procedure RunAlgorithmCode(Val Object, Val AlgorithmCode, Val ExecuteInSafeMode)
	...
	Else
		Execute AlgorithmCode; // CAC:487 For "maintenance" scenarios, the code can be executed by the Administrator.
	EndIf;
...
Function SubsystemExists(FullSubsystemName) Export
	...
	SubsystemsNames = SubsystemNames();
...
Function CommonModule(Name) Export
	...
	If TypeOf(Module) <> Type("CommonModule") Then
		Raise SubstituteParametersToString(NStr("ru = 'Общий модуль ""%1"" не найден.'; en = 'Common module %1 is not found.'; pl = 'Nie znaleziono wspólnego modułu ""%1"".';de = 'Gemeinsames Modul ""%1"" wurde nicht gefunden.';ro = 'Modulul comun ""%1"" nu a fost găsit.';tr = 'Ortak modül ""%1"" bulunamadı.'; es_ES = 'Módulo común ""%1"" no se ha encontrado.'"), Name);
	EndIf;
...
// CAC:487-off Copy of the GeneralPurpose.ExecuteInSafeMode(). code as data processor is "rejected".
