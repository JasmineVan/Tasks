...
Procedure SetInsertModeFromClipboard(TemplateWithData, ColumnsInformation, TypesDetails) Export
	...
	For each Type In TypesDetails.Types() Do
		...
		ColumnHeader = NStr("ru = 'Введенные данные'; en = 'Entered data'; pl = 'Wprowadzone dane';de = 'Daten eingegeben';ro = 'Datele introduse';tr = 'Girilen veri'; es_ES = 'Datos introducidos'");
...
Function DocumentByPresentation(Presentation, Types)
	...
	For each Type In Types Do
		...
		If StrFind(Presentation, ItemPresentation) > 0 Then
			...
			DocumentDate = StringToDate(DateRoundedToDay);
...
Procedure ColumnsInformationFromCatalogAttributes(ImportParameters, ColumnsInformation)
	...
	For each Attribute In CatalogMetadata.Attributes Do
		...
		If Attribute.Type.ContainsType(Type("Boolean")) Then 
			ColumnTypeDetails = NStr("ru = 'Флаг, Да или 1 / Нет или 0'; en = 'Check box, Yes or 1 / No or 0'; pl = 'Pole wyboru, Tak lub 1 / Nie lub 0';de = 'Kontrollkästchen, Ja oder 1 / Nein oder 0';ro = 'Caseta de validare, Da sau 1 / Nu sau 0';tr = 'Onay kutusu, Evet veya 1 / Hayır veya 0'; es_ES = 'Casilla de verificación, Sí o 1 / No o 0'");
		ElsIf Attribute.Type.ContainsType(Type("Number")) Then 
			ColumnTypeDetails =  StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Число, Длина: %1, Точность: %2'; en = 'Digit, Length: %1, Precision: %2'; pl = 'Cyfra, Długość: %1, Dokładność: %2';de = 'Ziffer, Länge: %1, Genauigkeit: %2';ro = 'Cifra, Lungime: %1, Precizie: %2';tr = 'Basamak, Uzunluk:%1, Doğruluk:%2'; es_ES = 'Dígito, Longitud: %1, Exactitud: %2'"),
		...
		ElsIf Attribute.Type.ContainsType(Type("String")) Then
			If Attribute.Type.StringQualifiers.Length > 0 Then
				...
				ColumnTypeDetails =  StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Строка, макс. количество символов: %1'; en = 'String, length limit: %1'; pl = 'Ciąg, maksimum znaków: %1';de = 'Zeichenfolge, maximale Zeichen: %1';ro = 'Rândul, numărul max de caractere: %1';tr = 'Dize, maksimum karakterler:%1'; es_ES = 'Línea, máximo de símbolos: %1'"), StringLength);
			Else
				ColumnTypeDetails = NStr("ru = 'Строка неограниченной длины'; en = 'String of unlimited length'; pl = 'Ciąg o nieograniczonej długości';de = 'Zeichenfolge von unbegrenzter Länge';ro = 'Rând de lungime nelimitată';tr = 'Sınırsız uzunluk'; es_ES = 'Línea de longitud no limitada'");
			EndIf;
		...
		ElsIf Attribute.Type.ContainsType(Type("UUID")) Then
			ColumnTypeDetails = NStr("ru = 'УникальныйИдентификатор'; en = 'UUID'; pl = 'UnikalnyIdentyfikator';de = 'EindeutigeKennung';ro = 'IdentificatorUnic';tr = 'EşsizKimlik'; es_ES = 'UUID'");
		EndIf;
...
Procedure CreateStandardAttributesColumn(ColumnsInformation, CatalogMetadata, ColumnName, Position)
	...
	If DataType = Type("String") Then 
		TypePresentation = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Строка (не более %1 символов)'; en = 'String (up to %1 characters)'; pl = 'Wiersz (nie więcej niż %1 znaki)';de = 'Reihe (nicht mehr als %1 Zeichen)';ro = 'Rând (nu mai mult de %1 caractere)';tr = 'Satır (en fazla%1 karakter içermez)'; es_ES = 'Fila (no más de %1 símbolos)'"), TypeDetails.StringQualifiers.Length);
	...
	ElsIf DataType = Type("Number") Then
		TypePresentation = NStr("ru = 'Число'; en = 'Number'; pl = 'Liczba';de = 'Nummer';ro = 'Numărul';tr = 'Numara'; es_ES = 'Número'");
	Else
...
Procedure DetermineColumnsInformationTabularSection(Val ColumnsInformation, Template, TableHeaderArea, Val ImportParameters)
	...
	If MetadataObject = Undefined Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru='Загрузка данных из файла в табличную часть не поддерживается для объектов типа: %1'; en = 'Cannot import data from a file to a tabular section for objects of %1 type.'; pl = 'Pobieranie danych z pliku do części tabelarycznej nie jest obsługiwane dla obiektów typu: %1';de = 'Das Laden von Daten aus einer Datei in das Tabellenteil wird für Objekte des Typs nicht unterstützt: %1';ro = 'Importul datelor din fișier în secțiunea tabelară nu este susținută pentru obiectele de tipul: %1';tr = 'Bir dosyadan tablo bölümüne veri yükleme %1türü nesneleri için desteklenmiyor'; es_ES = 'La carga del archivo a la parte de tabla no se admite para los objetos del tipo: %1'"),
				MetadataObjectName);
		Raise ErrorText;
	EndIf;
	...
	Else
		Raise NStr("ru = 'Не найден макет для загрузки данных из файла'; en = 'Cannot find a template for importing data.'; pl = 'Szablon do importu danych z pliku nie został znaleziony';de = 'Vorlage für Datenimport aus Datei wurde nicht gefunden';ro = 'Șablonul pentru importul de date din fișier nu a fost găsit';tr = 'Dosyadan veri içe aktarma için şablon bulunamadı'; es_ES = 'Modelo para la importación de datos desde el archivo no se ha encontrado'");
	EndIf;
...
Procedure CreateColumnsInformationFromTemplate(TableHeaderArea, ImportFromFileParameters, ColumnsInformation, NamesOfColumnsToAdd = Undefined)
	...
	For ColumnNumber = 1 To TableHeaderArea.TableWidth Do
		...
		If AttributePresentation = NStr("ru = '<Дополнительные реквизиты>'; en = '<Additional attributes>'; pl = '<Dodatkowe rekwizyty>';de = '<Zusätzliche Details>';ro = '<Atribute suplimentare>';tr = '<Ek özellikler>'; es_ES = '<Requisitos adicionales>'") Then
			CatalogMetadata = Metadata.FindByFullName(ImportFromFileParameters.FullObjectName);
			If Common.SubsystemExists("StandardSubsystems.Properties") Then
				ModulePropertyManagerInternal = Common.CommonModule("PropertyManagerInternal");
				ModulePropertyManagerInternal.ColumnsForDataImport(CatalogMetadata, ColumnsInformation);
			EndIf;
		ElsIf AttributePresentation = NStr("ru = '<Контактная информация>'; en = '<Contact information>'; pl = '<Informacje kontaktowe>';de = '<Kontaktinformationen>';ro = '<Informații de contact>';tr = '<İletişim bilgileri>'; es_ES = '<Contactos>'") Then
			CatalogMetadata = Metadata.FindByFullName(ImportFromFileParameters.FullObjectName);
			If Common.SubsystemExists("StandardSubsystems.ContactInformation") Then
				ModuleContactsManager = Common.CommonModule("ContactsManager");
				ModuleContactsManager.ColumnsForDataImport(CatalogMetadata, ColumnsInformation);
...
Function AdjustValueToType(Val Value, TypesDetails)
	
	For each Type In TypesDetails.Types() Do
		If Type = Type("Date") Then
			Return StringToDate(Value, TypesDetails.DateQualifiers.DateFractions);
...
Procedure ImportCSVFileToTable(FileName, TemplateWithData, ColumnsInformation)
	...
	If Row = Undefined Then 
		MessageText = NStr("ru = 'Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.'; en = 'Cannot import data from the file. Probably it contains invalid data.'; pl = 'Nie można zaimportować danych z tego pliku. Upewnij się, że dane w pliku są poprawne.';de = 'Kann Daten aus dieser Datei nicht importieren. Stellen Sie sicher, dass die Daten in der Datei korrekt sind.';ro = 'Nu se pot importa date din acest fișier. Asigurați-vă că datele din fișier sunt corecte.';tr = 'Bu dosyadan veri alınamıyor. Dosyadaki verilerin doğru olduğundan emin olun.'; es_ES = 'No se puede importar los datos desde este archivo. Asegurarse de que los datos en el archivo sean correctos.'");
		Raise MessageText;
	EndIf;
	...
	For each TableRow In MappedData Do
		...
		Try
			If ValueIsFilled(TableRow.MappingObject) Then
				...
				If CatalogItem = Undefined Then
					Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Номенклатура с артикулом %1 не существует.'; en = 'A product with product ID %1 does not exist.'; pl = 'Brak produktów z artykułu (SKU) %1.';de = 'Keine Produkte mit Artikelnummer %1.';ro = 'Nu există nomenclatorul cu articolul %1.';tr = 'SKU''lu ürün yok.%1'; es_ES = 'No hay productos con SKU %1.'"),
					TableRow.SKU);
				EndIf;
			Else
		...
		Except
			...
			NStr("ru = 'Не удалось записать элемент справочника %1 по причине:
				...
				|%2';
				|ro = 'Eșec la înregistrarea elementului clasificatorului %1 din motivul:
				|%2';
				|tr = 'Kılavuz %1öğesi 
...
Procedure GenerateReportTemplate(ReportTable, TemplateWithData)
	...
	FillTemplateHeaderCell(Cell, NStr("ru ='Результат'; en = 'Result'; pl = 'Wynik';de = 'Ergebnis';ro = 'Rezultat';tr = 'Sonuç'; es_ES = 'Resultado'"), 12, NStr("ru ='Результат загрузки данных'; en = 'Data import result'; pl = 'Wynik importu danych';de = 'Daten Ladeergebnis';ro = 'Rezultatul importului de date';tr = 'Veri içe aktarma sonucu'; es_ES = 'Resultado de la importación de datos'"), True);
...
Function EventLogEvent() 
	...
	Return NStr("ru = 'Загрузка данных из файла'; en = 'Import data from file'; pl = 'Importuj dane z pliku';de = 'Daten aus der Datei importieren';ro = 'Importați date din fișier';tr = 'Verileri dosyadan içe aktar'; es_ES = 'Importar los datos del archivo'", Common.DefaultLanguageCode());
	
EndFunction

Function StringToDate(Val Value, DatePart = Undefined)
	...
	Else
		...
		IsOnlyNumbers = StrSplit(Item, NumbersSet, False).Count() = 0;
		If IsOnlyNumbers Then
			Return DateTypeDetails.AdjustValue(Item);
	...
	If ValueIsFilled(DateValue) Then
	...
	If ValueIsFilled(TimeValue) Then
