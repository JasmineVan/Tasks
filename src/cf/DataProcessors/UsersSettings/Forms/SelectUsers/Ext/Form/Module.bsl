...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If SettingsClearing Then
		Title =
			NStr("ru = 'Выбор пользователей для очистки настроек'; en = 'Select users to clear settings'; pl = 'Wybierz użytkowników, aby oczyścić ustawienia';de = 'Wählen Sie die Benutzer aus, um die Einstellungen zu löschen';ro = 'Selectați utilizatorii pentru a șterge setările';tr = 'Ayarları temizlemek için kullanıcıları seçin'; es_ES = 'Seleccionar usuario para eliminar las configuraciones'");
		Items.Label.Title =
			NStr("ru = 'Выберите пользователей, которым необходимо очистить настройки'; en = 'Select the users whose settings you want to clear:'; pl = 'Wybierz użytkowników, dla których należy oczyścić ustawienia';de = 'Wählen Sie Benutzer aus, für die Einstellungen gelöscht werden müssen';ro = 'Selectați utilizatorii pentru care trebuie golite setările';tr = 'Ayarları temizlemek için kimlerin gerekli olduğunu seçin'; es_ES = 'Seleccionar usuarios para los cuales se requiere eliminar las configuraciones'");
	EndIf;
...
Procedure Select(Command)
	...
	If UsersDestination.Count() = 0 Then
		ShowMessageBox(,NStr("ru = 'Необходимо отметить одного или несколько пользователей.'; en = 'Please select one or several users.'; pl = 'Zaznacz jednego lub wielu użytkowników.';de = 'Markieren Sie einen oder mehrere Benutzer.';ro = 'Marcați unul sau mai mulți utilizatori.';tr = 'Bir veya birden çok kullanıcıyı işaretleyin.'; es_ES = 'Marcar uno o usuarios múltiples.'"));
		Return;
	EndIf;
...
Procedure UpdateGroupTitle(Form, UsersGroup, UserListRow, MarkValue)
	...
	If MarkedUser <> Undefined AND MarkValue <> UserListRow.Check Then
		...
		UsersGroup.GroupDescriptionAndUserMarkCount = 
			StringFunctionsClientServer.SubstituteParametersToString(NStr("ru='%1 (%2)'; en = '%1 (%2)'; pl = '%1 (%2)';de = '%1 (%2)';ro = '%1 (%2)';tr = '%1 (%2)'; es_ES = '%1 (%2)'"), String(UsersGroup.Group), UsersGroup.MarkedUsersCount);
	EndIf;
