...
Function DuplicatesGroups(Val SearchParameters, Val SampleObject = Undefined) Export
	...
	// List of fields perhaps modified by the applied code.
	...
	For Each FieldName In FieldsNamesInChoice Do
		...
		If AvailableDCField = Undefined Then
			WriteLogEvent(DuplicateObjectDetection.SubsystemDescription(False),
				EventLogLevel.Warning, MetadataObject, SampleObject,
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Поле ""%1"" не существует.'; en = 'Field %1 does not exist.'; pl = 'Pole ""%1"" nie istnieje.';de = 'Das Feld ""%1"" existiert nicht.';ro = 'Câmpul ""%1"" nu există.';tr = '""%1"" Alanı mevcut değil.'; es_ES = 'El campo ""%1"" no existe.'"), String(DCField)));
			Continue;
		EndIf;
	...
	While NextSelectionItem(SampleObjectsSelection) Do
		...
		If SimilarityFieldsStructure.Count() > 0 Then
			...
			If FuzzySearch = Undefined Then
				Result.ErrorDescription = 
					NStr("ru = 'Не удалось подключить внешнюю компоненту FuzzyStringMatchExtension из макета ""ОбщийМакет.КомпонентаПоискаСтрок""
					           |Подробнее см. в журнале регистрации.'; 
					           |en = 'Cannot attach add-in FuzzyStringMatchExtension from template CommonTemplate.StringSearchAddIn.
					           |For more information, see the event log.'; 
					           |pl = 'Nie udało się podłączyć zewnętrzny komponent FuzzyStringMatchExtension z makiety ""CommonTemplate.StringSearchAddIn""
					           |Więcej informacji można znaleźć w dzienniku rejestracji.';
					           |de = 'Fehler beim Verbinden der externen Komponente FuzzyStringMatchExtension des Layouts ""CommonTemplate.StringSearchAddIn""
					           |Weitere Informationen finden Sie im Ereignisprotokoll.';
					           |ro = 'Eșec la conectarea componentei externe FuzzyStringMatchExtension din macheta ""CommonTemplate.StringSearchAddIn""
					           |Detalii vezi în registrul logare.';
					           |tr = 'Harici bileşen FuzzyStringMatchExtension, ""CommonTemplate.StringSearchAddIn"" 
					           |düzeninden bağlanamadı. Ayrıntılar için günlüğe bakın.'; 
					           |es_ES = 'No se ha podido conectar el componente externo FuzzyStringMatchExtension de la plantilla ""CommonTemplate.StringSearchAddIn""
					           |Véase más en registro de eventos.'");
				Return Result;
			EndIf;
			For Each KeyValue In SimilarityFieldsStructure Do
				FieldName = KeyValue.Key;
		...
		If ReturnedBatchSize > 0 AND (DuplicatesTable.Count() > ReturnedBatchSize) Then
			...
			If Found > 0 AND DuplicatesTable.Count() = 0 Then
				Result.ErrorDescription = NStr("ru = 'Найдено слишком много дублей одного элемента.'; en = 'Too many duplicates of the item were found.'; pl = 'Wykryto zbyt wiele duplikatów pojedynczego elementu.';de = 'Es wurden zu viele Duplikate eines einzelnen Elements gefunden.';ro = 'Au fost găsite prea multe duplicate ale unui element.';tr = 'Tek bir öğenin çok fazla kopyası bulundu.'; es_ES = 'Se ha encontrado demasiados duplicados de un elemento.'");
			Else
				Result.ErrorDescription = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Найдено слишком много дублей. Показаны только первые %1.'; en = 'Too many duplicates were found. First %1 items are shown.'; pl = 'Wykryto zbyt wiele duplikatów. Pokazane są tylko pierwsze %1.';de = 'Zu viele Duplikate gefunden. Es werden nur die ersten angezeigt %1.';ro = 'Au fost găsite prea multe duplicate. Sunt afișate doar primele %1.';tr = 'Çok fazla yinelenen bulundu. Sadece ilk gösterilir%1.'; es_ES = 'Se ha encontrado demasiados duplicados. Se muestran solo los primeros %1.'"), ReturnedBatchSize);
			EndIf;
			Break;
		EndIf;
	EndDo;
