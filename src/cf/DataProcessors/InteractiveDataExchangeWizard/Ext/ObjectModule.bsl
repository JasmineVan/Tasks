...
Procedure ExecuteDataImportFromExchangeMessagesIntoCache(Cancel, RowIndexes)
	...
	If TablesToImport.Count() > 0 Then
		...
		If DataExchangeDataProcessor.ErrorFlag() Then
			Cancel = True;
			NString = NStr("ru = 'При загрузке сообщения обмена возникли ошибки: %1'; en = 'Errors occurred while importing the exchange message: %1'; pl = 'Podczas importu wiadomości wymiany wystąpiły błędy: %1';de = 'Beim Importieren der Austauschnachricht sind Fehler aufgetreten: %1';ro = 'Au apărut erori la importul mesajului de schimb: %1';tr = 'Değişim mesajları alınırken hatalar oluştu: %1'; es_ES = 'Errores ocurridos al importar el mensaje de intercambio: %1'");
