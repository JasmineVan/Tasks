...
Procedure TryToDeleteReference(Ref, ExecutionParameters, Information, Result)
	...
	If Object.DeletionMark <> True Then
		Result.Success = False;
		Result.ErrorInfo = NStr("ru = 'Объект не помечен на удаление.'; en = 'Object is not marked for deletion.'; pl = 'Obiekt nie jest oznaczony do usunięcia.';de = 'Das Objekt ist nicht zum Löschen markiert.';ro = 'Obiectul nu este marcat pentru ștergere.';tr = 'Nesne silinmek için işaretlenmemiş.'; es_ES = 'Objeto no está marcado para borrar.'");
		Return;
	EndIf;
	...
	If Not Result.Success Then
		Result.ErrorInfo = NStr("ru = 'Объект используется в других объектах программы.'; en = 'Object is in use by other application objects.'; pl = 'Obiekt jest używany przez inne obiekty aplikacji.';de = 'Objekt wird von anderen Anwendungsobjekten verwendet.';ro = 'Obiectul este utilizat de alte obiecte din aplicație.';tr = 'Nesne diğer uygulama nesneleri tarafından kullanılıyor.'; es_ES = 'Objeto está en uso por los objetos de otra aplicación.'");
...
Procedure DeleteMarkedObjectsExclusively(ExecutionParameters)
	...
	While ObjectsToDelete.Count() > 0 Do
		...
		Except
			...
			WriteWarning(Undefined, ErrorInformation);
		...
		If ItemsPreventingDeletion.Columns.Count() < 3 Then
			Raise NStr("ru = 'Не удалось выполнить удаление объектов.'; en = 'Cannot delete objects.'; pl = 'Nie udało się wykonać usunięcie obiektów.';de = 'Objekte konnten nicht gelöscht werden.';ro = 'Eșec la ștergerea obiectelor.';tr = 'Nesneler silinemedi.'; es_ES = 'No se ha podido borrar los objetos.'");
		EndIf;
		...
		For Each TableRow In ItemsPreventingDeletion Do
			...
			Index = ObjectsToDelete.Find(TableRow.FoundItemReference);
			If Index <> Undefined Then
				Continue; // The link does not prevent deletion.
			EndIf;
...
Function IsTechnicalObject(Val FullObjectName)
	...
	IsTechnicalObject = False;
	
	If Common.SubsystemExists("StandardSubsystems.ReportsOptions") Then
		ModuleReportsServer = Common.CommonModule("ReportsServer");
		IsTechnicalObject = ModuleReportsServer.IsTechnicalObject(FullObjectName);
	EndIf;
	
	Return IsTechnicalObject 
		Or FullObjectName = Upper("Catalog.MetadataObjectIDs")
...
Procedure WriteWarning(Ref, ErrorInformation)
	...
	WriteLogEvent(
		NStr("ru = 'Удаление помеченных'; en = 'Deletion of marked objects'; pl = 'Usunięcie oznaczonych obiektów';de = 'Löschen von markierten Objekten';ro = 'Ștergerea celor marcate';tr = 'İşaretli nesneleri silme'; es_ES = 'Eliminación de los objetos marcados'", Common.DefaultLanguageCode()),
	...
	If TableRow.FoundItemReference = Undefined Then
		...
		Else
			Reason.FoundItemReference = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Обнаружены неразрешимые ссылки (%1)'; en = 'Incorrect references are detected (%1)'; pl = 'Wykryto nieprawidłowe linki (%1)';de = 'Falsche Referenzen werden erkannt (%1)';ro = 'Sunt detectate referințe incorecte (%1)';tr = 'Yanlış referanslar tespit edildi (%1)'; es_ES = 'Referencias incorrectas se han detectado (%1)'"),
...
Procedure MarkCollectionTraversalProgress(ExecutionParameters, CollectionName)
	...
	If CollectionName = "BeforeSearchForItemsMarkedForDeletion" Then
		...
		Text = NStr("ru = 'Подготовка к поиску объектов, помеченных на удаление.'; en = 'Preparing to search for objects marked for deletion.'; pl = 'Przygotowanie do wyszukiwania obiektów, oznaczonych do usunięcia.';de = 'Vorbereitung, um nach den zum Löschen markierten Objekten zu suchen.';ro = 'Pregătire pentru căutarea obiectelor marcate la ștergere.';tr = 'Silmek için işaretlenmiş nesneleri bulmak için hazırlayın.'; es_ES = 'Preparación para la búsqueda de los objetos marcados para borrar.'");
		
	ElsIf CollectionName = "FindMarkedForDeletion" Then
		
		Text = NStr("ru = 'Поиск объектов, помеченных на удаление.'; en = 'Search for objects marked for deletion.'; pl = 'Wyszukaj obiekty, oznaczone do usunięcia.';de = 'Suchen Sie nach Objekten, die zum Löschen markiert sind.';ro = 'Căutați obiecte marcate pentru ștergere.';tr = 'Silinmek üzere işaretlenmiş nesneleri arayın.'; es_ES = 'Buscar los objetos marcados para borrar.'");
		
	ElsIf CollectionName = "AllObjectsMarkedForDeletion" Then
		
		Text = NStr("ru = 'Анализ помеченных на удаление.'; en = 'Analysis of objects marked for deletion.'; pl = 'Analiza oznaczonych do usunięcia.';de = 'Analyse von zur Löschung markiert.';ro = 'Analiza celor marcate la ștergere.';tr = 'Silinmek için işaretli olanların analizi.'; es_ES = 'Análisis de los marcados para borrar.'");
		
	ElsIf CollectionName = "TechnologicalObjects" Then
		
		Text = NStr("ru = 'Подготовка к удалению.'; en = 'Prepare for deletion.'; pl = 'Przygotowanie do usunięcia.';de = 'Bereiten Sie das Löschen vor.';ro = 'Pregătire pentru ștergere.';tr = 'Silme için hazırlık.'; es_ES = 'Preparar para borrar.'");
		
	ElsIf CollectionName = "ExclusiveDeletion" Then
		
		Text = NStr("ru = 'Выполняется удаление объектов.'; en = 'Deleting objects.'; pl = 'Trwa usuwanie obiektów.';de = 'Objekte entfernen';ro = 'Are loc ștergerea obiectelor.';tr = 'Nesneler siliniyor.'; es_ES = 'Eliminando los objetos.'");
	...
	ElsIf CollectionName = "UserObjects" Then
		...
		If NotDeleted = 0 Then // Cannot go to the StrTemplate.
			Text = NStr("ru = 'Удалено: %1 из %2 объектов.'; en = 'Deleted: %1 of %2 objects.'; pl = 'Usunięto: %1 z %2 obiektów.';de = 'Gelöscht: %1 von %2 Objekten.';ro = 'Șters: %1 din %2 obiecte.';tr = 'Silinen:  %1 nesnelerin %2 ''i.'; es_ES = 'Borrado: %1 de %2 objetos.'");
			Text = StringFunctionsClientServer.SubstituteParametersToString(Text, PresentationNumber, PresentationTotal);
		Else
			Text = NStr("ru = 'Обработано: %1 из %2 объектов, из них не удалено: %3.'; en = 'Processed: %1 out of %2 objects; not deleted: %3.'; pl = 'Przetworzono: %1 z %2 obiektów; nie usunięto: %3.';de = 'Verarbeitet: %1 aus %2 Objekten; nicht gelöscht: %3.';ro = 'Procesat: %1 din %2 obiecte; nu a fost șters: %3.';tr = 'İşlenmiş: %1 nesnelerin  %2''i; silinmedi %3.'; es_ES = 'Procesado: %1 de %2 objetos; no borrado: %3.'");
	...
	ElsIf CollectionName = "ToRedelete" Then
		...
		Text = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Повторная проверка не удаленных объектов: %1 из %2.'; en = 'Recheck not deleted objects: %1 out of %2.'; pl = 'Sprawdź ponownie nieusunięte obiekty: %1 z %2.';de = 'Überprüfen Sie nicht gelöschte Objekte: %1 von %2.';ro = 'Verificare repetată a obiectelor neșterse: %1 din %2.';tr = 'Silinmeyen nesnelerin tekrar kontrolü: %1 ''den %2.'; es_ES = 'Volver a revisar los objetos no borrados: %1 de %2.'"),
	...
	ElsIf CollectionName = "ItemsPreventingDeletion" Then
		...
		Text = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Анализ объектов, препятствующих удалению: %1 из %2.'; en = 'Analysis of objects that prevent from deletion: %1 out of %2.'; pl = 'Sprawdź ponownie obiekty, które przeszkadzają usunięciu: %1 z %2.';de = 'Analyse von Objekten, die das Löschen verhindern: %1 von %2.';ro = 'Analiza obiectelor care împiedică ștergerea: %1 din %2.';tr = 'Silinmeye engel olan nesnelerin analizi: %1''den%2.'; es_ES = 'Análisis de los objetos que impiden la eliminación: %1 de %2.'"),
			Format(ExecutionParameters.Number, "NZ=0; NG="),
			Format(ExecutionParameters.Total, "NZ=0; NG="));
