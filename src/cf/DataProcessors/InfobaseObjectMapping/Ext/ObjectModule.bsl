...
Procedure ExecuteDataImportForInfobase(Cancel, TablesToImport) Export
	...
	If DataExchangeDataProcessor.ErrorFlag() Then
		NString = NStr("ru = 'При загрузке сообщения обмена возникли ошибки: %1'; en = 'Error importing the exchange message: %1'; pl = 'Podczas importu wiadomości wymiany wystąpiły błędy: %1';de = 'Beim Importieren der Austauschnachricht sind Fehler aufgetreten: %1';ro = 'Au apărut erori la importul mesajului de schimb: %1';tr = 'Değişim mesajları alınırken hatalar oluştu: %1'; es_ES = 'Errores ocurridos al importar el mensaje de intercambio: %1'");
...
Procedure ObjectsMappingData(SourceTable, UserFields, TempTablesManager)
	...
	If DataExchangeServer.IsXDTOExchangePlan(InfobaseNode) Then
		QueryText = QueryText + "
			...
			|	
			|		// {MAPPING REGISTER DATA}
			|		ISNULL(InfobaseObjectsMaps.SourceUUID, DestinationTable.Ref)                  AS DestinationUUID,
			|		ISNULL(InfobaseObjectsMaps.DestinationUUID, SourceTable.UUID) AS SourceUUID,
			...
			|	ON
			|		  InfobaseObjectsMaps.DestinationUUID = SourceTable.UUID
			...
			|		UnapprovedMappingTable AS UnapprovedMappingTable
			|	ON
			|		  UnapprovedMappingTable.DestinationUUID = SourceTable.UUID
	...
	QueryText = QueryText + "
		...
		|		
		|		// {MAPPING REGISTER DATA}
		|		DestinationTable.Ref       AS DestinationUUID,
		...
		|	WHERE
		|		MappedObjectsTable.DestinationUUID IS NULL
		|	) AS NestedQuery
		|;
		|
		|";
...
Procedure AutimaticMappingDataByGUIDPlusBySearchFields(SourceTable, MappingFieldsList, UserFields, TempTablesManager)
	...
	|		AutomaticallyMappedObjectsTableFull.DestinationUUID,
	...
	|	ON
	|		AutomaticallyMappedObjectsTableFull.DestinationUUID = WrongMappedDestinationObjectsTable.DestinationUUID
	|	
	|	WHERE
	|		  IncorrectlyMappedSourceObjectsTable.SourceUUID IS NULL
	|		AND WrongMappedDestinationObjectsTable.DestinationUUID IS NULL
	|	
	|	) AS NestedQuery
	|;
	|
	...
	For each Item In TableFieldsList Do
		...
		If Attribute = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для объекта метаданных ""%1"" не определен реквизит с именем ""%2""'; en = 'Attribute with the ""%2"" name is not defined for the ""%1"" metadata object'; pl = 'Atrybut o nazwie ""%2"" nie jest zdefiniowany dla ""%1"" obiektu metadanych';de = 'Attribut mit dem Namen ""%2"" ist nicht für das Metadatenobjekt ""%1"" definiert';ro = 'Atributul cu numele ""%2"" nu este definit pentru obiectul metadate ""%1""';tr = '%2 adlı metadata nesnesi için %1 adlı öznitelik belirlenmedi.'; es_ES = 'Atributo con el nombre ""%2"" no está definido para el objeto de metadatos ""%1""'"),
