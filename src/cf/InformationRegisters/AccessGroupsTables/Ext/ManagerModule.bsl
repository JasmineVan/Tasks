...
Procedure UpdateRegisterData(AccessGroups = Undefined,
	...
	If Catalogs.ExtensionsVersions.ExtensionsChangedDynamically() Then
		Raise NStr("ru = 'Расширения конфигурации обновлены, требуется перезапустить сеанс.'; en = 'Configuration extensions are updated, restart the session.'; pl = 'Rozszerzenia konfiguracji są zaktualizowane, musisz zrestartować sesję.';de = 'Konfigurationserweiterungen aktualisiert, Sitzungsneustart erforderlich.';ro = 'Extensiile configurației sunt actualizate, trebuie să relansați sesiunea.';tr = 'Yapılandırma uzantıları güncellendi, oturumu yeniden başlatın.'; es_ES = 'Extensiones de la configuración actualizadas, se requiere reiniciar la sesión.'");
	EndIf;
