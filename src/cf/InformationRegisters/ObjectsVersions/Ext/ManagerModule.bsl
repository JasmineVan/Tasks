	...
	For Each curRow In SpreadsheetDocumentChangeTable Do
		...
		For Index = 0 To UBound Do
			...
			If curRow[ColumnName] = "AND" Then
				...
				TextTemplate = NStr("ru='сравнить версию №%1 с версией №%2'; en = 'compare version #%1 with version #%2'; pl = 'porównać nr%1 wersji z numerem%2 wersji';de = 'um die Versionsnummer%1 mit der Versionsnummer%2 zu vergleichen';ro = 'compară versiunea nr.%1 cu versiunea nr.%2';tr = '%1 sayılı sürümü %2 sayılı sürüm ile karşılaştır'; es_ES = 'comparar la versión №%1 con la versión №%2'");
				Area.Text = StringFunctionsClientServer.SubstituteParametersToString(TextTemplate, VersionNumber0, VersionNumber1);
			...
			ElsIf curRow[ColumnName] = "U" Then
				...
				Area.Text = NStr("ru='Сохранение изменений табличного документа отключено'; en = 'Saving changes is disabled for spreadsheet documents'; pl = 'Zapisywanie zmian w dokumencie tabelarycznego jest wyłączone';de = 'Das Speichern von Änderungen in einem Tabellendokument ist deaktiviert';ro = 'Salvarea modificărilor documentului tabelar este dezactivată';tr = 'Tablo belgesindeki değişiklikleri kaydetme devre dışı bırakıldı'; es_ES = 'Está desactivado guardar los cambio del documento de tabla'");
				
			ElsIf curRow[ColumnName] = "A" Then
				Area = ReportTS.Join(TemplateCellAdded);
				Area.Text = NStr("ru='открыть'; en = 'open'; pl = 'otwórz';de = 'öffnen';ro = 'deschide';tr = 'açık'; es_ES = 'abrir'");
...
Procedure OutputHeader(ReportTS, VersionList, VersionsCount, CommonTemplate, ObjectRef)
	...
	SectionHeader.Parameters.ReportDescription = NStr("ru = 'Отчет по изменениям версий объекта'; en = 'Object version delta report'; pl = 'Sprawozdanie po przemianach wersji obiektu';de = 'Objektversionsänderungsbericht';ro = 'Raport privind modificarea versiunii obiectului';tr = 'Nesne sürüm değişikliği raporu'; es_ES = 'Informe de cambios de la versión del objeto'");
...
Function GetVersionDetails(ObjectRef, VersionNumber)
	...
	Details = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '№ %1 / (%2) / %3'; en = '#%1 / (%2) / %3'; pl = '№ %1 / (%2) / %3';de = '№ %1 / (%2) / %3';ro = 'Nr. %1 / (%2) / %3';tr = '№ %1 / (%2) / %3'; es_ES = '№ %1 / (%2) / %3'"), VersionNumber.Presentation, 
...
Procedure AddRowNumbersToTabularSections(TabularSections)
	...
	For Each Map In TabularSections Do
		...
		If Table.Columns.Find("LineNumber") <> Undefined Then
			Continue;
		EndIf;
		Table.Columns.Insert(0, "LineNumber",,NStr("ru = '№ строки'; en = '#'; pl = 'Wiersz #';de = 'zeile #';ro = 'Nr. rândului';tr = 'hat #'; es_ES = 'línea #'"));
