...
Procedure UpdateUsersGroupings(Parameters = Undefined, HasChanges = Undefined) Export
	...
	Else
		Raise
			NStr("ru = 'Ошибка в процедуре ОбновитьГруппировкиПользователей
			           ...
			           |pl = 'Błąd w procedurze ZaktualizowaćGrupyUżytkowników
			           |modułu menedżera rejestru informacji Grupy wartości dostępu.
			           |
			           |Określone są błędne parametry.';
			           |de = 'Fehler in der Prozedur AktualisierungGruppierungBenutzer
			           |des Informationsregistermanagers der Zugriffswertgruppen.
			           |
			           |Ungültige Parameter angegeben.';
			           |ro = 'Eroare în procedura ОбновитьГруппировкиПользователей
			           |a modulului managerului registrului de date Grupurile valorilor de acces.
			           |
			           |Sunt specificați parametrii incorecți.';
...
Procedure UpdateAccessValueGroups(AccessValue, HasChanges, ValuesWithChangesByTypes)
	...
	ErrorTitle =
		NStr("ru = 'Ошибка при обновлении групп значений доступа.'; en = 'An error occurred when updating access value groups.'; pl = 'Błąd podczas aktualizowania grup wartości dostępu.';de = 'Fehler beim Aktualisieren der Zugriffswertgruppen.';ro = 'Eroare la actualizarea grupurilor valorilor de acces.';tr = 'Erişim değeri grupları güncellenirken bir hata oluştu.'; es_ES = 'Error al actualizar los grupos de valores de acceso.'")
		+ Chars.LF
		+ Chars.LF;
	...
	If AccessKindProperties = Undefined Then
		Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для типа ""%1""
			           ...
			           |ro = 'Utilizarea grupurilor de acces
			           |nu este setată pentru tipul ""%1"".';
		...
		If AccessKindProperties.ValuesGroupsType = Type("Undefined") Then
			Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'У значения доступа ""%1""
				           ...
				           |typu ""%2"" 
				           |nie powiódł się odczyt wymaganych danych Link do powodu:
				           |%3';
				           |de = 'Der Zugriffswert ""%1""
				           | vom Typ ""%2""
				           |konnte das Attribut Link aus folgendem Grund nicht lesen:
				           |%3';
				           |ro = 'La valoarea de acces ""%1""
				           |de tipul ""%2""
				           |nu a putut fi citit atributul Referința din motivul:
				           |%3';
				           |tr = '""%1""
				           | tür %2""
			 ...
			 OR TabularSectionMetadata.Attributes.Find("AccessGroup") = Undefined Then
				...
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'У типа значений доступа ""%1""
					           ...
					           |ro = 'La tipul valorilor de acces ""%1""
					           |nu este creată secțiunea tabelară specială ГруппыДоступа
					           |cu atributul special ГруппаДоступа.';
			...
			Else
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'У значения доступа ""%1""
					           ...
					           |pl = 'Wartość dostępu ""%1""
					           |typu ""%2"" 
					           |nie udało się odczytać tabelarycznej części GrupaDostępu
					           | z danymi GrupaDostępu z powodu:
					           |%3';
					           |de = 'Der Zugriffswert ""%1""
					           | vom Typ ""%2""
					           ...
					           |ro = 'La valoarea de acces ""%1""
					           |de tipul ""%2""
					           |nu a putut fi citită secțiunea tabelară ГруппаДоступа
					           |cu atributul ГруппаДоступа din motivul:
					           |%3';
					           |tr = '""%1""
					           | tür %2""
			...
			If TypeMetadata.Attributes.Find("AccessGroup") = Undefined Then
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'У типа значений доступа ""%1""
					           ...
					           |pl = 'U typu wartości dostępu ""%1""
					           |nie utworzono specjalnego atrybutu GrupaDostępu.';
					           |de = 'Für die Zugriffswertetyp ""%1""
					           |wird kein spezielles Attribut für die ZugriffsGruppe erstellt.';
					           |ro = 'La tipul valorilor de acces ""%1""
					           |nu este creat atributul special ГруппаДоступа.';
			...
			Else
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'У значения доступа ""%1""
					           ...
					           |pl = 'Wartość dostępu ""%1""
					           |typu ""%2"" 
					           |nie udało się odczytać danych GrupaDostępu z powodu:
					           |%3';
					           |de = 'Der Zugriffswert ""%1""
					           |des Typs ""%2""
					           ...
					           |ro = 'La valoarea de acces ""%1""
					           |de tipul ""%2""
					           |nu a putut fi citit atributul ГруппаДоступа din motivul:
					           |%3';
					           |tr = '""%1""
					           | tür %2""
...
Procedure UpdatePerformersGroups(PerformersGroups = Undefined,
	...
	Else
		Raise
			NStr("ru = 'Ошибка в процедуре ОбновитьГруппыИсполнителей
			           ...
			           |Parameters are incorrect.'; 
			           |pl = 'Błąd w procedurze ZaktualizowaćGrupyUżytkowników
			           |modułu menedżera rejestru informacji GrupyWartościDostępu.
			           |
			           |Określone są błędne parametry.';
			           ...
			           |ro = 'Eroare în procedura ОбновитьГруппыИсполнителей
			           |a modulului managerului registrului de date ГруппыЗначенийДоступа.
			           |
			           |Sunt specificați parametrii incorecți.';
			           |tr = 'Bilgi kayıt yöneticisi modülü Erişim değeri gruplarının 
			           |KullanıcıGruplarınıYenile prosedüründe bir hata oluştu. 
