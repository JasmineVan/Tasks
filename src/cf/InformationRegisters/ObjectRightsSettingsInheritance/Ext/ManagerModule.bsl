...
Procedure UpdateOwnerParents(RightsSettingsOwner, HasChanges = False, UpdateHierarchy = False, ObjectsWithChanges = Undefined) Export
	...
	ErrorTitle =
		NStr("ru = 'Ошибка при обновлении иерархии владельцев прав по значениям доступа.'; en = 'An error occurred when updating the rights owner hierarchy by access values.'; pl = 'Podczas aktualizacji hierarchii posiadaczy praw wg wartości dostępu wystąpił błąd.';de = 'Beim Aktualisieren der Rechteinhaberhierarchie durch Zugriffswerte ist ein Fehler aufgetreten.';ro = 'Eroare la actualizarea ierarhiei titularilor drepturilor conform valorilor de acces.';tr = 'Hak sahibi hiyerarşisini erişim değerleriyle güncellerken bir hata oluştu.'; es_ES = 'Ha ocurrido un error al actualizar la jerarquía de propietarios de derechos por valores de acceso.'")
		+ Chars.LF
		+ Chars.LF;
	...
	If AvailableRights.ByTypes.Get(OwnerType) = Undefined Then
		Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для типа ""%1""
			           ...
			           |ro = 'Pentru tipul ""%1""
			           | nu este setată utilizarea setărilor pentru drepturile obiectelor.';
			           |tr = '""%1""
	...
	Lock = New DataLock;
	Lock.Add("InformationRegister.ObjectRightsSettingsInheritance");
