...
Procedure FileSystemExtensionAttachedOwnerWorkingDirectorySelectionStartFollowUp(Result, AdditionalParameters) Export
	...
	If OpenFileDialog.Choose() Then
		...
		Except
			...
			ErrorText =
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Неверный путь или отсутствуют права на запись в каталог ""%1""'; en = 'Invalid path or insufficient rights to write to directory ""%1"".'; pl = 'Błędna ścieżka lub niewystarczające prawa zapisu do katalogu ""%1""';de = 'Falscher Pfad oder unzureichende Rechte zum Schreiben in Verzeichnis ""%1""';ro = 'Traseu incorect sau drepturi insuficiente de a scrie în directorul ""%1""';tr = '""%1"" dizinine yazmak için yanlış yol veya yetersiz haklar '; es_ES = 'Ruta incorrecta o insuficientes derechos para inscribir al directorio ""%1""'"), DirectoryName);
