			...
			If Object.Ref.IsEmpty() Then
				Raise
					NStr("ru = 'В демонстрационном режиме не поддерживается
					           ...
					           |ro = 'Utilizatorii noi nu pot fi creați
					           |în regimul demo.';
...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If Not WriteParameters.Property("WithFirstAdministratorAdding")
	   AND ValueIsFilled(InfobaseUserName)
	...
	If IBUserWritingRequired(ThisObject) Then
		...
		If WriteParameters.Property("WithFirstAdministratorAdding") Then
			CurrentObject.AdditionalProperties.Insert("CreateAdministrator",
				NStr("ru = 'Первый пользователь информационной базы назначается администратором.'; en = 'The first infobase user is granted administrator rights.'; pl = 'Pierwszy użytkownik bazy informacyjnej jest wyznaczany na administratora.';de = 'Der erste Benutzer der Informationsbasis wird vom Administrator ernannt.';ro = 'Primul utilizator al bazei de informații se atribuie de administrator.';tr = 'Veritabanın ilk kullanıcısı admin tarafından atanır.'; es_ES = 'El primer usuario de la base de información se nombra por el administrador.'"));
		EndIf;
	EndIf;
	...
	If Not Items.Roles.ReadOnly Then
		...
		For Each Row In TreeItems Do
			...
			If Row.IsUnavailableRole Then
				CommonClientServer.AddUserError(Errors,
					"Roles[%1].RolesSynonym",
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" недоступна для пользователей.'; en = 'Role ""%1"" is unavailable to users.'; pl = 'Rola ""%1"" nie jest dostępna dla użytkowników.';de = 'Die Rolle ""%1"" ist für Benutzer nicht verfügbar.';ro = 'Rolul ""%1"" este inaccesibil pentru utilizatori.';tr = '""%1"" rolü kullanıcılar için kullanılamaz.'; es_ES = 'El rol ""%1"" no está disponible para los usuarios.'"), Row.Synonym),
					"Roles",
					TreeItems.IndexOf(Row),
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" в строке %%1 недоступна для пользователей.'; en = 'Role ""%1"" in line #%%1 is unavailable to users.'; pl = 'Rola ""%1"" w wierszu %%1 nie jest dostępna dla użytkowników.';de = 'Die Rolle ""%1"" in der Zeile %%1 steht den Benutzern nicht zur Verfügung.';ro = 'Rolul ""%1"" în rândul %%1 este inaccesibil pentru utilizatori.';tr = '%%1 satırındaki rol ""%1"" kullanıcılar için kullanılamaz.'; es_ES = 'El rol ""%1"" en la línea %%1 no está disponible para los usuarios.'"), Row.Synonym));
			EndIf;
		EndDo;
	...
	If Object.DeletionMark AND CanSignIn Then
		...
		ShowMessageBox(,
			NStr("ru = 'Чтобы разрешить вход в программу, требуется снять
			           ...
			           |marcajul de ștergere de la acest utilizator.';
...
Procedure Attachable_ContactInformationAutoComplete(Item, Text, ChoiceData, DataGetParameters, Wait, StandardProcessing)
	...
	ModuleContactsManagerClient =
		CommonClient.CommonModule("ContactsManagerClient");
	ModuleContactsManagerClient.AutoCompleteAddress(Item, Text, ChoiceData, DataGetParameters, Wait, StandardProcessing);
...
Procedure SetConditionalAppearance()
	...
	ItemFilter.RightValue = NStr("ru = 'ПолныеПрава'; en = 'FullRights'; pl = 'FullRights';de = 'FullRights';ro = 'FullRights';tr = 'FullRights'; es_ES = 'FullRights'");
...
&AtServer
Function Attachable_UpdateContactInformation(Result)
	
	ModuleContactsManager =
		Common.CommonModule("ContactsManager");
	
	Return ModuleContactsManager.UpdateContactInformation(
...
Procedure CheckEmailFilling(Cancel)
	...
	Common.MessageToUser(
		NStr("ru = 'Не заполнен адрес электронной почты'; en = 'The email address is blank.'; pl = 'Nie wypełniono adresu poczty elektronicznej';de = 'Nicht ausgefüllte E-Mail-Adresse';ro = 'Nu este completat e-mail';tr = 'E-mail adresi doldurulmadı'; es_ES = 'La dirección de correo electrónico no rellenada'"),,
...
Function DetermineContactInformationItemsAvailability()
	...
	Result = New Map;
	For Each ContactInformationRow In ThisObject.ContactInformationAdditionalAttributesDetails Do
