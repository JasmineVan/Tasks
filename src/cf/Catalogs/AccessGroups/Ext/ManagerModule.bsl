...
Function PersonalAccessGroupsParent(Val DoNotCreate = False, ItemsGroupDescription = Undefined) Export
	...
	ItemsGroupDescription = NStr("ru = 'Персональные группы доступа'; en = 'Personal access groups'; pl = 'Osobiste grupy dostępu';de = 'Persönliche Zugriffsgruppen';ro = 'Grupuri de acces personal';tr = 'Kişisel erişim grupları'; es_ES = 'Grupos de acceso personal'");
...
Procedure UpdateAccessGroupsAuxiliaryDataChangedOnImport() Export
	...
	If AccessManagementInternal.LimitAccessAtRecordLevelUniversally() Then
		AccessManagementInternal.ScheduleAccessGroupsSetsUpdate(
			"UpdateAccessGroupsAuxiliaryDataChangedOnImport");
	EndIf;
...
Procedure ProcessDataForMigrationToNewVersion(Parameters) Export
	...
	AccessGroupProcessingErrorTemplate =
		NStr("ru = 'Не удалось обработать группу доступа ""%1"" по причине:
		           ...
		           |%2';
		           |ro = 'Eșec la procesarea grupului de acces ""%1"" din motivul:
		           |%2';
	...
	AccessGroupsTablesUpdateErrorTemplate =
		NStr("ru = 'Не удалось обновить таблицы группы доступа ""%1"" по причине:
		           ...
		           |%2';
		           |ro = 'Eșec la actualizarea tabelelor grupului de acces ""%1"" din motivul:
		           |%2';
	...
	AccessGroupsValuesUpdateErrorTemplate =
		NStr("ru = 'Не удалось обновить значения доступа группы доступа ""%1"" по причине:
		           ...
		           |%2';
		           |ro = 'Eșec la actualizarea valorii de acces a grupului de acces ""%1"" din motivul:
		           |%2';
	...
	If ObjectsProcessed = 0 AND ObjectsWithIssuesCount <> 0 Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Процедуре Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось
			           ...
			           |de = 'Das Verfahren Catalogs.AccessGroups.ProcessDataForMigrationToNewVersion kann die Hilfsdaten einiger Zugriffsgruppen nicht
			           |aktualisieren (diese Gruppen werden übersprungen):%1';
			           |ro = 'Procedura Catalogs.AccessGroups.ProcessDataForMigrationToNewVersion nu poate
			           |actualiza datele auxiliare ale unor grupuri de acces (aceste grupuri sunt ignorate):%1';
			           |tr = 'Procedure Catalogs.AccessGroups.ProcessDataForMigrationToNewVersion prosedürü, 
			           |bazı erişim grupların yardımcı verilerini güncelleyemedi (atlatıldı): %1'; 
	...
	Else
		WriteLogEvent(InfobaseUpdate.EventLogEvent(),
			...
			NStr("ru = 'Процедура Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию
			           ...
			           |pl = 'Procedura Katalogi.GrupyDostępu.PrzetwórzDaneDoPrzejściaNaNowąWersję
			           |przetworzyła kolejną partię grup dostępu: %1';
			           |de = 'Prozedur Verweise.Zugriffsgruppen.BerarbeitungVonDatenFürDenWechselZurNeuenVersion
			           |hat einen weiteren Teil von Zugriffsgruppen verarbeitet: %1';
			           |ro = 'Procedura Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию
			           |a procesat porțiunea de rând a grupurilor de acces: %1';
...
Function RefsKindProperties(RefsKind)
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Недопустимое значение ""%1"" параметра ВидСсылок функции СвойстваВидаСсылок.'; en = 'Invalid value ""%1"" of the RefsKind parameter of the RefsKindProperties function.'; pl = 'Niedopuszczalna wartość ""%1"" parametru RodzajŁączy funkcji WłaściwościRodzajuŁączy.';de = 'Ungültiger Wert ""%1"" des Parameters ReferenzTyp der Funktion EigenschaftenDesReferenzTyps.';ro = 'Valoare inadmisibilă ""%1"" a parametrului ВидСсылок funcției СвойстваВидаСсылок.';tr = 'RefsKindProperties işlevin RefsKind parametresinin ""%1"" izin verilmeyen değeri.'; es_ES = 'Valor no admitido ""%1"" del parámetro ВидСсылок de la función RefsKindProperties.'"),
			RefsKind);
	EndIf;
