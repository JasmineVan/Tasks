...
Function InstalledExtensions() Export
	...
	InstalledExtensions.Insert("MainState", ExtensionsChecksums(Main, True));
	InstalledExtensions.Insert("PatchesState", ExtensionsChecksums(Patches, True));
...
Function ExtensionsChangedDynamically() Export
	...
	Return SessionParameters.InstalledExtensions.MainState <> InstalledExtensions.MainState
	    Or SessionParameters.InstalledExtensions.PatchesState <> InstalledExtensions.PatchesState;
...
Procedure DeleteObsoleteParametersVersions() Export
	...
	|SELECT TOP 1
	|	ExtensionsVersions.Ref AS ExtensionsVersion,
	|	ExtensionsVersions.LastSecondVersionAddingDate AS LastSecondVersionAddingDate
	|FROM
	|	Catalog.ExtensionsVersions AS ExtensionsVersions
	|WHERE
	...
	|SELECT TOP 1
	|	ExtensionsVersions.Ref AS ExtensionsVersion,
	|	ExtensionsVersions.DateOfFirstAuthorizationAfterDeleteAllExtensions AS DateOfFirstAuthorizationAfterDeleteAllExtensions
	|FROM
	|	Catalog.ExtensionsVersions AS ExtensionsVersions
	|WHERE
	...
	For Each VersionDetails In DataExported.Rows Do
		...
		Lock = New DataLock;
		LockItem = Lock.Add("Catalog.ExtensionsVersions");
		LockItem.SetValue("Ref", CurrentVersion);
...
//  ConsiderExtencionsState - Boolean - flags are considered whether Active and SafeMode.
//
// Returns:
//  String - strings of the following format: "<Extension name> (<Extension version>) <Checksum>".
//
Function ExtensionsChecksums(Extensions, ConsiderExtencionsState = False)
	
	List = New ValueList;
	
	For Each Extension In Extensions Do
		Checksum = Extension.Name + " (" + Extension.Version + ") " + Base64String(Extension.HashSum);
		If ConsiderExtencionsState Then 
			Checksum = Checksum 
				+ " Active:" + Extension.Active 
				+ " SafeMode:" + Extension.SafeMode
				+ " PassToSubordinateDIBNodes:" + Extension.UsedInDistributedInfoBase;
		EndIf;
		List.Add(Checksum);
	EndDo;
