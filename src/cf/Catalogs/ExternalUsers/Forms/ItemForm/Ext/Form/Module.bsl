...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	ClearMessages();
	QuestionTitle = NStr("ru = 'Запись пользователя информационной базы'; en = 'Save infobase user'; pl = 'Zapis użytkownika bazy informacyjnej';de = 'Infobase-Benutzerdatensatz';ro = 'Înregistrarea utilizatorului bazei de date';tr = 'Veritabanı kullanıcı kayıtları'; es_ES = 'Registro del usuario de la infobase'");
	   ...
	   AND (Not WriteParameters.Property("DoNotCopyUserRights")
		...
		ShowQueryBox(
			...
			StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Скопировать также права пользователя ""%1""?'; en = 'Do you want to copy the rights of the user ""%1""?'; pl = 'Skopiować również uprawnienia użytkownika ""%1""?';de = 'Kopieren Sie auch die Benutzerrechte ""%1""?';ro = 'Copiați și drepturile utilizatorului ""%1""?';tr = 'Ayrıca ""%1"" kullanıcının hakları kopyalansın mı?'; es_ES = '¿Copiar también los derechos de usuario ""%1""?'"), String(CopyingValue)),
	   ...
	   AND InfobaseUserRoles.Count() = 0 Then
		...
		If NOT WriteParameters.Property("WithEmptyRoleList") Then
			...
			ShowQueryBox(
				...
				NStr("ru = 'Пользователю информационной базы не установлено ни одной роли. Продолжить?'; en = 'No roles are assigned to the infobase user. Do you want to continue?'; pl = 'Użytkownikowi bazy informacyjnej nie przypisano żadnej roli. Kontynuować?';de = 'Infobase-Benutzer wurde keine Rolle zugewiesen. Fortsetzen?';ro = 'Utilizatorul bazei de date nu are atribuit niciun rol. Continuați?';tr = 'Veritabanın kullanıcısı için herhangi bir rol atanmadı. Devam etmek istiyor musunuz?'; es_ES = 'Usuario de la infobase no se ha asignado a ningún rol. ¿Continuar?'"),
				QuestionDialogMode.YesNo,
				,
				,
				NStr("ru = 'Запись пользователя информационной базы'; en = 'Save infobase user'; pl = 'Zapis użytkownika bazy informacyjnej';de = 'Infobase-Benutzerdatensatz';ro = 'Înregistrarea utilizatorului bazei de date';tr = 'Veritabanı kullanıcı kayıtları'; es_ES = 'Registro del usuario de la infobase'"));
			Return;
		EndIf;
	EndIf;
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If CanSignIn
	   ...
	   AND ValidityPeriod <= BegOfDay(CurrentSessionDate()) Then
		...
		Common.MessageToUser(
			NStr("ru = 'Ограничение должно быть до завтра или более.'; en = 'The password expiration date must be tomorrow or later.'; pl = 'Data ważności hasła musi być jutro lub później.';de = 'Die Begrenzung sollte bis morgen oder später gelten.';ro = 'Restricția trebuie să fie până mâine sau mai mult.';tr = 'Kısıtlama yarına kadar veya daha fazla olmalıdır.'; es_ES = 'La restricción debe ser hasta mañana o más.'"),, "CanSignIn",, Cancel);
	EndIf;
	...
	If Not Items.Roles.ReadOnly Then
		...
		For Each Row In TreeItems Do
			...
			If Row.IsUnavailableRole Then
				CommonClientServer.AddUserError(Errors,
					"Roles[%1].RolesSynonym",
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" недоступна для внешних пользователей.'; en = 'Role ""%1"" is unavailable to external users.'; pl = 'Rola ""%1"" nie jest dostępna dla użytkowników zewnętrznych.';de = 'Die Rolle ""%1"" ist für externe Benutzer nicht verfügbar.';ro = 'Rolul ""%1"" este inaccesibil pentru utilizatorii externi.';tr = '""%1"" rolü harici kullanıcılar için kullanılamaz.'; es_ES = 'El rol ""%1"" no está disponible para los usuarios externos.'"), Row.Synonym),
					"Roles",
					TreeItems.IndexOf(Row),
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Роль ""%1"" в строке %%1 недоступна для внешних пользователей.'; en = 'Role ""%1"" in line #%%1 is unavailable to external users.'; pl = 'Rola ""%1"" w wierszu %%1 nie jest dostępna dla użytkowników zewnętrznych.';de = 'Die Rolle ""%1"" in der Zeile %%1 ist für externe Benutzer nicht verfügbar.';ro = 'Rolul ""%1"" în rândul %%1 este inaccesibil pentru utilizatorii externi.';tr = '%%1 satırındaki rol ""%1"" harici kullanıcılar için kullanılamaz.'; es_ES = 'El rol ""%1"" en la línea %%1 no está disponible para los usuarios externos.'"), Row.Synonym));
			EndIf;
		EndDo;
...
Procedure CanSignInOnChange(Item)
	...
	If Object.DeletionMark AND CanSignIn Then
		...
		ShowMessageBox(,
			NStr("ru = 'Чтобы разрешить вход в программу, требуется снять
			           ...
			           |ro = 'Pentru a permite logarea, ștergeți 
			           |marcajul de ștergere de la acest utilizator extern.';
	...
	If Not AccessLevel.ChangeAuthorizationPermission
	   AND Not CanSignIn Then
		...
		ShowMessageBox(,
			NStr("ru = 'После записи вход в программу сможет разрешить только администратор.'; en = 'Once you save the changes, only administrator can allow signing in to the application.'; pl = 'Po zapisaniu tylko administrator może zezwolić na logowanie.';de = 'Nach dem Speichern kann nur der Administrator die Anmeldung zulassen.';ro = 'După salvare numai administratorul poate permite intrarea în aplicație.';tr = 'Kaydedildikten sonra uygulamaya giriş izni yalnızca yönetici tarafından verilebilecektir.'; es_ES = 'Después de haber guardado, solo el administrador puede permitir el inicio de sesión.'"));
	EndIf;
...
&AtClientAtServerNoContext
Procedure CheckPasswordSet(Form, PasswordSet, AuthorizedUser)
	
	UsersInternalClientServer.CheckPasswordSet(Form, PasswordSet, AuthorizedUser);
...
Procedure FindUserAndIBUserDifferences(WriteParameters = Undefined)
	...
	ElsIf AccessLevel.ListManagement Then
		...
		If InfobaseUserOSAuthentication <> False Then
			PropertiesToResolve.Add(NStr("ru = 'Аутентификация ОС (включена)'; en = 'OS authentication (enabled)'; pl = 'Autoryzacja przez system operacyjny (włączone)';de = 'Betriebssystemauthentifizierung (aktiviert)';ro = 'Autentificarea OS (activată)';tr = 'OS kimlik doğrulama (açık)'; es_ES = 'Autenticación del sistema operativo (activado)'"));
		EndIf;
		...
		If InfobaseUserFullName <> Object.Description Then
			...
			PropertiesToResolve.Insert(0, StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Полное имя ""%1""'; en = 'Full name: ""%1""'; pl = 'Imię i nazwisko ""%1""';de = 'Vollständiger Name ""%1""';ro = 'Numele complet ""%1""';tr = 'Tam isim ""%1""'; es_ES = 'Nombre completo ""%1""'"),
				InfobaseUserFullName));
		EndIf;
		
		If InfobaseUserOSUser <> "" Then
			PropertiesToResolve.Add(NStr("ru = 'Пользователь ОС (указан)'; en = 'OS user (specified)'; pl = 'Użytkownik systemu operacyjnego (określony)';de = 'Betriebssystembenutzer (angegeben)';ro = 'Utilizator OS (specificat)';tr = 'OS kullanıcı (belirlenmiş)'; es_ES = 'Usuario del sistema operativo (especificado)'"));
		EndIf;
		
		If InfobaseUserShowInList Then
			HasDifferencesResolvableWithoutAdministrator = True;
			PropertiesToResolve.Add(NStr("ru = 'Показывать в списке выбора (включено)'; en = 'Show in selection list (enabled)'; pl = 'Pokazywać w liście wyboru (włączone)';de = 'In der Auswahlliste anzeigen (aktiviert)';ro = 'Afișați în lista de selecție (activată)';tr = 'Seçim listesinde göster (açık)'; es_ES = 'Mostrar en la lista de selección (activado)'"));
		EndIf;
		
		If InfobaseUserRunMode <> "Auto" Then
			HasDifferencesResolvableWithoutAdministrator = True;
			PropertiesToResolve.Add(NStr("ru = 'Режим запуска (не Авто)'; en = 'Run mode (not Auto)'; pl = 'Tryb uruchamiania (nie Auto)';de = 'Startmodus (nicht Auto)';ro = 'Regimul de lansare (nu Auto)';tr = 'Başlatma modu (Oto değil)'; es_ES = 'Empezar el modo (no Auto)'"));
		EndIf;
		...
		If PropertiesToResolve.Count() > 0 Then
			 ...
			 Or HasDifferencesResolvableWithoutAdministrator
			   AND FormActions.ItemProperties = "Edit" Then
				...
				Recommendation = Chars.LF
					+ NStr("ru = 'Нажмите ""Записать"", чтобы устранить различия и не выводить это предупреждение.'; en = 'To resolve the differences and not to show this message again, click ""Save"".'; pl = 'Kliknij ""Zapisz"", aby usunąć różnice i nie pokazywać ponownie tego komunikatu.';de = 'Klicken Sie auf ""Schreiben"", um die Unterschiede zu beheben und diese Nachricht nicht mehr anzuzeigen.';ro = 'Faceți clic pe ""Înregistrare"" pentru a înlătura diferențele și a nu mai afișa acest avertisment.';tr = 'Farklıları gidermek ve bu uyarıyı tekrar göstermemek için ""Kayıt"" ''a tıklayın.'; es_ES = 'Hacer clic en ""Inscribir"" para solucionar las diferencias y no mostrar este mensaje de nuevo.'");
			
			ElsIf Not Users.IsFullUser() Then
				Recommendation = Chars.LF
					+ NStr("ru = 'Обратитесь к администратору, чтобы устранить различия.'; en = 'To resolve the differences, contact your system administrator.'; pl = 'Skontaktuj się z administratorem, aby usunąć różnice.';de = 'Wenden Sie sich an Ihren Administrator, um die Unterschiede zu beheben.';ro = 'Contactați administratorul pentru a înlătura diferențele.';tr = 'Farklılıkları gidermek için yöneticinize başvurun.'; es_ES = 'Contactar su administrador para resolver las diferencias.'");
			Else
				Recommendation = "";
			EndIf;
			Items.PropertiesMismatchNote.Title = StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Следующие свойства пользователя информационной базы отличаются от указанных в этой форме:
					           ...
					           |%1.';
					           |ro = 'Următoarele proprietăți ale utilizatorului bazei de informații diferă de cele specificate în această formă:
					           |%1.';
	...
	If FormActions.ItemProperties = "Edit" Then
		Recommendation = Chars.LF
			+ NStr("ru = 'Нажмите ""Записать"", чтобы устранить проблему и не выводить это предупреждение.'; en = 'To eliminate the issue and not to show this message again, click ""Save"".'; pl = 'Aby wyeliminować problem i nie pokazywać go ponownie, kliknij ""Zapisz"".';de = 'Drücken Sie ""Schreiben"", um das Problem zu beheben und diese Warnung nicht anzuzeigen.';ro = 'Faceți clic pe ""Înregistrare"" pentru a înlătura problema și a nu mai afișa acest avertisment.';tr = 'Sorunu ortadan kaldırmak ve bu uyarıyı tekrar göstermemek için ""Kayıt"" ''a tıklayın.'; es_ES = 'Haga clic en ""Guardar"" para solucionar el problema y no mostrar este aviso.'");
	...
	Items.MappingMismatchNote.Title =
		NStr("ru = 'Пользователь информационной базы не найден.'; en = 'The infobase user is not found.'; pl = 'Użytkownik bazy informacyjnej nie został znaleziony.';de = 'Der Benutzer der Informationsbasis wurde nicht gefunden.';ro = 'Utilizatorul bazei de informații nu a fost găsit.';tr = 'Veritabanı kullanıcısı bulunamadı.'; es_ES = 'Usuario de la infobase no encontrado.'") + Recommendation;
