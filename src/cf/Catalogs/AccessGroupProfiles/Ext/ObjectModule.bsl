	...
	If Selection.Count() > 1 Then
		...
		BriefErrorPresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибка при записи профиля ""%1"".
			           ...
			           |Zainstalowany profil już istnieje:';
			           |de = 'Fehler beim Schreiben des Profils ""%1"". 
			           |Das angegebene Profil existiert bereits:';
			           |ro = 'Eroare la înregistrarea profilului ""%1"". 
			           |Profilul furnizat există deja.';
		...
		DetailedErrorPresentation = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибка при записи профиля ""%1"".
			           ...
			           |pl = 'Wystąpił błąd podczas zapisywania profilu ""%1"".
			           |Zastosowany identyfikator danych ""%2"" jest już używany w profilu:';
			           |de = 'Fehler beim Schreiben des Profils ""%1"". 
			           |Die Kennung der gelieferten Daten ""%2"" wird bereits im Profil verwendet:';
			           |ro = 'Eroare la înregistrarea profilului ""%1"". 
			           | ID-ul de date furnizate ""%2"" este deja folosit în profil:';
			           |tr = 'Profil ""%1"" yazılırken bir hata oluştu.
		...
		If FillChecking Then
			Common.MessageToUser(BriefErrorPresentation,,,, Cancel);
		Else
			WriteLogEvent(
				NStr("ru = 'Управление доступом.Нарушение однозначности поставляемого профиля'; en = 'Access management.Violation of supplied profile uniqueness'; pl = 'Zarządzanie dostępem.Naruszenie jednoznaczności dostarczanego profilu';de = 'Zugangskontrolle. Verletzung der Eindeutigkeit des angegebenen Profils';ro = 'Administrarea accesului.Încălcarea unicității profilului furnizat';tr = 'Erişim kontrolü. Sağlanan profilin tek değerliliğin ihlali'; es_ES = 'Gestión de acceso.Violación de unicidad del perfil suministrado'",
