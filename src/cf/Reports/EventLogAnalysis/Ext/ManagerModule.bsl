...
Procedure CustomizeReportOptions(Settings, ReportSettings) Export
	...
	OptionSettings.Details = 
		...
		|ro = 'Permite monitorizarea activității
		|utilizatorilor în aplicație (cât de intens și cu ce obiecte lucrează utilizatorii).';
	...
	OptionSettings.Details = 
		...
		|(detailed).'; 
		|pl = 'Szczegółowe informacje na temat obiektów,
		|z którymi użytkownik pracował w programie.';
		|de = 'Ausführliche Informationen über die Objekte,
		|mit denen der Benutzer im Programm gearbeitet hat.';
		|ro = 'Informații detaliate
		|despre obiectele cu care utilizatorul a lucrat în aplicație.';
	...
	OptionSettings.Details = NStr("ru = 'Список критичных записей журнала регистрации.'; en = 'Event log records with ""Critical"" importance.'; pl = 'Lista krytycznych wpisów w dzienniku zdarzeń.';de = 'Liste kritischer Einträge im Ereignisprotokoll.';ro = 'Lista înregistrărilor critice din registrul logare.';tr = 'Olay günlüğündeki kritik girişlerin listesi.'; es_ES = 'Lista de entradas críticas en el registro de eventos.'");
	...
	OptionSettings.Details = NStr("ru = 'Выводит график выполнения регламентных заданий в программе.'; en = 'Scheduled jobs schedule.'; pl = 'Wyświetla harmonogram zaplanowanych prac w aplikacji.';de = 'Zeigt den geplanten Jobplan in der Anwendung an.';ro = 'Afișează programul de programare programat în aplicație.';tr = 'Uygulamada zamanlanmış iş programını görüntüler.'; es_ES = 'Visualiza el horario de tareas programadas en la aplicación.'");
...
Function EventLogData(ReportParameters) Export
	...
	SourceData.Columns.Add("ErrorsCount", New TypeDescription("Number", New NumberQualifiers(10)));
	...
	If ReportOption = "UsersActivityAnalysis" Then
		...
		For Each EventLogDataRow In EventLogData Do
			...
			If EventLogDataRow.Level = EventLogLevel.Error Then
				...
				SourceDataString.Week 	  = WeekOfYearString(EventLogDataRow.Date);
				SourceDataString.User = UsernameRef;
				SourceDataString.ErrorsCount = ErrorsCount;
			EndIf;
...
Function WeekOfYearString(DateInYear)
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru='Неделя %1'; en = 'Week %1'; pl = 'Tydzień %1';de = 'Woche %1';ro = 'Săptămâna %1';tr = 'Hafta %1'; es_ES = 'Semana %1'"), WeekOfYear(DateInYear));
EndFunction
	...
	For Each Item In MaxUsersArray Do
		...
		For Each Username In TemporaryArray Do
			If Username = Item Then
				...
				UserAndNumber = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (%2)'; en = '%1 (%2)'; pl = '%1 (%2)';de = '%1 (%2)';ro = '%1 (%2)';tr = '%1 (%2)'; es_ES = '%1 (%2)'"),
					Item,
					UserSessionsCounter);
			EndIf;
		EndDo;
...
Function GenerateScheduledJobsDurationReport(FillingParameters) Export
	...
	If OutputFilter.Value = DataCompositionTextOutputType.Output
		...
		If MinScheduledJobSessionDuration > 0 Then
			IntervalsViewMode = NStr("ru = 'Отключено отображение интервалов с нулевой продолжительностью'; en = 'Hide intervals with zero duration'; pl = 'Odłączono wyświetlanie interwałów o zerowym czasie trwania';de = 'Die Anzeige von Intervallen mit einer Dauer von Null ist deaktiviert';ro = 'Afișarea intervalelor cu durata zero este dezactivată';tr = 'Sıfır süreli aralıkların görüntülenmesi devre dışı bırakıldı'; es_ES = 'Visualizado de los intervalos con la duración cero está desactivado'");
		Else
			IntervalsViewMode = NStr("ru = 'Включено отображение интервалов с нулевой продолжительностью'; en = 'Show intervals with zero duration'; pl = 'Włączono wyświetlanie interwałów o zerowym czasie trwania';de = 'Die Anzeige von Intervallen mit einer Dauer von Null ist aktiviert';ro = 'Afișarea intervalelor cu durata zero este activată';tr = 'Sıfır süreli aralıkların görüntülenmesi etkinleştirildi'; es_ES = 'Visualizado de los intervalos con la duración cero está activado'");
		EndIf;
		...
		For Each ConcurrentSessionsRow In ConcurrentSessions Do
			...
			For Each Item In ScheduledJobsArray Do
				If Not TypeOf(Item) = Type("Number")
					...
					Area.Parameters.ScheduledJobsList = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (сеанс %2)'; en = '%1 (session %2)'; pl = '%1 (sesja %2)';de = '%1 (Sitzung - %2)';ro = '%1 (sesiune %2)';tr = '%1 (oturum %2)'; es_ES = '%1 (sesión %2)'"),
						Item,
		...
		If ScheduledJobIntervalDuration >= MinScheduledJobSessionDuration Then
			If CurrentEvent <> ScheduledJobsRow.EventName Then
				If CurrentEvent <> Undefined
					...
					PointName = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 (%2 из %3)'; en = '%1 (%2 out of %3)'; pl = '%1 (%2 z %3)';de = '%1 (%2 von %3)';ro = '%1 (%2 din %3)';tr = '%1 (%2''den %3)'; es_ES = '%1 (%2 de %3)'"),
						Dot.Value,
						ScheduledJobStarts,
				...
				ElsIf Not ValueIsFilled(ScheduledJobsRow.EventMetadata) Then
					PointName = NStr("ru = 'Фоновые задания'; en = 'Background jobs'; pl = 'Zadania w tle';de = 'Hintergrund Aufgaben';ro = 'Sarcini de fundal';tr = 'Arkaplan işleri'; es_ES = 'Tareas de fondo'");
					Dot = GanttChart.SetPoint(PointName);
					OverallScheduledJobsDuration = 0;
				EndIf;
			EndIf;
			...
			Interval.Text = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 - %2'; en = '%1 - %2'; pl = '%1 - %2';de = '%1 - %2';ro = '%1 - %2';tr = '%1 - %2'; es_ES = '%1 - %2'"),
	...
	For Each EventLogDataRow In EventLogData Do 
		...
		If CurrentDate <> ConcurrentScheduledJobsDate Then
			If TableRow <> Undefined Then
				...
				TableRow.ConcurrentScheduledJobsDate = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 - %2'; en = '%1 - %2'; pl = '%1 - %2';de = '%1 - %2';ro = '%1 - %2';tr = '%1 - %2'; es_ES = '%1 - %2'"),
...
Function ScheduledJobDetails(Details) Export
	...
	Area = Template.GetArea("Table");
	Area.Parameters.JobType = NStr("ru = 'Регламентное'; en = 'Scheduled'; pl = 'Kwota planowana';de = 'Geplant';ro = 'Reglementară';tr = 'Planlı'; es_ES = 'Programado'");
...
Function ScheduledJobDuration(SJDuration)
	...
	ElsIf SJDuration <= 60 Then
		OverallScheduledJobsDuration = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 сек'; en = '%1 sec'; pl = '%1 sek.';de = '%1 s';ro = '%1 sec';tr = 'saniye%1'; es_ES = '%1 seg'"), SJDuration);
	ElsIf 60 < SJDuration <= 3600 Then
		...
		OverallScheduledJobsDuration = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 мин %2 сек'; en = '%1 min %2 sec'; pl = '%1 min %2 sek.';de = '%1 min %2 s';ro = '%1 min %2 sec';tr = '%1 dak %2 san'; es_ES = '%1 minutos %2 segundos'"), DurationMinutes, DurationSeconds);
	ElsIf SJDuration > 3600 Then
		...
		OverallScheduledJobsDuration = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 ч %2 мин'; en = '%1 h %2 min'; pl = '%1 h %2 min';de = '%1 h %2 min';ro = '%1 h %2 min';tr = '%1 sa %2 dk'; es_ES = '%1 horas %2 minutos'"), DurationHours, DurationMinutes);
	EndIf;
