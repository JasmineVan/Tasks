...
Procedure RemoveFromGroup(Command)
	...
	If NOT ValueIsFilled(CurrentAccessGroup) Then
		ShowMessageBox(, NStr("ru = 'Группа доступа не выбрана.'; en = 'No access group is selected.'; pl = 'Nie wybrano grupy dostępu.';de = 'Zugriffsgruppe ist nicht ausgewählt.';ro = 'Grupul de acces nu este selectat.';tr = 'Erişim grubu seçilmedi.'; es_ES = 'Grupo de acceso no está seleccionado.'"));
		Return;
	EndIf;
	...
	Else
		ShowMessageBox(,
			NStr("ru = 'Недостаточно прав для редактирования группы доступа.
			           ...
			           |ro = 'Drepturile insuficiente pentru editarea grupului de acces.
			           |Persoana responsabilă pentru participanții la grupul de acces și administratorul poate edita grupul de acces.';
		...
		If Add Then
			ErrorDescription =
				NStr("ru = 'Невозможно включить пользователя в группу доступа,
				           ...
				           |ro = 'Utilizatorul nu poate fi inclus în grupul de acces,
				           |deoarece utilizatorul curent
				           |nu este responsabil pentru membrii grupului de acces și
				           |nu este administrator cu drepturi depline.';
		...
		Else
			ErrorDescription =
				NStr("ru = 'Невозможно исключить пользователя из группы доступа,
				           ...
				           |ro = 'Utilizatorul nu poate fi exclus din grupul de acces,
				           |deoarece utilizatorul curent
				           |nu este responsabil pentru membrii grupului de acces și
				           |nu este administrator cu drepturi depline.';
				           |tr = 'Kullanıcıyı bir erişsilinemez, 
				           |çünkü mevcut 
				           |kullanıcı erişim grubunun üyelerinden 
				           |sorumlu değildir ve tam yetkili bir yönetici değildir.'; 
	...
	If NOT Add AND NOT UserIncludedInAccessGroup(CurrentAccessGroup) Then
		ErrorDescription =
			NStr("ru = 'Невозможно исключить пользователя из группы доступа,
			           ...
			           |ro = 'Utilizatorul nu poate fi exclus din grupul de acces,
			           |deoarece el este inclus în ea indirect.';
