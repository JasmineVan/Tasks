...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Common.IsMobileClient() Then
		Cancel = True;
		Raise NStr("ru = 'Операция не доступна в мобильном клиенте, необходимо использовать тонкий клиент.'; en = 'The operation is not available in the mobile client. Use the thin client.'; pl = 'Operacja nie jest dostępna w kliencie mobilnym, musisz użyć cienkiego klienta.';de = 'Der Vorgang ist im mobilen Client nicht verfügbar, es ist notwendig, den Thin Client zu verwenden.';ro = 'Operația nu este accesibilă pe clientul mobil, trebuie să utilizați thin-client.';tr = 'İşlem mobil istemcide mevcut değil, ince istemci kullanılmalıdır.'; es_ES = 'La operación no está disponible en el cliente móvil, es necesario usar cliente ligero.'");
	EndIf;
