...
Procedure SignInToDataAreaAfterSignOut()
	...
	If NOT IsDataAreaFilled(DataArea) Then
		...
		ShowQueryBox(NotifyDescription, NStr("ru = 'Выбранная область данных не используется, продолжить вход?'; en = 'The selected data area is not used. Do you want to sign in?'; pl = 'Wybrany obszar danych nie jest używany, kontynuować logowanie?';de = 'Der ausgewählte Datenbereich wird nicht verwendet, Anmeldung fortsetzen?';ro = 'Domeniul de date selectat nu este utilizat, continuați conectarea?';tr = 'Seçilen veri alanı kullanılmıyor, giriş devam edilsin mi?'; es_ES = 'Área de datos seleccionada no se utiliza, ¿continuar el inicio de sesión?'"),
