				...
				If ModuleSaaS.DataAreaLocked(DataArea) Then
					...
					Else
						Raise StringFunctionsClientServer.SubstituteParametersToString(
							NStr("ru = 'Не удалось выполнить обмен сообщениями по причине: область данных %1 заблокирована.'; en = 'Cannot perform the message exchange because data area %1 is locked.'; pl = 'Nie udało się wykonać wymiany komunikatów z powodu: obszar danych %1 jest zablokowany.';de = 'Der Nachrichtenaustausch konnte aus folgendem Grund nicht ausgeführt werden: Der Datenbereich %1 ist gesperrt.';ro = 'Eșec la schimbul de mesaje din motivul: domeniul de date %1 este blocat.';tr = 'Aşağıdakiler nedeniyle bir e-posta değişimi yapılamıyor: veri alanı %1 kilitlendi.'; es_ES = 'No se puede realizar un intercambio de correos electrónicos debido a: el área de datos %1 está bloqueada.'"),
							DataArea);
					EndIf;
				EndIf;
...
Procedure OnDefineSupportedInterfaceVersions(Val SupportedVersionsStructure) Export
	...
	SupportedVersionsStructure.Insert("MessageExchange", VersionsArray);
...
Procedure SetThisEndpointCode() Export
	...
	If IsBlankString(ThisNodeCode()) Then
		...
		IsEndpoint = ThisNode().GetObject();
		IsEndpoint.Code = String(New UUID());
		IsEndpoint.Write();
...
Procedure MoveMessageExchangeTransportSettings() Export
	...
	Try
		For Each SettingString In SettingsTable Do
			...
			RecordSetNew.Filter.Endpoint.Set(SettingString.Endpoint);
			
			SettingsRecord = RecordSetNew.Add();
			FillPropertyValues(SettingsRecord, SettingString);
			
			RecordSetNew.Write(True);
			
			RecordManagerOld = InformationRegisters.DeleteExchangeTransportSettings.CreateRecordManager();
			RecordManagerOld.InfobaseNode = SettingString.Endpoint;
    ...
    If SaaS.DataSeparationEnabled() AND SaaS.SeparatedDataUsageAvailable() Then
        ...
        WriteLogEvent(ThisSubsystemEventLogMessageText(),
                ...
                NStr("ru = 'Обработка очереди сообщений системы запущена из сеанса с установленными
                      ...
                      |ro = 'Procesarea rândului de mesaje ale sistemului este lansată din sesiunea cu valorile separatorilor
                      |instalate. Procesarea va fi făcută numai pentru mesajele
                      |salvate în clasificatorul separat, în elementele cu valorile separatorilor,
                      |care corespund cu valorile separatorilor sesiunii.';
        ...
        Try
            ...
            Try
                ...
                If AttemptsCounter > 3 Then
                    If NOT ValueIsFilled(MessageObject.DetailedErrorPresentation) Then
                        MessageObject.DetailedErrorPresentation = NStr("ru = 'Исчерпано количество попыток обработки сообщения.'; en = 'Number of attempts to process the message is exceeded.'; pl = 'Została wyczerpana ilość prób przetwarzania komunikatu.';de = 'Die Anzahl der Versuche, eine Nachricht zu verarbeiten, ist erschöpft.';ro = 'Numărul de încercări de procesare a mesajului este depășit.';tr = 'İletiyi işlemek için deneme sayısı limitine ulaşıldı.'; es_ES = 'Número de intentos para procesar el mensaje está superado.'");
                    EndIf;
            ...
            If MessageObject.Locked Then
                ...
                WriteLogEvent(ThisSubsystemEventLogMessageText(),
                        ...
                        StringFunctionsClientServer.SubstituteParametersToString(
                            NStr("ru = 'Исчерпано количество попыток обработки сообщения %1'; en = 'Number of attempts to process message %1 is exceeded'; pl = 'Została wyczerpana ilość prób przetwarzania komunikatu %1';de = 'Die Anzahl der Versuche, eine Nachricht %1 zu verarbeiten, ist erschöpft';ro = 'Numărul de încercări de procesare a mesajului %1 este depășit';tr = '%1İletiyi işlemek için deneme sayısı limitine ulaşıldı.'; es_ES = 'Número de intentos para procesar el mensaje está superado %1'"),
            ...
            Try
                ...
                If Selection.EndpointLocked Then
                    MessageObject.Locked = True;
                    Raise NStr("ru = 'Попытка обработки сообщения, полученного от заблокированной конечной точки.'; en = 'Attempting to process a message received from a locked endpoint.'; pl = 'Próba przetworzenia wiadomości otrzymanej z zablokowanego punktu końcowego.';de = 'Versuch, die vom gesperrten Endpunkt empfangene Nachricht zu verarbeiten.';ro = 'Încercarea de a procesa mesajul primit de la punctul final blocat.';tr = 'Kilitli uç noktadan alınan mesajı işlemeye çalışılıyor.'; es_ES = 'Intentando procesar el mensaje recibido desde el punto extremo bloqueado.'");
                EndIf;

                If FoundRows.Count() = 0 Then
                    MessageObject.Locked = True;
                    Raise NStr("ru = 'Не назначен обработчик для сообщения.'; en = 'No handlers are assigned for this message.'; pl = 'Nie ustawiono modułu obsługi wiadomości.';de = 'Nachrichten-Anwender ist nicht festgelegt.';ro = 'Handlerul pentru mesaj nu este specificat.';tr = 'Mesaj işleyici ayarlanmamış.'; es_ES = 'Manipulador de mensajes no está establecido.'");
                EndIf;
                ...
                For Each TableRow In FoundRows Do
                    ...
                    If TransactionActive() Then
                        ...
                        EndDo;
                        MessageObject.Locked = True;
                        Raise NStr("ru = 'В обработчике сообщения не была зафиксирована транзакция.'; en = 'No transactions are registered in the message handler.'; pl = 'Transakcja nie została zapisana w module obsługi wiadomości.';de = 'Die Transaktion wurde nicht im Nachrichtenanwender aufgezeichnet.';ro = 'Tranzacția nu a fost înregistrată în handlerul mesajului.';tr = 'İşlem, mesaj işleyicisine kaydedilmedi.'; es_ES = 'La transacción no se ha grabado en el manipulador de mensajes.'");
                    EndIf;
            ...
            Except
                ...
                WriteLogEvent(ThisSubsystemEventLogMessageText(),
                        EventLogLevel.Error,,,
                        StringFunctionsClientServer.SubstituteParametersToString(
                            NStr("ru = 'Ошибка обработки сообщения %1: %2'; en = 'Error while processing message %1: %2.'; pl = 'Podczas przetwarzania wiadomości %1 wystąpił błąd: %2';de = 'Bei der Verarbeitung der Nachricht ist ein Fehler aufgetreten %1: %2';ro = 'Eroare la procesarea mesajului %1: %2';tr = 'İleti işlenirken bir hata %2 oluştu: %1'; es_ES = 'Ha ocurrido un error al procesar el mensaje %1: %2'"),
            ...
            If ProcessMessagesInSharedData AND SaaS.DataSeparationEnabled() AND SaaS.SeparatedDataUsageAvailable() Then
                ...
                ErrorMessageText = StringFunctionsClientServer.SubstituteParametersToString(
                    NStr("ru = 'После обработки сообщения канала %1 не было выключено разделение сеанса.'; en = 'Session separation was not disabled after processing the %1 channel message.'; pl = 'Po przetwarzaniu komunikatu kanału %1 nie był wyłączony podział sesji.';de = 'Nach der Verarbeitung der Nachricht des Kanals %1 wurde die Sitzungsfreigabe nicht deaktiviert.';ro = 'După procesarea mesajului canalului %1 nu a fost dezactivată separarea sesiunii.';tr = 'Kanal mesajı oturumunun veri işlemcisi ayrıldıktan sonra %1 devre dışı bırakıldı.'; es_ES = 'Después de que el procesador de datos de %1 la separación de la sesión de mensajes del canal no se ha desactivado.'"),
...
Procedure SetLeadingEndpointAtRecipient(ThisEndpointCode, LeadingEndpointCode) Export
	...
	If ExchangePlans.MessageExchange.FindByCode(ThisEndpointCode) <> ThisNode() Then
		ErrorMessageString = NStr("ru = 'Заданы неверные параметры подключения к конечной точке. Параметры подключения указывают на другую конечную точку.'; en = 'Invalid endpoint connection parameters. The connection parameters refer to another endpoint.'; pl = 'Określono nieprawidłowe parametry połączenia z punktem końcowym. Parametry połączenia wskazują inny punkt końcowy.';de = 'Falsche Parameter der Verbindung zum Endpunkt wurden festgelegt. Verbindungsparameter zeigen einen anderen Endpunkt an.';ro = 'Sunt specificați parametrii incorecți de conectare la punctul final. Parametrii de conectare indică un alt punct final.';tr = 'Uç noktasına yanlış bağlantı parametreleri ayarlandı. Bağlantı parametreleri başka bir uç noktasını gösterir.'; es_ES = 'Parámetros incorrectos de la conexión al punto extremo se han establecido. Parámetros de conexión indican otro punto extremo.'");
		ErrorMessageStringForEventLog = NStr("ru = 'Заданы неверные параметры подключения к конечной точке.
			...
			|Verbindungsparameter zeigen einen anderen Endpunkt an.';
			|ro = 'Sunt specificați parametrii incorecți de conectare la punctul final.
			|Parametrii de conectare indică un alt punct final.';
			...
			|Parámetros de conexión indican otro punto extremo.'", Common.DefaultLanguageCode());
	...
	Try
		...
		If EndpointNode.IsEmpty() Then
			...
			Raise NStr("ru = 'Конечная точка в базе-корреспонденте не обнаружена.'; en = 'The endpoint is not found in the correspondent infobase.'; pl = 'Nie znaleziono punktu końcowego w bazie korespondencie.';de = 'Endpunkt wird nicht in der Korrespondenzbasis gefunden.';ro = 'Punctul final nu este găsit în baza corespondentă.';tr = 'Uç noktası çalışma dizininde bulunamadı.'; es_ES = 'Punto extremo no se ha encontrado en la base correspondiente.'");
	...
	Try
		While CanReadXML(XMLReader) Do
			...
			If TypeOf(Data) = Type("ObjectDeletion") Then
				...
				Raise NStr("ru = 'Передача объекта УдалениеОбъекта через механизм быстрых сообщений не поддерживается.'; en = 'Cannot transfer the ObjectDeletion object using the instant message tool.'; pl = 'Przekazanie obiektu ObjectDeletion poprzez mechanizm szybkich komunikatów nie jest obsługiwane.';de = 'Das Objekt ObjectDeletion kann nicht mit dem Instant-Message-Tool übertragen werden.';ro = 'Nu se poate transfera obiectul ObjectDeletion utilizând instrumentul de mesaje instantanee.';tr = 'ObjectDeletion nesnesinin anlık mesaj aracıyla aktarılması desteklenmiyor.'; es_ES = 'Paso del objeto ObjectDeletion a través de la herramienta de mensaje instantáneo no de admite.'");
...
Function EndpointConnectionEventLogEvent() Export
	...
	Return NStr("ru = 'Обмен сообщениями.Подключение конечной точки'; en = 'Message exchange. Connecting the endpoint'; pl = 'Wymiana wiadomości. Podłączenie punktu końcowego';de = 'Nachrichtenaustausch. Endpunktverbindung';ro = 'Schimb de mesaje.Conectarea punctului final';tr = 'Mesaj alışverişi. Uç noktası bağlantısı'; es_ES = 'Intercambio de mensajes.Conexión del punto extremo'", Common.DefaultLanguageCode());
...
Function LeadingEndpointSettingEventLogMessage() Export
	...
	Return NStr("ru = 'Обмен сообщениями.Установка ведущей конечной точки'; en = 'Message exchange. Setting the leading endpoint'; pl = 'Wymiana wiadomości. Ustawienia wiodącego punktu końcowego';de = 'Nachrichtenaustausch. Festlegen des führenden Endpunkts';ro = 'Schimb de mesaje.Setarea punctului final principal';tr = 'Mesaj alışverişi.  Ana uç noktayı belirleme'; es_ES = 'Intercambio de mensajes.Estableciendo el punto extremo principal'", Common.DefaultLanguageCode());
...
Function ThisSubsystemEventLogMessageText() Export
	...
	Return NStr("ru = 'Обмен сообщениями'; en = 'Message exchange'; pl = 'Wymiana wiadomości';de = 'Nachrichtenaustausch';ro = 'Schimbul de mesaje';tr = 'Mesaj alışverişi'; es_ES = 'Intercambio de mensajes'", Common.DefaultLanguageCode());
...
Procedure SendReceiveMessagesViaWebServiceExecute(Cancel)
	...
	// Exporting data for all endpoints.
	For Each Recipient In NodesArray Do
...
Procedure ConnectEndpointAtSender(Cancel,
	...
	If Not EndpointNode.IsEmpty() Then
		Cancel = True;
		ErrorMessageString = NStr("ru = 'Конечная точка уже подключена к информационной базе; наименование точки: %1'; en = 'The endpoint is already connected to the infobase. Endpoint name: %1'; pl = 'Punkt końcowy jest już połączony do bazy informacyjnej; nazwa punktu: %1';de = 'Der Endpunkt ist bereits mit der Infobase verbunden. Punktname: %1';ro = 'Punctul final este deja conectat la baza de date; numele punctului: %1';tr = 'Uç noktası zaten veritabanına bağlandı; nokta adı: %1'; es_ES = 'Punto extremo ya se ha conectado a la infobase; nombre del punto: %1'", Common.DefaultLanguageCode());
...
Function MessageHandlers()
	...
	MessagesExchangeOverridable.GetMessagesChannelsHandlers(Result);
