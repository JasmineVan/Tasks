	...
	If CanChangeUsers() Then
		...
		Else
			...
			If HasRightToAddUsers() Then
				Return ActionsWithNewSaaSUser();
			Else
				Raise NStr("ru = 'Недостаточно прав доступа для добавления новых пользователей'; en = 'Insufficient access rights for adding users'; pl = 'Niewystarczające prawa dostępu do dodania użytkowników';de = 'Unzureichende Zugriffsrechte zum Hinzufügen von Benutzern';ro = 'Drepturi de acces insuficiente pentru adăugarea utilizatorilor noi';tr = 'Kullanıcıları eklemek için yetersiz erişim hakları.'; es_ES = 'Insuficientes derechos de acceso para añadir usuarios'");
			EndIf;
...
Procedure BeforeStartIBUserProcessing(UserObject, ProcessingParameters) Export
	...
	If TypeOf(UserObject) = Type("CatalogObject.ExternalUsers")
	   AND SaaS.DataSeparationEnabled() Then
		...
		Raise NStr("ru = 'Внешние пользователи не поддерживаются в модели сервиса.'; en = 'External users are not supported in SaaS mode.'; pl = 'Użytkownicy zewnętrzni nie są obsługiwani w SaaS.';de = 'Externe Benutzer werden in SaaS nicht unterstützt.';ro = 'Utilizatorii externi nu sunt susținuți în modelul serviciului.';tr = 'Harici kullanıcılar SaaS''de desteklenmez.'; es_ES = 'Usuarios externos no admitidos en SaaS.'");
	EndIf;
		...
		If NOT SaaS.SessionWithoutSeparators() Then
			Raise
				NStr("ru = 'Обновление пользователя по сообщению
				           ...
				           |ro = 'Actualizarea utilizatorului la mesajul
				           |canalului de administrare la distanță
				           |este accesibilă numai utilizatorilor neseparați.';
		...
		If ValueIsFilled(OldUser.ServiceUserID) Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка при записи пользователя ""%1"".
				           ...
				           |Nu se permite modificarea identificatorului
				           |deja stabili al utilizatorului serviciului în elementul clasificatorului.';
		...
		If UsersInternal.UserByIDExists(
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка при записи пользователя ""%1"".
				           ...
				           |ro = 'Eroare la înregistrarea utilizatorului ""%1"".
				           |Nu se permite stabilirea identificatorului
				           |utilizatorului serviciului ""%2""
				           |în acest element al clasificatorului, deoarece el
				           |deja se utilizează în elementul ""%3"".';
				           |tr = '%1Kullanıcı yazılırken bir hata oluştu. "
" adlı öğede 
				           |zaten kullanıldığından, %3 bu katalog öğesine 
	...
	If AutoAttributes.ServiceUserID <> UserObject.ServiceUserID Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |ro = 'A apărut o eroare la scrierea utilizatorului ""%1"". Atributul
			           |ServiceUserID nu poate fi modificat.
			           |Atributul devine actualizat automat.';
			           |tr = '""%1"" kullanıcısı yazılırken bir hata oluştu. Öznitelik 
			           |HizmetiKullanıcıKimliği değiştirilemez. 
	...
	For each ObjectAccessRightsXDTO In ObjectsAccessRightsXDTO.Item Do
		...
		ElsIf ObjectAccessRightsXDTO.Object.Type() = UserCIType Then
			...
			If CIKind = Undefined Then
				MessageTemplate = NStr("ru = 'Получен неизвестный вид контактной информации: %1'; en = 'Unknown contact information kind was received: %1'; pl = 'Otrzymano nieznany rodzaj informacji kontaktowych: %1';de = 'Unbekannte Kontaktinformationen wurden empfangen: %1';ro = 'Au fost primite informații necunoscute privind informațiile de contact: %1';tr = 'Bilinmeyen iletişim bilgileri alındı:%1'; es_ES = 'Tipo de la información de contacto desconocido se ha recibido: %1'");
			...
			EndDo;
		Else
			MessageTemplate = NStr("ru = 'Получен неизвестный тип объектов доступа: %1'; en = 'Unknown access object type received: %1'; pl = 'Otrzymano nieznany typ obiektu dostępu: %1';de = 'Unbekannter Zugriffsobjekttyp erhalten: %1';ro = 'Tip obiect de acces necunoscut primit: %1';tr = 'Bilinmeyen erişim nesnesi türü alındı: %1'; es_ES = 'Tipo del objeto de acceso desconocido recibido: %1'");
