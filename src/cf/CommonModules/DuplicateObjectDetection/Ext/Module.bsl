...
Function FindItemDuplicates(Val SearchArea, Val SampleObject, Val AdditionalParameters) Export
	...
	If UseAppliedRules Then
		...
		AppliedParameters.Insert("ItemsCountToCompare", 1500);
...
Procedure ReplaceReferences(Parameters, Val ResultAddress) Export
	...
	MethodParameters.Insert("DeletionMethod", Parameters.DeletionMethod);
	MethodParameters.Insert("IncludeBusinessLogic", True);
	MethodParameters.Insert("ReplacePairsInTransaction", False);
	
	Result = Common.ReplaceReferences(Parameters.ReplacementPairs, MethodParameters);
	PutToTempStorage(Result, ResultAddress);
...
Function SubsystemDescription(ForUser) Export
	...
	Return NStr("ru = 'Поиск и удаление дублей'; en = 'Duplicate purge'; pl = 'Wyszukiwanie i usuwanie duplikatów';de = 'Duplikate suchen und entfernen';ro = 'Căutare obiect duplicat';tr = 'Kopyaları arama ve silme'; es_ES = 'Buscar y eliminar los duplicados'", LanguageCode);
EndFunction
