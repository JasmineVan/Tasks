	...
	If Not SignInToDataArea
	 ...
	 Or SignInToDataArea
	   AND Not AccessRight("DataAdministration", Metadata) Then
		...
		Raise NStr("ru = 'Недостаточно прав для работы с отключенной логикой работы системы.'; en = 'Insufficient rights to disable startup logic.'; pl = 'Nie masz wystarczających uprawnień do pracy z odłączoną logiką działania systemu.';de = 'Unzureichende Rechte für die Arbeit mit deaktivierter Systemlogik.';ro = 'Drepturi insuficiente pentru lucrul cu logica dezactivată de lucru al sistemului.';tr = 'Devre dışı bırakılmış sistem mantığı ile çalışmak için yeterli hak yok.'; es_ES = 'Insuficientes derechos para usar lógica del uso del sistema desactivada.'");
	EndIf;
...
Function WriteErrorToEventLogOnStartOrExit(Shutdown, Val Event, Val ErrorText) Export
	...
	If Event = "Startup" Then
		EventName = NStr("ru = 'Запуск программы'; en = 'Application startup'; pl = 'Uruchom aplikację';de = 'Starten Sie die Anwendung';ro = 'Lansarea aplicației';tr = 'Uygulamayı başlat'; es_ES = 'Iniciar la aplicación'", Common.DefaultLanguageCode());
		If Shutdown Then
			ErrorDescriptionBeginning = NStr("ru = 'Возникла исключительная ситуация при запуске программы. Запуск программы аварийно завершен.'; en = 'An exception occurred during the application startup. The application is closed.'; pl = 'Wyjątek jest zgłaszany podczas uruchamiania aplikacji. Uruchomienie aplikacji zostało przerwane.';de = 'Beim Starten der Anwendung wird eine Ausnahme ausgelöst. Anwendungsstart wird abgebrochen.';ro = 'La lansarea programului a apărut situație excepțională. Lansarea programului este finalizată forțat.';tr = 'Uygulamayı başlatırken bir olağandışı durum oluştu. Uygulama başlaması iptal edildi.'; es_ES = 'Ha surgido una excepción al iniciar la aplicación. Lanzamiento de la aplicación tiene que cancelarse.'");
		Else
			ErrorDescriptionBeginning = NStr("ru = 'Возникла исключительная ситуация при запуске программы.'; en = 'An exception occurred during the application startup.'; pl = 'Wyjątek jest zgłaszany podczas uruchamiania aplikacji.';de = 'Beim Starten der Anwendung wird eine Ausnahme ausgelöst.';ro = 'La lansarea programului a apărut situație excepțională.';tr = 'Uygulamayı başlatırken bir olağandışı durum oluştu.'; es_ES = 'Ha surgido una excepción al iniciar la aplicación.'");
		EndIf;
	Else
		EventName = NStr("ru = 'Завершение программы'; en = 'Exit from application'; pl = 'Zamknij aplikację';de = 'Anwendung verlassen';ro = 'Finalizarea lucrului aplicației';tr = 'Uygulamadan çık'; es_ES = 'Salir de la aplicación'", Common.DefaultLanguageCode());
		ErrorDescriptionBeginning = NStr("ru = 'Возникла исключительная ситуация при завершении программы.'; en = 'An exception occurred while exiting the application.'; pl = 'Podczas zamykania aplikacji generowany jest wyjątek.';de = 'Beim Schließen der Anwendung wird eine Ausnahme ausgelöst.';ro = 'La finalizarea programului a apărut situație excepțională.';tr = 'Uygulama kapatılırken olağandışı bir durum oluştu'; es_ES = 'Ha surgido una excepción al cerrar la aplicación.'");
	EndIf;
	
	ErrorDescription = ErrorDescriptionBeginning
		+ Chars.LF + Chars.LF
		+ ErrorText;
	EventLogOperations.AddMessageForEventLog(EventName, EventLogLevel.Error,,, ErrorDescription);
