...
Function ExecuteTranslation(Val InitialObject, Val InitialVersionDescription, Val ResultingVersionDescription) Export
	...
	If InterfaceTranslationChain = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'В конфигурации не зарегистрирован обработчик трансляции из версии %1 в версию %2.'; en = 'Handler of translation from version %1 to version %2 is not registered in the configuration.'; pl = 'W konfiguracji nie jest zarejestrowana procedura przetwarzania przekazania z wersji%1 do wersji%2.';de = 'Anwender der Übersetzung von Version %1 zu Version %2 ist in der Konfiguration nicht registriert.';ro = 'Handler-ul de traducere de la versiunea %1 la versiunea %2 nu este înregistrat în configurație.';tr = '%2Versiyondan %1 versiyona çeviri işleyicisi konfigürasyonda kayıtlı değil.'; es_ES = 'Manipulador de la traducción de la versión %1 para la versión %2 no se ha registrado en la configuración.'"),
...
Function TranslateObject(Val Object, Val InterfaceTranslationRules)
	...
	If InterfaceTranslationChain = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось определить обработчик трансляции для пакета {%1}, невозможно выполнение стандартной трансляции для обработки данного свойства'; en = 'Cannot define a translation handler for package {%1}. Cannot perform the standard translation to process this property'; pl = 'Nie można zdefiniować procedury przetwarzania przekazania dla pakietu {%1}, nie można wykonać standardowego przekazania w celu przetworzenia tej właściwości';de = 'Kann einen Übersetzungsanwender für Paket {%1} nicht definieren, kann die Standardübersetzung nicht ausführen, um diese Eigenschaft zu verarbeiten';ro = 'Nu se poate defini o traducere pentru manipulatorul pachetului {%1}, nu se poate executa traducerea standard pentru procesarea acestei proprietăți';tr = '{%1} Paketi için bir çeviri işleyicisi tanımlanamıyor, bu özelliği işlemek için standart çeviriyi yürütemiyor'; es_ES = 'No se puede definir un manipulador de la traducción para el paquete {%1}, no se puede ejecutar la traducción estándar para procesar esta propiedad'"), 
...
Function StandardProcessing(Val Object, Val ResultingObjectPackage, Val InterfaceTranslationRules)
	...
	Else
		...
		If ResultingObjectType = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось выполнить стандартную обработку трансляции типа %1 в пакет %2: тип %1 не существует в пакете %2.'; en = 'Cannot peform the standard processing for translation of %1 type to package %2: the %1 type does not exist in package %2.'; pl = 'Nie można wykonać standardowego przetwarzania dla tłumaczenia typu %1 na pakiet %2: typ %1 nie istnieje w pakiecie %2.';de = 'Die Standardverarbeitung der Übersetzung vom Typ %1 im Paket %2 konnte nicht ausgeführt werden: Der Typ %1 ist im Paket nicht vorhanden%2.';ro = 'Eșec la executarea procesării standard a translației de tipul %1 în pachetul %2: tipul %1 nu există în pachetul %2.';tr = '%1Paketin %2 türüne standart çeviri işlemi yapılamıyor: %1 pakette %2 tür mevcut değil.'; es_ES = 'No se puede ejecutar el procesamiento de la traducción estándar del tipo %1 al paquete %2: el tipo %1 no existe en el paquete %2.'"),
	...
	For Each Property In ResultingObjectType.Properties Do
		...
		If OriginalProperty = Undefined Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %1.'; en = 'Cannot peform the standard processing for conversion of %1 type to %2 type: property %3 is not defined for type %1.'; pl = 'Nie można wykonać standardowego przetwarzania typu %1 konwersji na typ %2: właściwość %3nie jest zdefiniowana dla typu %1.';de = 'Die Standardverarbeitung des Konvertierungstyps %1 kann nicht in Typ %2 ausgeführt werden: die Eigenschaft %3 ist für den Typ nicht definiert %1.';ro = 'Nu se poate executa procesarea standard a tipului de conversie %1 pentru a tasta %2: proprietatea %3 nu este definită pentru tipul %1.';tr = '%1 türün %2 türüne standart dönüşümü yapılamıyor: %3 türü için özellik %1 tanımlanmamıştır.'; es_ES = 'No se puede ejecutar el procesamiento estándar del tipo de conversión %1 al tipo %2: la propiedad %3 no está definida para el tipo %1.'"),
				"{" + InitialObjectType.NamespaceURI + "}" + InitialObjectType.Name,
	...
	For Each Property In InitialObjectType.Properties Do
		...
		If PropertyToTranslate = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %2.'; en = 'Cannot peform the standard processing for conversion of %1 type to %2 type: property %3 is not defined for type %2.'; pl = 'Nie można wykonać standardowego przetwarzania typu %1 konwersji na typ %2: właściwość %3nie jest zdefiniowana dla typu %2.';de = 'Die Standardverarbeitung des Konvertierungstyps %1 kann nicht in Typ %2 ausgeführt werden: Eigenschaft %3 ist für den Typ nicht definiert %2.';ro = 'Nu se poate executa procesarea standard a tipului de conversie %1 pentru a tasta %2: proprietatea %3 nu este definită pentru tipul %2.';tr = '%1 türün %2 türüne standart dönüşümü yapılamıyor: %3 türü için özellik %2 tanımlanmamıştır.'; es_ES = 'No se puede ejecutar el procesamiento estándar del tipo de conversión %1 al tipo %2: la propiedad %3 no está definida para el tipo %2.'"),
				"{" + InitialObjectType.NamespaceURI + "}" + InitialObjectType.Name,
				"{" + ResultingObjectType.NamespaceURI + "}" + ResultingObjectType.Name,
				Property.LocalName);
		EndIf;
...
Function GetTranslationHandlers()
	...
	MessagesTranslationHandlers = MessageInterfacesSaaS.GetMessageTranslationHandlers();
	CommonClientServer.SupplementArray(TranslationHandlerArray, MessagesTranslationHandlers);
