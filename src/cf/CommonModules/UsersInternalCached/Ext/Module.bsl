...
Function UnavailableRoles(Assignment = "ForUsers", Service = Undefined) Export
	...
	CheckAssignment(Assignment,
		NStr("ru = 'Ошибка в функции НедоступныеРоли общего модуля ПользователиСлужебныйПовтИсп.'; en = 'Error occurred in function UnavailableRoles of the UsersInternalCached common module.'; pl = 'Błąd w funkcji NiedostępneRole modułu ogólnego UżytkownicySłużbowyPonow.Korz.';de = 'Fehler in der Funktion NichtZugänglicheRollen des allgemeinen Moduls BenutzerService WiederhNutzung.';ro = 'Eroare în funcția НедоступныеРоли a modulului general ПользователиСлужебныйПовтИсп.';tr = 'KullanıcılarİşTekr.Yürüt genel modülünün Erişilmeyen Roller işlevinde bir hata oluştu'; es_ES = 'Error en la función UnavailableRoles del módulo común UsersInternalCached.'"));
...
//
Function RolesTree(BySubsystems = True, Assignment = "ForUsers") Export
	
	CheckAssignment(Assignment,
		NStr("ru = 'Ошибка в функции ДеревоРолей общего модуля ПользователиСлужебныйПовтИсп.'; en = 'Error in RolesTree function of UsersInternalCached common module.'; pl = 'Błąd w funkcji DrzewkoRól modułu ogólnego UżytkownicySłużbowyPonow.Korz.';de = 'Fehler in der Funktion BaumRolle des allgemeinen Moduls BenutzerService WiederhNutzung.';ro = 'Eroare în funcția ДеревоРолей a modulului general ПользователиСлужебныйПовтИсп.';tr = 'KullanıcılarİşTekr.Yürüt genel modülünün Roller Ağacı işlevinde bir hata oluştu'; es_ES = 'Error en la función RoleTree del módulo UsersInternalCached.'"));
	...
	If Assignment <> "ForAdministrators"
	   ...
	   AND Assignment <> "BothForUsersAndExternalUsers" Then
		...
		Raise ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Параметр Назначение ""%1"" указан некорректно.
			           |
			           ...
			           |- ""DlaAdministratorów"",
			           |- ""DlaUżytkowników"",
			           |-""DlaZewnętznychUżytkowników"",
			           |- ""WspólnieDlaUżytkownikówIZewnętrznychUżytkowników"".';
			           |de = 'Der Parameter Zweck ""%1"" ist nicht korrekt angegeben.
			           |
			           |Es sind nur folgende Werte erlaubt:
			           |- ""FürAdministratoren"",
			           |- ""FürBenutzer"",
			           | - ""FürExterneBenutzer"",
			           |- ""GemeinsamFürBenutzerUndExterne Benutzer"".';
			           |ro = 'Parametrul Destinația ""%1"" este indicat incorect.
			           |
			           |Sunt admise următoarele valori:
			           |- ""ДляАдминистраторов"",
			           |- ""ДляПользователей"",
			           |- ""ДляВнешнихПользователей"",
			           |- ""СовместноДляПользователейИВнешнихПользователей"".';
