...
//     ValueTable - in SaaS mode, a table of values with properties:
...
Function FindJobs(Filter) Export
	...
	FilterCopy = Common.CopyRecursive(Filter);
...
Procedure DeleteJob(Val ID) Export
	...
	Else
		If TypeOf(ID) = Type("MetadataObject") AND ID.Predefined Then
			Raise( NStr("ru = 'Предопределенное регламентное задание удалить невозможно.'; en = 'Cannot delete a predefined scheduled job.'; pl = 'Usunąć wstępnie zdefiniowane planowe zadanie nie jest możliwie.';de = 'Die vordefinierte geplante Aufgabe kann nicht gelöscht werden.';ro = 'Sarcina reglementară predefinită nu poate fi ștearsă.';tr = 'Önceden tanımlanmış bir rutin görev silinemez.'; es_ES = 'Es imposible eliminar la tarea programada predeterminada.'") );
...
Function GetScheduledJob(Val ID) Export
	...
	If ScheduledJob = Undefined Then
		Raise( NStr("ru = 'Регламентное задание не найдено.
		                              ...
		                              |ro = 'Sarcina reglementară nu a fost găsită.
		                              |Posibil, ea a fost ștearsă de alt utilizator.';
