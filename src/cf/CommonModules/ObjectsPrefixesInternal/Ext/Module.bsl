...
Procedure ChangeIBPrefix(Parameters, ResultAddress = "") Export
	...
	Except
		...
		Raise NStr("ru = 'Не удалось изменить префикс.'; en = 'Cannot change prefix.'; pl = 'Nie udało się zmienić prefiksu.';de = 'Das Präfix konnte nicht geändert werden.';ro = 'Eșec la modificarea prefixului.';tr = 'Önek değiştirilemedi.'; es_ES = 'No se ha podido cambiar el prefijo.'");
...
Function MetadataUsingPrefixesDetails(DiagnosticsMode = False) Export
	...
	IsSAASSubsystem = Common.SubsystemExists("StandardSubsystems.SaaS");
	...
	For Each Subscription In Metadata.EventSubscriptions Do
		...
		For Each SourceType In Subscription.Source.Types() Do
			...
			ObjectDetails.IBPrefixIsUsed = IBPrefixUsed;
			...
			If Characteristics.CodeLength = 0 AND Characteristics.NumberLength = 0 Then
				...
				If Not DiagnosticsMode Then
					...
					Raise StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Ошибка внедрения подсистемы ""%1"" для объекта метаданных ""%2"".'; en = 'Error occurred implementing subsystem %1 for metadata object %2.'; pl = 'Błąd implementacji podsystemu ""%1"" dla obiektu metadanych ""%2"".';de = 'Fehler bei der Implementierung des Subsystems ""%1"" für das Metadatenobjekt ""%2"".';ro = 'Eroare de implementare a subsistemului ""%1"" pentru obiectul de metadate ""%2"".';tr = '%2Meta veri nesnesi için %1alt sistemi uygulama hatası.'; es_ES = 'Error de integrar el subsistema ""%1"" para el objeto de metadatos ""%2"".'"),
...
Function OneKindObjectDataForLastItemsRenumbering(Val ObjectDetails, Val PreviousPrefix = "")
	...
	CompanyFieldName = ?(CompanyPrefixIsUsed,
	...
	QueryText =
	"SELECT
	|	SelectionByDateNumber.Company AS Company,
	...
	|				AND MaxCodes.Company = SelectionByDateNumber.Company)
	|
	|GROUP BY
	|	SelectionByDateNumber.Company,
...
Function NewMetadataUsingPrefixesDetails()
	...
	Result.Columns.Add("IBPrefixIsUsed",          TypesDetailsBoolean);
...
Function EventLogEventReassignObjectsPrefixes()
	...
	Return NStr("ru = 'Префиксация объектов.Изменение префикса информационной базы'; en = 'Object prefixes. Infobase prefix changed.'; pl = 'Prefiks obiektu.Zmiana prefiksu bazy informacyjnej';de = 'Objektpräfixierung. Infobase-Präfixänderung';ro = 'Prefixarea obiectelor.Modificarea prefixului bazei de informații';tr = 'Nesne öneki. Veritabanı öneki değişikliği'; es_ES = 'Vuelta a colocar el prefijo del objeto.Cambio del prefijo de la infobase'",
