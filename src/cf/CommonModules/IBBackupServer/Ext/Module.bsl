...
Function CurrentBackupSetting() Export
	...
	If BackupSettings = Undefined Then
		Return NStr("ru = 'Для настройки резервного копирования необходимо обратиться к администратору.'; en = 'To set up backup, contact your administrator.'; pl = 'Aby skonfigurować rezerwowe kopiowanie, skontaktuj się z administratorem.';de = 'Wenden Sie sich an den Administrator, um die Sicherung zu konfigurieren.';ro = 'Pentru a configura backupul, contactați administratorul.';tr = 'Yedeklemeyi yapılandırmak için yöneticiye başvurun.'; es_ES = 'Para configurar la creación de una copia de respaldo, contactar el administrador.'");
	EndIf;
	
	CurrentSetting = NStr("ru = 'Резервное копирование не настроено, информационная база подвергается риску потери данных.'; en = 'Backup not set up. Set it up to prevent the risk of data loss.'; pl = 'Tworzenie kopii zapasowych nie zostało skonfigurowane. Baza informacyjna może być narażona na utratę danych.';de = 'Die Sicherung ist nicht konfiguriert. dies kann die Infobase dem Risiko von Datenverlust aussetzen.';ro = 'Backup-ul nu este configurat. Poate expune baza de date la riscurile de pierdere a datelor.';tr = 'Yedekleme yapılandırılmamış. Veritabanını veri kaybı risklerine maruz kalabilir.'; es_ES = 'La creación de una copia de respaldo no está configurada. Puede exponer la infobase a riesgos de pérdida de datos.'");
	...
	If Common.FileInfobase() Then
		...
		If BackupSettings.RunAutomaticBackup Then
			...
			If BackupSettings.ExecutionOption = "OnExit" Then
				CurrentSetting = NStr("ru = 'Резервное копирование выполняется регулярно при завершении работы.'; en = 'Backups are created automatically on exit.'; pl = 'Kopia zapasowa jest tworzona regularnie przy zakończeniu pracy aplikacji.';de = 'Die Sicherung wird regelmäßig beim Schließen der Anwendung durchgeführt.';ro = 'Copierea de rezervă se execută regulat la finalizarea lucrului.';tr = 'Yedekleme işlem bitiminde düzenli olarak yapılır. '; es_ES = 'Copia de respaldo se hace regularmente al cerrar la aplicación.'");
			ElsIf BackupSettings.ExecutionOption = "Schedule" Then // On schedule
				...
				If Not IsBlankString(Schedule) Then
					CurrentSetting = NStr("ru = 'Резервное копирование выполняется регулярно по расписанию: %1'; en = 'Backups are created on schedule: %1.'; pl = 'Regularne tworzenie kopii zapasowej wg harmonogramu: %1';de = 'Regelmäßige Sicherung nach Zeitplan: %1';ro = 'Rezervă periodică la timp: %1';tr = 'Düzenli aralıklarla yedekleme:%1'; es_ES = 'Copia de respaldo regular a la hora programada: %1'");
		...
		Else
			...
			If BackupSettings.BackupConfigured Then
				CurrentSetting = NStr("ru = 'Резервное копирование не выполняется (организовано сторонними программами).'; en = 'Backups are created by third-party backup tools.'; pl = 'Tworzenie kopii zapasowych nie jest uruchomione (organizowane przez aplikacje zewnętrzne).';de = 'Die Sicherung wird nicht ausgeführt (organisiert von externen Anwendungen).';ro = 'Backup-ul nu se execută (organizat de aplicații externe).';tr = 'Yedekleme çalışmıyor (harici uygulamalar tarafından düzenlenmiştir).'; es_ES = 'Copia de respaldo no está lanzada (organizado por las aplicaciones externas).'");
			EndIf;
	...
	Else
		...
		CurrentSetting = NStr("ru = 'Резервное копирование не выполняется (организовано средствами СУБД).'; en = 'Backups are created by the means of DBMS.'; pl = 'Tworzenie kopii zapasowych nie jest uruchomione (organizowane przez źródła SZBD).';de = 'Die Sicherung wird nicht ausgeführt (organisiert von DBMS-Ressourcen).';ro = 'Backup-ul nu se execută (organizat de resursele DBMS).';tr = 'Yedekleme çalışmıyor (DBMS kaynakları tarafından düzenlenmiş).'; es_ES = 'Copia de respaldo no está lanzada (organizado por los recursos DBMS).'");
...
Procedure OnFillToDoList(ToDoList) Export
	...
	For Each Section In Sections Do
		...
		If Not DisabledNotificationOfBackupSettings Then
			...
			ToDoItem.Presentation  = NStr("ru = 'Настроить резервное копирование'; en = 'Set up backup'; pl = 'Konfiguracja tworzenia kopii zapasowych';de = 'Konfigurieren Sie die Sicherung';ro = 'Configurați copia de rezervă';tr = 'Yedeklemeyi yapılandır'; es_ES = 'Configurar la creación de una copia de respaldo'");
		...
		If Not DisabledNotificationOfBackupExecution Then
			...
			ToDoItem.Presentation  = NStr("ru = 'Резервное копирование не выполнено'; en = 'Backup required'; pl = 'Tworzenie kopii zapasowej nie powiodło się';de = 'Sicherung nicht abgeschlossen';ro = 'Backup nu a fost finalizată';tr = 'Yedekleme tamamlanmadı'; es_ES = 'La creación de una copia de respaldo no finalizada'");
			ToDoItem.Important         = True;
...
Function BackupParameters() Export
	...
	If Parameters = Undefined Then
		Return InitialBackupSettingsFilling();
	EndIf;
	...
	ElsIf Not Result.BackupConfigured Then
		If NotifyOfBackupNecessity Then	
			...
			If BackupSettings <> Undefined
				AND BackupSettings.User <> Users.CurrentUser() Then
				NotificationOption = "DoNotNotify";
			Else
