...
//
// Parameters:
//  Form - ManagedForm - a form from which the notification data processor was called.
...
//  Object - FormDataStructure - details of the object, to which properties are attached, if the 
//                                  property is not specified or Undefined, the object is taken from 
//                                  the Object form attribute.
//
...
// Checks for dependent additional attributes on the form and attaches an idle handler for checking 
// attribute dependencies if needed.
//
// Parameters:
//  Form - ManagedForm - a form being checked.
//
...
// 
// Parameters:
//  Form - ManagedForm - a form with additional attributes preliminarily set in the 
...
//
// Parameters:
//  Form - ManagedForm - a form the method is called from.
//
	 ...
	 OR NOT ValueIsFilled(Sets[0].Value) Then
		...
		ShowMessageBox(,
			NStr("ru = 'Не удалось получить наборы дополнительных реквизитов объекта.
			           ...
			           |ro = 'Eșec la obținerea seturilor de atribute suplimentare ale obiectului.
			           |
			           |Probabil, atributele necesare nu au fost completate pentru obiect.';
