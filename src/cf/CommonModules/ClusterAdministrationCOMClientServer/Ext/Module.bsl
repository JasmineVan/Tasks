...
#Else
			Raise BriefErrorDescription(ErrorInfo()) + Chars.LF + Chars.LF
				+ StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В случае ошибки о несоответствии версии компоненты ""comcntr"" следует зарегистрировать ее на компьютере %1
					...
					|en = 'If comcntr version mismatch error occurs, register comcntr on computer %1
					|for the Windows user signed in to 1C:Enterprise. Example:
					|regsvr32.exe ""%2\comcntr.dll"".'; 
					...
					|regsvr32.exe ""%2\comcntr.dll""';
					|ro = 'În cazul erorii despre necorespundere a versiunii componentei ""comcntr"" trebuie s-o înregistrați pe computer %1
					|pentru accountul SO Windows, din numele căruia se execută 1С:Enterprise. De exemplu:
					|regsvr32.exe ""%2\comcntr.dll""';
					|tr = '""Comcntr"" bileşeninin sürümünün tutarsızlığı ile ilgili bir hata durumunda, 1C:İşletme adına çalışan %1
					|Windows işletim sistemi hesabını bilgisayara kaydetmelisiniz. Örneğin: 
					|regsvr32.exe ""%2comcntr.dll""'; 
...
#ElsIf MobileClient Then
	Raise NStr("ru = 'Внимание! Администрирование кластера невозможно в мобильном клиенте'; en = 'Warning! The mobile client does not support cluster administration.'; pl = 'Uwaga! Administrowanie klastera jest niemożliwe w kliencie mobilnym';de = 'Achtung! Die Clusterverwaltung ist im mobilen Client nicht möglich.';ro = 'Atenție! Administrarea cluster-ului nu este disponibilă în regim de client mobil';tr = 'Dikkat! Küme yönetimi mobil istemcide güvenli modda kullanılamaz.'; es_ES = '¡Atención! Administración del clúster no está disponible en el cliente móvil'");
#Else
...
Function GetIB(WorkingProcessConnection, Cluster, Val NameInCluster, Val IBAdministratorName, Val IBAdministratorPassword)
	...
	Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1:%2 не найдена информационная база ""%3""'; en = 'The infobase ""%3"" is not found in the server cluster%1:%2.'; pl = 'W klasterze serwerów %1:%2 nie odnaleziono bazy informacyjnej ""%3""';de = 'Im Server-Cluster %1:%2 keine Informationsbasis ""%3"" gefunden';ro = 'Baza de date ""%3"" nu a fost găsită în clusterul serverelor %1: %2';tr = 'Sunucu kümesinde ""%3"" veritabanı bulunamadı%1:%2.'; es_ES = 'En el clúster de servidores %1:%2 no se ha encontrado base de información ""%3""'"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		NameInCluster);
...
Function GetSecurityProfile(ServerAgentConnection, Cluster, ProfileName)
	...
	Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1:%2 не найден профиль безопасности ""%3""'; en = 'The security profile ""%3"" is not found in the server cluster %1:%2.'; pl = 'W klasterze serwerów %1:%2 nie odnaleziono profilu bezpieczeństwa ""%3""';de = 'Im Server-Cluster %1:%2 kein Sicherheitsprofil ""%3"" gefunden';ro = 'Profilul de securitate ""%3"" nu a fost găsit în clusterul serverelor %1: %2';tr = 'Güvenlik profili %2 %1 sunucu kümesinde bulunamadı:%3'; es_ES = 'En el clúster de servidores %1:%2 no se ha encontrado perfil de seguridad ""%3""'"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		ProfileName);
