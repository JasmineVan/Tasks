...
Function MapBetweenErrorHandlersAndAliases() Export
	...
	ErrorsHandlers = New Map;
	
	// Filling the embedded error handlers.
	ErrorsHandlers.Insert("JobQueueInternal.HandleError","JobQueueInternal.CancelErrorHandlerJobs");
	ErrorsHandlers.Insert("JobQueueInternal.CancelErrorHandlerJobs","JobQueueInternal.CancelErrorHandlerJobs");
	
	SSLSubsystemsIntegration.OnDefineErrorsHandlers(ErrorsHandlers);
	JobsQueueOverridable.OnDefineErrorsHandlers(ErrorsHandlers);
	
	Result = New Map;
	For each KeyAndValue In ErrorsHandlers Do
...
Function JobQueueSelectionFields(Val CatalogJob = Undefined) Export
	...
	For each ParameterDetails In JobsQueueInternalCached.QueueJobParameters() Do
		...
		SelectionFieldDescription = "Queue." + ParameterDetails.Field + " AS " + ParameterDetails.Name;
