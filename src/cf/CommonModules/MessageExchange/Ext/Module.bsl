...
Procedure DeliverMessages() Export
	...
	If TransactionActive() Then
		Raise NStr("ru = 'Доставка быстрых сообщений системы не может выполняться в активной транзакции.'; en = 'Quick system message delivery is not available in active transactions.'; pl = 'Nie można odebrać szybkich komunikatów systemowych w aktywnej transakcji.';de = 'Sie können keine schnellen Systemmeldungen in einer aktiven Transaktion erhalten.';ro = 'Nu puteți primi mesaje de sistem rapide în tranzacție activă.';tr = 'Etkin bir işlemde hızlı sistem mesajları alamazsınız.'; es_ES = 'Usted no puede recibir mensajes rápidos del sistema en una transacción activa.'");
	EndIf;
	...
	ElsIf TypeOf(Recipient) = Type("Array") Then
		...
		For Each Item In Recipient Do
			...
			If TypeOf(Item) <> Type("ExchangePlanRef.MessageExchange") Then
				...
				Raise NStr("ru = 'Указан неправильный получатель для метода ОбменСообщениями.ОтправитьСообщение().'; en = 'Invalid recipient is specified for MessageExchange method.SendMessage().'; pl = 'W metodzie MessagesExchange.SendMessage() wskazano nieprawidłowego odbiorcę.';de = 'Für den Nachrichtenaustausch wurde ein falscher Empfänger angegeben. NachrichtSenden() Methode.';ro = 'Destinatar incorect specificat pentru metoda MessagesExchange.SendMessage().';tr = 'MesajAlışverişi.MesajGönder () yöntemi için yanlış alıcı belirtildi.'; es_ES = 'Destinatario incorrecto está especificado para el método MessagesExchange.SendMessage().'");
...
Procedure SendMessageToRecipient(MessagesChannel, MessageBody, Recipient, IsInstantMessage)
	...
	If Not TransactionActive() Then
		...
		Raise NStr("ru = 'Отправка сообщений может выполняться только в транзакции.'; en = 'Message delivery is only available in transactions.'; pl = 'Wysyłanie wiadomości jest możliwe tylko w transakcji.';de = 'Nachrichten können nur in einer Transaktion gesendet werden.';ro = 'Mesajele pot fi trimise numai într-o tranzacție.';tr = 'Mesajlar sadece bir işlemde gönderilebilir.'; es_ES = 'Mensajes pueden enviarse solo en una transacción.'");
		
	EndIf;
	
	If Not ValueIsFilled(MessagesChannel) Then
		
		Raise NStr("ru = 'Не задано значение параметра ""КаналСообщений"" для метода ОбменСообщениями.ОтправитьСообщение.'; en = 'MessageChannel parameter value is not set for MessageExchange method.SendMessage.'; pl = 'Wartość parametru MessagesChannel metody MessagesExchange.SendMessage nie została określona.';de = 'Der Wert des Parameters Nachrichtenkanal ist nicht für den Nachrichtenaustausch angegeben. NachrichtSenden Methode.';ro = 'Valoarea parametrului ""MessagesChannel"" nu este specificată pentru metoda MessagesExchange.SendMessage.';tr = 'MesajKanalı parametresinin değeri, MesajAlışverişi.MesajGönder yöntemi için belirtilmemiş.'; es_ES = 'Valor del parámetro MessagesChannel no está especificado para el método MessagesExchange.SendMessage.'");
		
	ElsIf StrLen(MessagesChannel) > 150 Then
		
		Raise NStr("ru = 'Длина имени канала сообщений не должна превышать 150 символов.'; en = 'The length of message channel name cannot exceed 150 characters.'; pl = 'Długość nazwy kanału wiadomości nie może być dłuższa niż 150 znaków.';de = 'Die Länge des Nachrichtenkanals darf nicht länger als 150 Zeichen sein.';ro = 'Lungimea numelui canalului mesajelor nu poate fi mai mare de 150 de caractere.';tr = 'Mesaj kanalı adının uzunluğu 150 karakterden uzun olamaz.'; es_ES = 'La longitud del nombre del canal de mensajes no puede ser más larga de 150 símbolos.'");
		
	ElsIf Not ValueIsFilled(Recipient) Then
		
		Raise NStr("ru = 'Не задано значение параметра ""Получатель"" для метода ОбменСообщениями.ОтправитьСообщение.'; en = 'Recipient parameter value is not set for MessageExchange method.SendMessage.'; pl = 'Wartość parametru Recipient metody MessagesExchange.SendMessage nie jest określona.';de = 'Der Wert des Empfänger-Parameters ist für den Nachrichtenaustausch nicht angegeben. NachrichtSenden Methode';ro = 'Valoarea parametrului ""Destinatar"" nu este specificată pentru metoda MessagesExchange.SendMessage.';tr = 'Alıcı parametresinin değeri, MesajAlışverişi.MesajGönder yöntemi için belirtilmemiş.'; es_ES = 'Valor del parámetro Destinatario no está especificado para el método MessagesExchange.SendMessage.'");
		
	ElsIf Common.ObjectAttributeValue(Recipient, "Locked") Then
		
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Попытка отправки сообщения заблокированной конечной точке ""%1"".'; en = 'Attempting to send message to locked endpoint %1.'; pl = 'Próba wysłania wiadomości do zablokowanego punktu końcowego ""%1"".';de = 'Versuch, eine Nachricht an den gesperrten Endpunkt zu senden ""%1"".';ro = 'Încercare de a trimite mesajul către punctul final blocat ""%1"".';tr = 'Kilitli uç noktaya ""%1"" mesaj gönderilmeye çalışılıyor.'; es_ES = 'Intentando enviar el mensaje al punto extremo bloqueado ""%1"".'"),
			String(Recipient));
	EndIf;
...
Function StartSendingInstantMessages()
	...
	Try
		Lock = New DataLock;
		Lock.Add("Constant.InstantMessageSendingLocked");
		Lock.Lock();
