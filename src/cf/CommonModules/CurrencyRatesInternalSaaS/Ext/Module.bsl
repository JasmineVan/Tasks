...
Procedure ImportCurrencyRates() Export
	...
	Descriptors = SuppliedData.SuppliedDataDescriptorsFromManager("ExchangeRates");
	
	If Descriptors.Descriptor.Count() < 1 Then
		Raise(NStr("ru = 'В менеджере сервиса отсутствуют данные вида ""КурсыВалют""'; en = 'The service manager has no CurrencyRates data type'; pl = 'Nie ma danych o typie KursyWymiany w menedżerze usług';de = 'Im Service Manager sind keine Daten der Art der Wechselkurse enthalten';ro = 'Nu există date de tipul ""ExchangeRate"" în managerul de servicii';tr = 'Servis yöneticisinde DövizKurları türüne ait veri mevcut değil'; es_ES = 'No hay datos sobre el tipo de ExchangeRates en el gestor de servicios'"));
	EndIf;
...
//   FilePath - a string. Full name of the extracted file.
//
...
Procedure HandleSuppliedRatesPerDay(Val Descriptor, Val PathToFile)
	...
	If RatesDate = "" Then
		Raise NStr("ru = 'Данные вида ""КурсыВалютЗаДень"" не содержат характеристики ""Дата"". Обновление курсов невозможно.'; en = 'Data of CurrencyRatesForDay kind does not contain Data characteristics. Cannot update the rates.'; pl = 'Dane typu ""ExchangeRatesForDay"" nie zawierają charakterystyki ""Date"". Nie można zaktualizować kursów walut.';de = 'Daten vom Typ ""WechselkurseFürTag"" enthalten nicht die ""Datum""-Eigenschaften. Die Kurse können nicht aktualisiert werden.';ro = 'Datele tipului ""ExchangeRatesForDay"" nu conțin caracteristicile ""Date"". Cursurile nu pot fi actualizate.';tr = '""GünlükDövizKurları"" türünün verileri ""Tarih"" özelliklerini içermez. Kurlar güncellenemiyor.'; es_ES = 'Datos del tipo ""ExchangeRatesForDay"" no contienen las características ""Fecha"". No se puede actualizar las tasas.'"); 
	EndIf;
	...
	For each DataArea In AreasForUpdate Do
		...
		Except
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось установить разделение сеанса %1 по причине:
				...
				|%2';
				|ro = 'Eșec la stabilirea separării sesiunii %1 din motivul:
				|%2';
				|tr = 'Oturum %1öğesi 
			...
			WriteLogEvent(NStr("ru = 'Поставляемые данные.Распространение курсов валют по областям данных'; en = 'Supplied data.Currency distribution around data areas'; pl = 'Dostarczane dane.Rozpowszechnianie kursów walut wg obszarów danych';de = 'Zu liefernde Daten. Verbreitung der Wechselkurse nach Datenbereichen';ro = 'Date furnizate.Distribuirea cursurilor valutare pe domeniile de date';tr = 'Sağlanan veriler. Döviz kurlarının veri alanlarına yayılması'; es_ES = 'Datos suministrados.La extensión de los tipos de cambios por las áreas de datos'", Common.DefaultLanguageCode()),
				EventLogLevel.Error,,,
				ErrorText);
		...
		Except
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось обновить курсы валют в области %1 по причине:
				...
				|%2';
				|ro = 'Eșec la actualizarea cursurilor valutelor în domeniul %1 din motivul:
				|%2';
				|tr = 'Aşağıdaki nedenle alandaki döviz kurlarını güncellenemedi %1:
				|%2'; 
	...
	While CurrencySelection.Next() Do
		...
		While CommonSelection.Next() Do
			...
			If Common.SubsystemExists("StandardSubsystems.PeriodClosingDates") Then
				...
				If ModulePeriodClosingDatesInternal.PeriodEndClosingChecked(Metadata.InformationRegisters.ExchangeRates) Then
			...
			If Write Then
				RecordSet.Write();
			Else
				Comment = NStr("ru = 'Загрузка курса валюты %1 на дату %2 отменена из-за нарушения даты запрета изменений.'; en = 'The %1 currency exchange rate import as of %2 is canceled due to change closing date violation.'; pl = 'Pobieranie kursu waluty %1 na dzień %2 jest anulowane z powodu naruszenia daty zakazu zmian.';de = 'Das Herunterladen des Wechselkurses %1 am Tag %2 wird wegen der Verletzung des Datums des Änderungsverbots abgebrochen.';ro = 'Importul cursurilor valutei %1 la data %2 este revocată din cauza încălcării datei de interdicție a modificării.';tr = 'Değişiklik yasağı tarihi ihlal edildiğinden dolayı %1 tarihi itibariyle döviz%2 kuru yüklenemedi.'; es_ES = 'La descarga del tipo de cambio %1 de la fecha %2 ha sido cancelada a causa de la prohibición de los cambios.'"); 
				Comment = StringFunctionsClientServer.SubstituteParametersToString(Comment, CurrencySelection.Code, CommonSelection.Date);
				EventName = NStr("ru = 'Поставляемые данные.Отмена загрузки курсов валюты'; en = 'Supplied data. Cancel import of exchange rates'; pl = 'Dostarczane dane.Anulowanie pobierania kursów waluty';de = 'Gelieferte Daten. Wechselkurse herunterladen abbrechen';ro = 'Date furnizate.Revocarea încărcării cursurilor valutelor';tr = 'Sağlanan veriler.  Döviz kurların içe aktarımı iptal edildi.'; es_ES = 'Datos suministrados.Cancelar la descarga de los tipos de cambio'", Common.DefaultLanguageCode());
