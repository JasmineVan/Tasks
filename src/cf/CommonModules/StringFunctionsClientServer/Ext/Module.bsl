...
Function SplitStringIntoSubstringsArray(Val Value, Val Separator = ",", Val SkipEmptyStrings = Undefined, 
	...
	If Separator = "," 
		...
		Result = StrSplit(Value, ",", False);
		For Index = 0 To Result.UBound() Do
			Result[Index] = TrimAll(Result[Index])
		EndDo;
		Return Result;
	...
	For Position = 1 To StrLen(Value) Do
		...
		Else
			If OnlyUppercaseInString Then 
				LatinChar = Upper(LatinChar); // restoring the case
...
// To create a formatted string, use similarly named functions in common modules
// StringFunctions and StringFunctionsClient that support enhanced features of text organization by style items.
//
...
//  Structure - parameters values, where the key is the name of the parameter, the value is the parameter value.
//
Function GetParametersFromString(Val ParametersString, Val Separator = ";") Export
	Result = New Structure;
...
// Obsolete. Use StringFunctionsClientServerRussia.OnlyLatinInString
...
Function GenerateFormattedString(StringPattern, StyleItems,
	...
	For each Fragment In ParticlesStrings Do
		
		StringBody = "";
		PositionTag = StrFind(Fragment, ">");
		
		If PositionTag = 0 Then
			StringBody = Fragment;
			
		ElsIf StrStartsWith(Fragment, "/") Then
			StringBody = Mid(Fragment, PositionTag + 1);
		...
		Else
			...
			TagDetails = Left(Fragment, PositionTag - 1);
			FirstSpase = StrFind(TagDetails, " ");
			
			If FirstSpase > 0 Then
				TagName = TrimAll(Left(TagDetails, FirstSpase));
				...
				While PositionEqual > 0 Do
					...
					If StrCompare(AttributeName, "style") = 0 Then
						SetStylesByAttributeValue(AttributeValue, StyleItems, CurrentBackground, CurrentColor, CurrentFont);
					ElsIf StrCompare(AttributeName, "href") = 0 AND StrCompare(TagName, "a") = 0 Then
						CurrentRef = AttributeValue;
					ElsIf StrCompare(AttributeName, "src") = 0 AND StrCompare(TagName, "img") = 0 Then
