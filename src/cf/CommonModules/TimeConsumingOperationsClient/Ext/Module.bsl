...
// Obsolete. Please use WaitForCompletion with the IdleParameters.OutputIdleWindow = True parameter.
...
// 
// Parameters:
//  FormOwner        - ManagedForm - the form used to open the time-consuming operation progress form.
//  JobID - UUID - the background job ID.
//
// Returns:
//  ManagedForm     - the reference to the opened form.
// 
...
// 
// Parameters:
//  TimeConsumingOperationForm - ManagedForm - the reference to the time-consuming operation indication form. 
//
Procedure CloseTimeConsumingOperationForm(TimeConsumingOperationForm) Export
	
	If TypeOf(TimeConsumingOperationForm) = Type("ManagedForm") Then
...
Procedure CheckParametersWaitForCompletion(Val TimeConsumingOperation, Val CompletionNotification, Val IdleParameters)
	...
	If IdleParameters <> Undefined Then
		...
		If IdleParameters.OwnerForm <> Undefined Then
			PropertyTypes.Insert("OwnerForm", Type("ManagedForm"));
		EndIf;
		...
		CommonClientServer.Validate(IdleParameters.Interval = 0 Or IdleParameters.Interval >= 1, 
			NStr("ru = 'Параметр ПараметрыОжидания.Интервал должен быть больше или равен 1'; en = 'The IdleParameters.Interval parameter must be equal to or greater than 1.'; pl = 'Opcje IdleParameters.Interval musi być większy lub równy 1';de = 'Parameter IdleParameters.Interval muss größer oder gleich 1 sein.';ro = 'Parametrul IdleParameters.Interval trebuie să fie mai mare sau egal cu 1';tr = 'IdleParameters.Interval parametresi birden fazla veya 1 olmalıdır.'; es_ES = 'El parámetro IdleParameters.Interval debe ser mayor o igual al 1.'"),
		...
		CommonClientServer.Validate(Not (IdleParameters.ExecutionProgressNotification <> Undefined AND IdleParameters.OutputIdleWindow), 
			NStr("ru = 'Если параметр ПараметрыОжидания.ВыводитьОкноОжидания установлен в Истина, то параметр ПараметрыОжидания.ОповещениеОПрогрессеВыполнения не поддерживается'; en = 'If the IdleParameters.OutputIdleWindow parameter is True, the IdleParameters.ExecutionProgressNotification parameter is ignored.'; pl = 'Jeśli parametr WaitingParameters.DisplayWaitingWindow ma wartość True, to parametr WaitingParameters.NotifyAboutExecuteProgress nie jest obsługiwany';de = 'Wenn die Option WarteParameter.Anzeige Wartefenster auf True gesetzt ist, wird die Option WarteParameter.Fortschrittsanzeige nicht unterstützt';ro = 'Dacă parametrul ПараметрыОжидания.ВыводитьОкноОжидания este setat la Истина, atunci parametrul ПараметрыОжидания.ОповещениеОПрогрессеВыполнения nu este susținut';tr = 'BeklemeParametreleri.BeklemePenceresiniGöster parametresi Doğru olarak ayarlanırsa, BeklemeParametreleri.GidişatBildirimi parametresi desteklenmiyor'; es_ES = 'Si el parámetro IdleParameters.OutputIdleWindow es Verdadero, se ignora el parámetro IdleParameters.ExecutionProgressNotification.'"),
			"TimeConsumingOperationsClient.WaitForCompletion");
	EndIf;
