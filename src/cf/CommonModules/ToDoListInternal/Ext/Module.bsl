...
//    * HasToDoList - Boolean - if True, a to-do item is displayed in the user's to-do list.
//    * Important - Boolean - if True, a to-do is highlighted in red.
//    * Presentation - String - a to-do presentation displayed to the user.
	...
	For Each ToDoItem In ToDoList Do
		...
		Else
			If TypeOf(ToDoItem.Owner) = Type("DataProcessorManager.ToDoList") Then
				...
				ToDoItem.ID = StrReplace(ToDoItem.ID, " ", "");
			EndIf;
			
			IsUserTaskID = (ToDoList.Find(ToDoItem.Owner, "ID") <> Undefined);
			If Not IsUserTaskID Then
				IsUserTaskID = (ToDoList.Find(StrReplace(ToDoItem.Owner, " ", ""), "ID") <> Undefined);
			EndIf;
			
			ToDoItem.OwnerID = StrReplace(ToDoItem.Owner, " ", "");
			If Not IsUserTaskID Then
				ToDoItem.IsSection              = True;
				ToDoItem.SectionPresentation   = ToDoItem.Owner;
			EndIf;
		EndIf;
...
Function NewToDoListTable()
	...
	UserTasks.Columns.Add("HasToDoItems", New TypeDescription("Boolean"));
