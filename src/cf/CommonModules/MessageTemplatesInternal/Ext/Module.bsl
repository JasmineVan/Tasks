...
Function GenerateMessageAndSend(SendOptions) Export
	...
	If SendOptions.Template.ForSMSMessages Then
		If Message.Recipient.Count() = 0 Then
			Result.ErrorDescription  = NStr("ru = 'Для отправки сообщения необходимо ввести номер телефонов получателей.'; en = 'To send a message, enter the recipient phone numbers.'; pl = 'To send a message, enter the recipient phone numbers.';de = 'To send a message, enter the recipient phone numbers.';ro = 'To send a message, enter the recipient phone numbers.';tr = 'To send a message, enter the recipient phone numbers.'; es_ES = 'To send a message, enter the recipient phone numbers.'");
			Return Result;
		EndIf;
		...
		If Common.SubsystemExists("StandardSubsystems.SendSMSMessage") Then
			...
			If ModuleSMS.CanSendSMSMessage() Then
				...
				SMSMessageSendingResult = ModuleSMS.SendSMSMessage(RecipientsNumbers, Message.Text, Message.AdditionalParameters.From, Message.AdditionalParameters.Transliterate);
			...
			Else
				...
				Result.ErrorDescription = NStr("ru = 'Сообщение SMS не может быть отправлено сразу.'; en = 'Cannot send SMS at once.'; pl = 'Cannot send SMS at once.';de = 'Cannot send SMS at once.';ro = 'Cannot send SMS at once.';tr = 'Cannot send SMS at once.'; es_ES = 'Cannot send SMS at once.'");
	...
	Else
		If Message.Recipient.Count() = 0 Then
			Result.ErrorDescription  = NStr("ru = 'Сообщение не может быть отправлено сразу, т.к необходимо ввести адрес электронной почты.'; en = 'Cannot send message at once, enter email address.'; pl = 'Cannot send message at once, enter email address.';de = 'Cannot send message at once, enter email address.';ro = 'Cannot send message at once, enter email address.';tr = 'Cannot send message at once, enter email address.'; es_ES = 'Cannot send message at once, enter email address.'");
			Return Result;
		EndIf;
		...
		If Common.SubsystemExists("StandardSubsystems.EmailOperations") Then
			...
			If ModuleEmailOperations.CanSendEmails() Then
				...
				ModuleEmailOperations.SendEmailMessage(Account, EmailParameters);
				Result.Sent = True;
				
			Else
				
				Result.ErrorDescription  = NStr("ru = 'Сообщение не может быть отправлено сразу.'; en = 'Cannot send message at once.'; pl = 'Cannot send message at once.';de = 'Cannot send message at once.';ro = 'Cannot send message at once.';tr = 'Cannot send message at once.'; es_ES = 'Cannot send message at once.'");
				Return Result;
...
Function TemplateInfo(TemplateParameters) Export
	...
	TemplateInfo = New Structure("Attributes, CommonAttributes, Attachments, Purpose");
	TemplateInfo.Attributes = AttributeTree();
	TemplateInfo.Attachments = AttachmentsTable();
	
	MessagesTemplatesSettings = MessagesTemplatesInternalCachedModules.OnDefineSettings();
	TemplateInfo.CommonAttributes = MessagesTemplatesSettings.CommonAttributes;
...
Procedure DefineAttributesAndAttachmentsList(TemplateInfo, TemplateParameters)
	...
	If ValueIsFilled(TemplateParameters.FullAssignmentTypeName) Then
		...
		ObjectRef.Presentation = NStr("ru = 'Ссылка на'; en = 'Ref to'; pl = 'Ref to';de = 'Ref to';ro = 'Ref to';tr = 'Ref to'; es_ES = 'Ref to'") + " """ + Presentation + """";
	...
	For each ArbitraryParameter In TemplateParameters.Parameters Do
		...
		If ArbitraryParameter.Value.TypeDetails.Types().Count() > 0 Then
			...
			Else
				Prefix = NStr("ru='Произвольные'; en = 'Arbitrary'; pl = 'Arbitrary';de = 'Arbitrary';ro = 'Arbitrary';tr = 'Arbitrary'; es_ES = 'Arbitrary'");
				RelatedObjectAttributes = RelatedObjectAttributes(TemplateInfo.Attributes, Prefix);
...
Function TemplatesKinds() Export
	...
	TemplatesTypes.Add("Email", NStr("ru = 'Электронного письма'; en = 'Email'; pl = 'Email';de = 'Email';ro = 'Email';tr = 'Email'; es_ES = 'Email'"));
	TemplatesTypes.Add("SMS", NStr("ru = 'Сообщения SMS'; en = 'Text messages'; pl = 'Text messages';de = 'Text messages';ro = 'Text messages';tr = 'Text messages'; es_ES = 'Text messages'"));
...
Function WriteEmailAttachmentFromTempStorage(Owner, Info, FileName, Size, CountOfBlankNamesInAttachments = 0) Export
	...
	If Common.SubsystemExists("StandardSubsystems.FilesOperations") Then
		...
		If IsBlankString(NameWithoutExtension) Then
			...
			NameWithoutExtension = NStr("ru = 'Вложение без имени'; en = 'Attachment without name'; pl = 'Attachment without name';de = 'Attachment without name';ro = 'Attachment without name';tr = 'Attachment without name'; es_ES = 'Attachment without name'") + ?(CountOfBlankNamesInAttachments = 0, ""," " + String(CountOfBlankNamesInAttachments + 1));
		...
		If AttachmentPrintForm.Status = "PrintForm" AND NameOfParameterWithPrintFormInTemplate <> Undefined Then
			...
			If Common.SubsystemExists("StandardSubsystems.Print") Then
				...
				Except
					...
					WriteLogEvent(
						EventLogEventName(),
						EventLogLevel.Error,,, NStr("ru = 'Ошибка при создании внешней печатной формы. По причине:'; en = 'An error occurred when creating an external print form. Due to:'; pl = 'An error occurred when creating an external print form. Due to:';de = 'An error occurred when creating an external print form. Due to:';ro = 'An error occurred when creating an external print form. Due to:';tr = 'An error occurred when creating an external print form. Due to:'; es_ES = 'An error occurred when creating an external print form. Due to:'") + Chars.LF
							+ DetailErrorDescription(ErrorInformation));
						
					Common.MessageToUser(ErrorInformation.Description) // messages are processed in GenerateMessage
				EndTry;
...
Function FillMessageParameters(TemplateParameters, MessageTextParameters, SendOptions)
	...
	If Topic <> Undefined 
		...
		ObjectName = Topic.Metadata().Name;
		...
		For each ArbitraryTemplateParameter In MessageTextParameters Do
			...
			If ArbitraryParameterValue <> Undefined Then
				...
				If TypeOf(ArbitraryParameterValue) = Type("String")
					...
					EndIf;
				Else
					ObjectName = ArbitraryParameterValue.Metadata().Name;
					If MessageTextParameters[ObjectName] <> Undefined Then
...
Procedure FillAttributesByDCS(Attributes, Topic, TemplateParameters) Export
	...
	For Each Attribute In Attributes Do
		...
		SelectedField.Field = New DataCompositionField(Attribute.Key);
	EndDo;
	...
	If HasBlankParameter Then
		Raise NStr("ru = 'Не удалось сформировать сообщение, т.к. отсутствуют необходимые параметры.'; en = 'Cannot generate the message as the required parameters are missing.'; pl = 'Cannot generate the message as the required parameters are missing.';de = 'Cannot generate the message as the required parameters are missing.';ro = 'Cannot generate the message as the required parameters are missing.';tr = 'Cannot generate the message as the required parameters are missing.'; es_ES = 'Cannot generate the message as the required parameters are missing.'");
	EndIf;
...
Function CommonAttributesTitle() Export
	Return NStr("ru = 'Общие реквизиты'; en = 'Common attributes'; pl = 'Common attributes';de = 'Common attributes';ro = 'Common attributes';tr = 'Common attributes'; es_ES = 'Common attributes'");
EndFunction
...
Function CommonAttributesNodeName() Export
	Return NStr("ru = 'ОбщиеРеквизиты'; en = 'CommonAttributes'; pl = 'CommonAttributes';de = 'CommonAttributes';ro = 'CommonAttributes';tr = 'CommonAttributes'; es_ES = 'CommonAttributes'");
EndFunction
...
Function DetermineCommonAttributes() Export
	...
	AddCommonAttribute(CommonRowAttributes, "CurrentDate", NStr("ru = 'Текущая дата'; en = 'Current date'; pl = 'Current date';de = 'Current date';ro = 'Current date';tr = 'Current date'; es_ES = 'Current date'"), New TypeDescription("Date"));
	AddCommonAttribute(CommonRowAttributes, "SystemTitle", NStr("ru = 'Заголовок системы'; en = 'System title'; pl = 'System title';de = 'System title';ro = 'System title';tr = 'System title'; es_ES = 'System title'"));
	AddCommonAttribute(CommonRowAttributes, "InfobaseInternetAddress", NStr("ru = 'Адрес базы в Интернете'; en = 'Address of the base in the Internet'; pl = 'Address of the base in the Internet';de = 'Address of the base in the Internet';ro = 'Address of the base in the Internet';tr = 'Address of the base in the Internet'; es_ES = 'Address of the base in the Internet'"), New TypeDescription("String"));
	AddCommonAttribute(CommonRowAttributes, "InfobaseLocalAddress", NStr("ru = 'Адрес базы в локальной сети'; en = 'LAN base address'; pl = 'LAN base address';de = 'LAN base address';ro = 'LAN base address';tr = 'LAN base address'; es_ES = 'LAN base address'"), New TypeDescription("String"));
	AddCommonAttribute(CommonRowAttributes, "CurrentUser", NStr("ru = 'Текущий пользователь'; en = 'Current user'; pl = 'Current user';de = 'Current user';ro = 'Current user';tr = 'Current user'; es_ES = 'Current user'"), New TypeDescription("CatalogRef.Users"));
...
Function EventLogEventName()
	...
	Return NStr("ru = 'Формирование шаблона сообщений'; en = 'Create message template'; pl = 'Create message template';de = 'Create message template';ro = 'Create message template';tr = 'Create message template'; es_ES = 'Create message template'", Common.DefaultLanguageCode());
...
Procedure GenerateMessageInBackground(ServerCallParameters, StorageAddress) Export
	...
	Else
		Message = MessageTemplates.GenerateMessage(SendOptions.Template, SendOptions.Topic, SendOptions.UUID, SendOptions.AdditionalParameters);
