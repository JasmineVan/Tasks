	...
	If MessagesSaaS.BodyContainsTypedMessage(MessageBody, Message) Then
		...
		If SaaS.SeparatedDataUsageAvailable()
			...
			If SaaS.SessionSeparatorValue() <> Message.Body.Zone Then
				MessageTemplate = NStr("ru = 'Попытка отправить сообщение от имени области %1 из области %2'; en = 'Message delivery from area %2 on behalf of area %1 is attempted'; pl = 'Próba wysłania wiadomości w imieniu obszaru %1 z obszaru %2';de = 'Versucht, eine Nachricht im Auftrag des Gebiets %1 aus dem Gebiet %2 zu senden';ro = 'Încercarea de a trimite un mesaj în numele zonei %1 din zona %2';tr = 'Alan %1''den alan %2 adına mesaj göndermeye çalışılıyor'; es_ES = 'Intentando enviar un mensaje de parte del área %1 del área %2'");
...
Procedure ProcessAreaKeyInMessage(Message)
	...
	If Not MessageContainsAreaKey Then
		...
		ModuleToCall.MessagesChannelsHandlers(HandlersArray);
		For Each Handler In HandlersArray Do
			
			HandlerMessageType = ModuleToCall.SetDataAreaParametersMessage(
				Handler.Package());
		...
		Else
			...
			If CanCheckAreaKeyInMessages() Then
				...
				If CurrentAreaKey <> Message.Body.ZoneKey Then
					...
					Raise NStr("ru = 'Неверный ключ области данных в сообщении'; en = 'Incorrect data area key in the message'; pl = 'Nieprawidłowy klucz obszaru danych w wiadomości';de = 'Falscher Datenbereichsschlüssel in der Nachricht';ro = 'Cheia incorectă a domeniului de date în mesaj';tr = 'Mesajdaki veri alanının yanlış anahtarı.'; es_ES = 'Clave incorrecta del área de datos en el mensaje'");
...
Function CanCheckAreaKeyInMessages()
	...
	For Each SaaSVersion In SaaSVersions Do
		...
		If MaxVersion = Undefined Then
			MaxVersion = SaaSVersion;
		Else
			MaxVersion = ?(CommonClientServer.CompareVersions(
				SaaSVersion, MaxVersion) > 0, SaaSVersion,
				MaxVersion);
		EndIf;
	...
	Return (CommonClientServer.CompareVersions(MaxVersion, "1.0.4.1") >= 0);
