...
Function SecurityProfile(Val ClusterAdministrationParameters, Val ProfileName) Export
	...
	If SecurityProfiles.Count() <> 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1 не зарегистрирован профиль безопасности %2.'; en = 'The security profile %2 is not registered in the server cluster %1.'; pl = 'W klasterze serwerów %1 nie zarejestrowano profilu bezpieczeństwa %2.';de = 'Im Server-Cluster %1 ist kein Sicherheitsprofil %2 registriert.';ro = 'Profilul de securitate %2 nu este înregistrat în clusterul serverelor %1.';tr = 'Güvenlik profili %2 %1 sunucu kümesinde kayıtlı değil.'; es_ES = 'En el clúster de servidores %1 no se ha registrado perfil de seguridad %2.'"), ClusterID, ProfileName);
	EndIf;
...
Procedure CreateSecurityProfile(Val ClusterAdministrationParameters, Val SecurityProfileProperties) Export
	...
	If SecurityProfiles.Count() = 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1 уже зарегистрирован профиль безопасности %2.'; en = 'The security profile %2 is already registered in the server cluster %1.'; pl = 'W klasterze serwerów %1 już jest zarejestrowany profil bezpieczeństwa %2.';de = 'Im Servercluster %1 ist bereits ein Sicherheitsprofil %2 registriert.';ro = 'Profilul de securitate %2 este deja înregistrat în clusterul serverelor %1.';tr = 'Güvenlik profili %2, %1 sunucu kümesinde zaten kayıtlı.'; es_ES = 'En el clúster de servidores %1 ya se ha registrado perfil de seguridad %2.'"), ClusterID, ProfileName);
	EndIf;
...
Function InfoBaseID(Val ClusterID, Val ClusterAdministrationParameters, Val InfobaseAdministrationParameters) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'В кластере серверов %1 не зарегистрирована информационная база %2.'; en = 'The infobase %2 is not registered in the server cluster %1.'; pl = 'W klasterze serwerów %1 nie zarejestrowano bazy informacyjnej %2.';de = 'Im Server-Cluster %1 ist die Informationsbasis %2 nicht registriert.';ro = 'Baza de date %2 nu este înregistrată în clusterul serverelor %1.';tr = 'Veritabanı %2 %1 sunucu kümesinde kayıtlı değil.'; es_ES = 'En el clúster de servidores %1 no se ha registrado base de información %2.'"), ClusterID, InfobaseAdministrationParameters.NameInCluster);
	EndIf;
...
//
Function ExecuteCommand(Val Template, Val ClusterAdministrationParameters, Val ParametersValues = Undefined)
	...
	#If Server Then
		...
		If SafeMode() <> False Then
			Raise NStr("ru = 'Внимание! Администрирование кластера невозможно в безопасном режиме.'; en = 'Warning! Cluster administration is unavailable in safe mode.'; pl = 'Uwaga! Administrowanie klastera jest niemożliwe w trybie bezpiecznym.';de = 'Achtung! Die Cluster-Administration ist im abgesicherten Modus nicht möglich.';ro = 'Atenție! Administrarea clusterului nu este disponibilă în regim securizat.';tr = 'Dikkat! Küme yönetimi güvenli modda kullanılamaz.'; es_ES = '¡Atención! Administración del clúster no está disponible en el modo seguro.'");
		EndIf;
		
		If Common.DataSeparationEnabled() Then
			Raise NStr("ru = 'Внимание! В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера.'; en = 'Warning! The infobase features related to cluster administration are unavailable in SaaS mode.'; pl = 'Uwaga! W modelu serwisu jest niedostępne wykonanie funkcji administrowania klastera przez zastosowaną bazę informacyjną.';de = 'Achtung! Im Servicemodell ist es inakzeptabel, dass die Anwendungsinformationsbasis Cluster-Administrationsfunktionen ausführt.';ro = 'Atenție! În modelul serviciului nu se admite executarea de către baza de informații aplicată a funcțiilor de administrare a clusterului.';tr = 'Dikkat! Hizmet modeli, küme yönetimi işlevlerinin uygulama veri tabanını çalıştırmak için kullanılmaz.'; es_ES = '¡Atención! En el modelo de servicio no se admite la realización de las funciones de administrar el clúster por la base de información aplicada.'");
		EndIf;
	...
	If Not ClientFile.Exist() Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Невозможно выполнить операцию администрирования кластера серверов по причине: файл %1 не найден.
			      ...
			      |de = 'Es ist nicht möglich, den Vorgang der Server-Cluster-Administration durchzuführen, da die Datei %1 nicht gefunden wurde. 
			      |
			      ...
			      |- Pentru computere cu SO Windows trebuie să reinstalați platforma cu instalarea componentelor ""Administrarea componentelor serverului 1C:Enterprise"";
	...
	If ValueIsFilled(ParametersValues) Then
		For Each Parameter In ParametersValues Do
