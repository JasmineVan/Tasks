...
Function TemplateFromBinaryData(BinaryTemplateData) Export
	...
	If NOT Extension = "docx" Then
		WriteEventsToEventLog(EventLogEvent(), "Error", NStr("ru = 'Макет шаблона офисного документа имеет не верный формат.'; en = 'Invalid layout format for MS Word template.'; pl = 'Makieta szablonu dokumentu biurowego ma nieprawidłowy format.';de = 'Das Layout der Office-Dokumentvorlage ist nicht korrekt.';ro = 'Macheta șablonului documentului office are format incorect.';tr = 'Ofis belgenin şablonun maketinin biçimi yanlış.'; es_ES = 'Modelo de la plantilla del documento de oficina no es de formato incorrecto.'"));
		Raise(NStr("ru = 'Ошибка анализа макета шаблона. Макет шаблона офисного документа имеет не верный формат.'; en = 'Layout analysis error. Invalid layout format for MS Word template.'; pl = 'Błąd analizy makiety szablonu. Makieta szablonu dokumentu biurowego ma nieprawidłowy format.';de = 'Fehler in der Layout-Analyse. Das Layout einer Vorlage eines Office-Dokuments hat ein falsches Format.';ro = 'Eroare de analiză a machetei șablonului. Macheta șablonului documentului office are format incorect.';tr = 'Şablon maketin analiz hatası.  Ofis belgesinin şablon maketin biçimi yanlış.'; es_ES = 'Error al analizar el modelo de la plantilla. El modelo de la plantilla del documento de oficina no es de formato incorrecto.'"));
	EndIf;
...
Procedure CloseConnection(PrintForm) Export
	...
	Except
		...
		Raise(NStr("ru = 'Не удалось удалить временный каталог шаблона печатной формы по причине:'; en = 'Failed to delete temporary directory where print form template is stored. Reason:'; pl = 'Nie udało się usunąć katalog tymczasowy szablonu formularzu wydruku z powodu:';de = 'Der temporäre Katalog der gedruckten Formularvorlage konnte aus diesem Grund nicht gelöscht werden:';ro = 'Eșec la ștergerea directorului temporar al șablonului formei de tipar din motivul:';tr = 'Yazdırma formun şablonun geçici dizini aşağıdaki nedenle silinemedi:'; es_ES = 'No se ha podido eliminar el catálogo temporal de la plantilla del formulario de impresión a causa de:'") + Chars.LF 
...
Procedure ParseDOCXDocumentContainer(Val FullFileName, Val FileStructurePath)
	...
	Except
		...
		WriteEventsToEventLog(EventLogEvent(), "Error", DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось открыть файла шаблона по причине:'; en = 'Cannot open template file. Reason:'; pl = 'Nie udało się otworzyć plik szablonu z powodu:';de = 'Die Vorlagendatei konnte aus diesem Grund nicht geöffnet werden:';ro = 'Eșec la deschiderea fișierului șablonului din motivul:';tr = 'Şablon dosyası aşağıdaki nedenle açılamaz:'; es_ES = 'No se ha podido abrir el archivo de la plantilla a causa:'") + Chars.LF 
			+ BriefErrorDescription(ErrorInfo()));
	EndTry;
	...
	Except
		...
		DeleteFiles(FullFileName);
		WriteEventsToEventLog(EventLogEvent(), "Error", DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось выполнить разбор файла шаблона по причине:'; en = 'Cannot parse template file. Reason:'; pl = 'Nie udało się przeanalizować pliku szablonu z powodu:';de = 'Die Vorlagendatei konnte aus diesem Grund nicht analysiert werden:';ro = 'Eșec la parsarea fișierului șablonului din motivul:';tr = 'Şablon dosyası aşağıdaki nedenle detaylandırılamadı:'; es_ES = 'No se ha podido analizar la plantilla del modelo a causa de:'") + Chars.LF 
			+ BriefErrorDescription(ErrorInfo()));
	EndTry;
...
Procedure AssembleDOCXDocumentContainer(Val FullFileName, Val FileStructurePath)
	...
	Except
		WriteEventsToEventLog(EventLogEvent(), "Error", DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось создать файл документа по причине:'; en = 'Cannot create MS Word document. Reason:'; pl = 'Nie udało się utworzyć pliku dokumentu z powodu:';de = 'Aus diesem Grund konnte keine Dokumentdatei erstellt werden:';ro = 'Eșec la crearea fișierului documentului din motivul:';tr = 'Belge dosyası aşağıdaki nedenle oluşturulamadı:'; es_ES = 'No se ha podido crear el archivo del documento a causa de:'") + Chars.LF 
			+ BriefErrorDescription(ErrorInfo()));
	EndTry;
	...
	Except
		WriteEventsToEventLog(EventLogEvent(), "Error", DetailErrorDescription(ErrorInfo()));
		Raise(NStr("ru = 'Не удалось сформировать файл документа по причине:'; en = 'Cannot generate MS Word document. Reason:'; pl = 'Nie udało się sformować pliku dokumentu z powodu:';de = 'Die Dokumentdatei konnte aus diesem Grund nicht erstellt werden:';ro = 'Eșec la generarea fișierului documentului din motivul:';tr = 'Belgenin dosyası aşağıdaki nedenle oluşturulamadı:'; es_ES = 'No se ha podido generar el archivo del documento a causa de:'") + Chars.LF 
			+ BriefErrorDescription(ErrorInfo()));
	EndTry;
	...
	DataWriter.Write(DataReadingResult.GetBinaryData());
	DataWriter.Close();
...
Procedure WriteTextToStreams(XMLParseStructure, XMLReader, StreamName = "", Val Text = Undefined)
	...
	For Each StreamItem In XMLParseStructure.WriteStreams Do
		...
		RowsCount = ?(IsBlankString(Text), 1, StrLineCount(Text));
...
Function EventLogEvent()
	...
	Return NStr("ru = 'Печать'; en = 'Print'; pl = 'Drukuj';de = 'Drucken';ro = 'Forme de listare';tr = 'Yazdır'; es_ES = 'Impresión'", Common.DefaultLanguageCode());
