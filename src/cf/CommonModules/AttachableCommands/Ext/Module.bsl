...
//
// Parameters:
//   Form - ManagedForm - a form, where the commands are to be placed.
//   PlacementParameters - Structure - command placement parameters.
//       It is used when a form has several lists (in this case, several calls of this procedure are 
//       placed, specifying the second parameter), or when source types depend on the form opening 
//       parameters.
		...
		If SourcesCommaSeparated = "" Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'В формах отчетов, обработок и в общих формах
					...
					|ro = 'În formele rapoartelor, procesărilor și în formele comune
					|la apelarea procedurii ""%1""
					|trebuie să indicați explicit parametrul ""%2""';
...
//       * Sources - TypesDetails, Array - command sources.
...
Procedure ExecuteCommand(Val Form, Val CallParameters, Val Source, Result) Export
	...
	If TypeOf(CallParameters) <> Type("Structure")
		...
		Or TypeOf(Form) <> Type("ManagedForm") Then
		Return;
	EndIf;
...
//   ComparisonType            - DataCompositionComparisonType - a value comparison type.
...
//   InterfaceSettings - Arbitrary - the function execution result.
...
Function AttachableObjectSettings(FullName, InterfaceSettings = Undefined) Export
	...
	Except
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось прочитать настройки объекта %1 из модуля менеджера:'; en = 'Cannot read %1 object settings from the manager module:'; pl = 'Nie udało się przeczytać ustawienia obiektu %1 z modułu menedżera:';de = 'Es war nicht möglich, die Objekteinstellungen %1 aus dem Manager-Modul zu lesen:';ro = 'Eșec la citirea setării obiectului %1 din modulul managerului:';tr = 'Yönetici modülünden nesne ""%1"" ayarları okunamıyor:'; es_ES = 'No se ha podido leer los ajustes del objeto %1 del módulo del gestor:'"), FullName);
		ErrorText = ErrorText + Chars.LF + DetailErrorDescription(ErrorInfo());
		
		WriteLogEvent(
			NStr("ru = 'Подключаемые команды'; en = 'Attachable commands'; pl = 'Podłączane polecenia';de = 'Plug-in-Befehle';ro = 'Comenzile conectate';tr = 'Bağlanabilir komutlar'; es_ES = 'Comandos conectados'", Common.DefaultLanguageCode()),
			EventLogLevel.Error,
	...
	For Each Setting In InterfaceSettings Do
		...
		Except
			ErrorText = NStr("ru = 'Не удалось зарегистрировать настройку программного интерфейса подключаемых объектов.
				...
				|pl = 'Nie można zarejestrować ustawienia interfejsu aplikacji obiektów dołączalnych.
				...
				|ro = 'Nu se poate înregistra setarea interfeței de aplicații pentru obiecte atașabile. 
				|Cheie: ""%1"", descriere tip: ""%2"", detalii eroare: ""%3"".';
...
Procedure CheckCommandsKindName(KindName)
	...
	Except
		ErrorText = NStr("ru = 'Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.'; en = 'Command kind name ""%1"" does not meet naming requirements for variables.'; pl = 'Nazwa rodzaju poleceń ""%1"" nie odpowiada wymaganiom mianowania zmiennych.';de = 'Der Name des Befehlstyps ""%1"" erfüllt nicht die Anforderungen für die Benennung von Variablen.';ro = 'Numele tipului comenzilor ""%1"" nu satisface cerințele de atribuire a numelor variabilelor.';tr = '""%1"" komut türü adı değişkenleri isimlendirme şartlarına uygun değil.'; es_ES = 'Nombre del tipo de comandos ""%1"" no satisfecha a los requerimientos de renombrar los variables.'");
...
Procedure OutputCommand(Form, Commands, PlacementParameters)
	...
	If CapCommand = Undefined Then
		...
		CapCommand.Title = NStr("ru = '(нет)'; en = '(no)'; pl = '(nie ma)';de = '(nein)';ro = '(nu)';tr = '(yok)'; es_ES = '(no)'");
	EndIf;
...
Function RegisterSubmenu(Items, InfoOnAllSubmenus, SubmenuName, NewSubmenuTemplate = Undefined, CommandBar = Undefined, SubmenuByDefault = Undefined)
	...
	If ValueIsFilled(SubmenuName) Then
		...
		If Not Groups.Property("Important") Then
			...
			GroupImportant.Title = Submenu.Title + " (" + NStr("ru = 'Важное'; en = 'Important'; pl = 'Ważne';de = 'Wichtig';ro = 'Important';tr = 'Önemli'; es_ES = 'Importante'") + ")";
		...
		If Not Groups.Property("Ordinary") Then
			...
			DefaultGroup.Title = Submenu.Title + " (" + NStr("ru = 'Обычное'; en = 'Standard'; pl = 'Standardowy';de = 'Standard';ro = 'Standard';tr = 'Standart'; es_ES = 'Estándar'") + ")";
		...
		If Not Groups.Property("SeeAlso") Then
			...
			GroupSeeAlso.Title = Submenu.Title + " (" + NStr("ru = 'См. также'; en = 'See also:'; pl = 'Zobacz również';de = 'Siehe auch';ro = 'Mai vezi';tr = 'Ayrıca bakınız'; es_ES = 'Ver también'") + ")";
...
Function CommandDetails(CommandNameInForm, SettingsAddress) Export
	...
	If Command = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Сведения о команде ""%1"" не найдены.'; en = 'Cannot find Information on the ""%1"" command.'; pl = 'Informacje o poleceniu ""%1"" nie znaleziono.';de = 'Informationen zum Befehl ""%1"" nicht gefunden.';ro = 'Nu au fost găsite datele despre comanda ""%1"".';tr = '""%1"" grup bilgileri bulunamadı.'; es_ES = 'La información del comando ""%1"" no encontrada.'"),
			CommandNameInForm);
	EndIf;
		...
		If ValueIsFilled(CommandDetails.Handler) Then
			...
			If SubstringCount = 2 Then
				...
				If ModuleCommonMetadataObject = Undefined Then
					Raise StringFunctionsClientServer.SubstituteParametersToString(
						NStr("ru = 'Общий модуль ""%1"" не найден.'; en = 'Cannot find the ""%1"" common module.'; pl = 'Nie znaleziono wspólnego modułu ""%1"".';de = 'Gemeinsames Modul ""%1"" wurde nicht gefunden.';ro = 'Modulul comun ""%1"" nu a fost găsit.';tr = 'Ortak modül ""%1"" bulunamadı.'; es_ES = 'Módulo común ""%1"" no se ha encontrado.'"),
						ModuleName);
				EndIf;
