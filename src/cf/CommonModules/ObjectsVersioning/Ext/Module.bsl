...
Procedure UpdateObjectVersioningSettings() Export
	...
	While RecordsSelection.Next() Do
		If VersionedObjects.Find(RecordsSelection.ObjectType) = Undefined Then
	...
	For Each ObjectType In VersionedObjects Do
		VersionedObjectsVT.Add().ObjectType = ObjectType;
	EndDo;
...
Procedure EnableObjectVersioning(ObjectName, Val VersioningMode = Undefined) Export
	...
	ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
		NStr("ru = 'Неизвестный вариант версионирования: ""%1""'; en = 'Unknown versioning option: %1.'; pl = 'Nieznany wariant wersjonowania: ""%1""';de = 'Unbekannte Version der Versionsverwaltung: ""%1""';ro = 'Variantă de versionare necunoscută: ""%1""';tr = 'Bilinmeyen sürüm seçeneği: ""%1""'; es_ES = 'Variante desconocido de versionar: ""%1""'"), VersioningMode);
...
Procedure UpdateObjectsVersionsInformation(Parameters) Export
	...
	While Selection.Next() Do
		...
		Except
			WriteLogEvent(
				NStr("ru = 'Версионирование'; en = 'Versioning'; pl = 'Zarządzanie wersjami';de = 'Versionierung';ro = 'Versiunilor';tr = 'Sürüm belirleme'; es_ES = 'Versionando'", Common.DefaultLanguageCode()),
				...
				StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Не удалось обновить сведения о версии №%1 объекта ""%2"" по причине:
					|%3'; 
					|en = 'Cannot update information on version #%1 of object %2. Reason:
					|%3'; 
					|pl = 'Nie udało się zaktualizować infomacje o wersji nr. %1 obiektu ""%2"" z powodu:
					|%3';
					|de = 'Es war nicht möglich, die Versionsnummer %1des Objekts ""%2"" zu aktualisieren, da:
					|%3';
					|ro = 'Eșec la actualizarea informațiilor despre versiunea nr.%1 a obiectului ""%2"" din motivul:
					|%3';
	...
	If Selection.Count() > 0 Then
		If ProcessedRecords = 0 Then
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Процедуре ОбновитьСведенияОВерсияхОбъектов не удалось обработать некоторые записи регистра сведений ВерсииОбъектов (пропущены): %1'; en = 'UpdateObjectVersionInformation procedure failed to process some records from the ObjectVersions register: %1'; pl = 'Procedurze ОбновитьСведенияОВерсияхОбъектов nie udało się przetworzyć niektóre wpisy rejestru informacji ВерсииОбъектов (pominięte): %1';de = 'Die Prozedur UpdateInformationsObjektversionen konnte einige Informationsregistersätze der Objektversion nicht verarbeiten (weggelassen): %1';ro = 'Procedura ActualizareInformațiileDespreVersiunileObiectelor nu a reușit să proceseze unele înregistrări ale registrului de date VersiunileObiectelor (omise): %1';tr = 'NesneSürümBilgileriGüncelle işlemi NesneSürümü kayıt günlüğün bazı kayıtlarını işleyemedi (atlattı): %1'; es_ES = 'El procedimiento UpdateObjectVersionInformation no ha podido procesar algunos registros del registro de información ObjectVersions: %1'"), 
					Selection.Count());
			Raise MessageText;
		EndIf;
...
Procedure OnReceiveDataFromSlave(DataItem, GetItem, SendBack, Sender) Export
	...
	If TypeOf(DataItem) = Type("InformationRegisterRecordSet.ObjectsVersions") AND DataItem.Count() > 0 Then
		...
		SerialNumberOfVersionToSynchronize = DataItem.Filter.VersionNumber.Value - DataItem[0].BeforeAfter;
		OwnerVersionSerialNumber = DataItem[0].VersionOwner;
		...
		If ValueIsFilled(OwnerVersionSerialNumber) Then
			DataItem[0].VersionOwner = VersionNumberInRegister(Object, OwnerVersionSerialNumber);
		EndIf;
		...
		If Not HasConflict Then
			WriteVersionWithNumberChange(DataItem, GetItem, Sender, VersionNumber);
			Return;
		EndIf;
...
Procedure OnSendDataToSlave(DataItem, ItemSending, InitialImageCreation, Recipient) Export
	...
	OnSendDataToRecipient(DataItem, ItemSending, Recipient);
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Обновление сведений о записанных версиях объектов. После выполнения обновления увеличится скорость записи версионируемых объектов.'; en = 'Update information about written object versions. After updating, writing of versioned objects will be faster.'; pl = 'Aktualizacja informacji o zapisanych wersjach obiektów. Po wykonaniu aktualizacji zwiększy się prędkość zapisu wersjonowanych obiektów..';de = 'Aktualisierung von Informationen über aufgezeichnete Versionen von Objekten. Nach dem Update erhöht sich die Geschwindigkeit der Aufzeichnung der versionierten Objekte.';ro = 'Actualizarea informațiilor despre versiunile înregistrate ale obiectelor. După actualizare se va mări viteza de înregistrare a obiectelor versionate.';tr = 'Nesnelerin kaydedilmiş sürümleri hakkındaki bilgileri güncelleştirin. Yükseltmeyi gerçekleştirdikten sonra, yükseltilmiş nesnelerin yazma hızı artar.'; es_ES = 'El procedimiento de la información de las versiones guardadas de los objetos. Al actualizar se aumentará la velocidad de guardar los objetos versionados.'");
	
	Handler = Handlers.Add();
	Handler.Version = "2.2.4.13";
	Handler.Procedure = "ObjectsVersioning.MoveVersioningSettings";
	Handler.Comment = NStr("ru = 'Обновление настроек версионирования.'; en = 'Versioning setting update.'; pl = 'Aktualizacja ustawień wersjonowania.';de = 'Aktualisieren der Versionierungseinstellungen.';ro = 'Actualizarea setărilor de versionare.';tr = 'Sürüm ayarlarını güncelleme.'; es_ES = 'Actualización de los ajustes de versionado.'");
...
Procedure OnFillToDoList(ToDoList) Export
	...
	Tooltip = NStr("ru = 'Устаревших версий: %1 (%2)'; en = 'Obsolete versions: %1 (%2)'; pl = 'Przestarzałe wersje: %1 (%2)';de = 'Veraltete Versionen: %1 (%2)';ro = 'Versiuni învechite: %1 (%2)';tr = 'Eski sürümler: %1 (%2)'; es_ES = 'Versiones obsoletas: %1 (%2)'");
	...
	For Each Section In Sections Do
		...
		ToDoItem.Presentation = NStr("ru = 'Устаревшие версии объектов'; en = 'Obsolete object versions'; pl = 'Przestarzałe wersje obiektów';de = 'Veraltete Objektversionen';ro = 'Versiuni obiecte perimate';tr = 'Eski nesne sürümleri'; es_ES = 'Versiones del objeto obsoleto'");
		...
		If Not Object.IsNew() AND (PostingChanged AND ObjectVersionInfo.VersionNumber > 1 Or CurrentAndPreviousVersionMismatch(Object)) Then
			...
			If ObjectVersionInfo.VersionNumber = 1 Then
				If ObjectIsVersioned(Object.Ref, False) Then
					...
					VersionParameters.Insert("Comment", NStr("ru = 'Версия создана по уже имеющемуся объекту'; en = 'Version was created from existing object.'; pl = 'Wersja została utworzona z istniejącego obiektu';de = 'Die Version wurde aus dem vorhandenen Objekt erstellt';ro = 'Versiunea a fost creată din obiectul existent';tr = 'Sürüm mevcut nesneden oluşturuldu'; es_ES = 'Versión se ha creado desde el objeto existente'"));
...
Procedure CheckObjectEditRights(MetadataObject)
	...
	If Not PrivilegedMode() AND Not AccessRight("Update", MetadataObject)Then
		MessageText = NStr("ru = 'Недостаточно прав на изменение ""%1"".'; en = 'Insufficient rights to modify %1.'; pl = 'Niewystarczające uprawnienia do zmiany ""%1"".';de = 'Unzureichende Rechte zum Ändern ""%1"".';ro = 'Drepturi insuficiente de modificare ""%1"".';tr = '""%1"" değiştirmek için haklar yetersiz.'; es_ES = 'Insuficientes derechos para cambiar ""%1"".'");
...
Function ReportOnObjectVersion(ObjectRef, Val ObjectVersion = Undefined, CustomVersionNumber = Undefined) Export
	...
	Details = StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '№ %1 / (%2) / %3'; en = '#%1 / (%2) / %3'; pl = '№ %1 / (%2) / %3';de = '№ %1 / (%2) / %3';ro = 'Nr. %1 / (%2) / %3';tr = '№ %1 / (%2) / %3'; es_ES = '№ %1 / (%2) / %3'"), CustomVersionNumber,
...
Function RestoreObjectByXML(ObjectData, ErrorMessageText = "")
	...
	Except
		WriteLogEvent(NStr("ru = 'Версионирование'; en = 'Versioning'; pl = 'Zarządzanie wersjami';de = 'Versionierung';ro = 'Versiunilor';tr = 'Sürüm belirleme'; es_ES = 'Versionando'", Common.DefaultLanguageCode()),
		...
		ErrorMessageText = NStr("ru = 'Не удалось перейти на выбранную версию.
											...
											|de = 'Fehler beim Fortfahren mit der ausgewählten Version. 
											...
											|ro = 'Eșec de trecere la versiunea selectată.
											| Cauza posibilă: versiunea obiectului a fost înregistrată în altă versiune a aplicației.
											|Informații tehnice despre eroare: %1';
...
Function ObjectVersionInfo(Val Ref, Val VersionNumber) Export
	MessageCannotGetVersion = NStr("ru = 'Не удалось получить предыдущую версию объекта.'; en = 'Cannot get previous version of the object.'; pl = 'Nie można odebrać poprzedniej wersji obiektu.';de = 'Kann eine vorherige Version des Objekts nicht empfangen.';ro = 'Eșec la obținerea versiunii precedente a obiectului.';tr = 'Nesnenin önceki sürümü alınamıyor.'; es_ES = 'No se puede recibir una versión previa del objeto.'");
	...
	If Result.ObjectVersion = Undefined Then
		Raise NStr("ru = 'Выбранная версия объекта отсутствует в программе.'; en = 'Selected object version is not available in the application.'; pl = 'W aplikacji brakuje wybranej wersji obiektu.';de = 'Die ausgewählte Objektversion fehlt in der Anwendung.';ro = 'Versiunea selectată a obiectului lipsește în aplicație.';tr = 'Seçilen nesne sürümü uygulamada eksik.'; es_ES = 'La versión del objeto seleccionado está faltando en la aplicación.'");
	EndIf;
...
Procedure OnSendDataToRecipient(DataItem, ItemSending, Recipient)
	...
	If TypeOf(DataItem) = Type("InformationRegisterRecordSet.ObjectsVersions") AND DataItem.Count() > 0 Then
		...
		If Common.SubsystemExists("StandardSubsystems.DataExchange") Then
			...
			MetadataObjectDetails = ModuleDataExchangeCached.ExchangePlanContent(ExchangePlanName).Find(TypeOf(Record.Object), "Type");
		...
		Record.BeforeAfter = NumberOfUnsynchronizedVersions(Record.Object, Record.VersionNumber);
...
Function QueryOnObsoleteVersions(QueryText)
	...
	For Index = 0 To ObjectDeletionBoundaries.Count() - 1 Do
		If Not IsBlankString(AdditionalConditions) Then
			...
			|	OR";
		EndIf;
		IndexAsString = Format(Index, "NZ=0; NG=0");
		Condition = "";
		...
		Condition,
		IndexAsString);
		Query.SetParameter("TypesList" + IndexAsString, ObjectDeletionBoundaries[Index].TypesList);
		Query.SetParameter("DeletionBoundary" + IndexAsString, ObjectDeletionBoundaries[Index].DeletionBoundary);
	EndDo;
...
Function DataSizeString(Val DataSize)
	...
	UnitOfMeasure = NStr("ru = 'байт'; en = 'byte'; pl = 'bajt';de = 'Byte';ro = 'byte';tr = 'bayt'; es_ES = 'byte'");
	If 1024 <= DataSize AND DataSize < 1024 * 1024 Then
		DataSize = DataSize / 1024;
		UnitOfMeasure = NStr("ru = 'Кб'; en = 'KB'; pl = 'KB';de = 'KB';ro = 'Kb';tr = 'Kb'; es_ES = 'kB'");
	ElsIf 1024 * 1024 <= DataSize AND  DataSize < 1024 * 1024 * 1024 Then
		DataSize = DataSize / 1024 / 1024;
		UnitOfMeasure = NStr("ru = 'Мб'; en = 'MB'; pl = 'MB';de = 'MB';ro = 'MB';tr = 'MB'; es_ES = 'MB'");
	ElsIf 1024 * 1024 * 1024 <= DataSize Then
		DataSize = DataSize / 1024 / 1024 / 1024;
		UnitOfMeasure = NStr("ru = 'Гб'; en = 'GB'; pl = 'GB';de = 'GB';ro = 'Gb';tr = 'Gb'; es_ES = 'GB'");
	EndIf;
	...
	Return StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = '%1 %2'; en = '%1 %2'; pl = '%1 %2';de = '%1 %2';ro = '%1 %2';tr = '%1 %2'; es_ES = '%1 %2'"), DataSize, UnitOfMeasure);
...
Function GoToVersionServer(Ref, VersionNumber, ErrorMessageText, UndoPosting = False) Export
	...
	Object.AdditionalProperties.Insert("ObjectsVersioningVersionComment",
		StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Выполнен переход к версии №%1 от %2'; en = 'Migration to version #%1, %2 is completed.'; pl = 'Wykonano przejście do wersji nr %1 z %2';de = 'Die Übertragung auf die Version #%1 von %2 wurde durchgeführt.';ro = 'Transferul la versiunea nr.%1 din %2 este efectuat';tr = 'No.%1 ila %2 sürümü transferi gerçekleştirilir'; es_ES = 'Traslado a la versión #%1 de %2 se ha realizado'"),
...
Procedure CreateRejectedItemsOwnerVersion(LastRejectedVersionNumber, ConflictVersionNumber, Object, VersionAuthor)
	...
	Record.VersionNumber = ConflictVersionNumber;
	Record.VersionDate = CurrentSessionDate();
	Record.Comment = NStr("ru = 'Отклоненная версия (автоматическое разрешение конфликта).'; en = 'Rejected version (automatic conflict resolving).'; pl = 'Odrzucona wersja (automatyczne rozwiązywanie konfliktów).';de = 'Abgelehnte Version (automatische Konfliktlösung).';ro = 'Versiunea respinsă (soluționarea automată a conflictului).';tr = 'Reddedilen sürüm (otomatik çakışma çözümü).'; es_ES = 'Versión denegada (resolución automática del conflicto).'");
	Record.VersionAuthor = VersionAuthor;
...
Procedure WriteVersionWithNumberChange(DataItem, GetItem, Sender, VersionNumber)
