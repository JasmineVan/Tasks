...
Function DatesByCalendar(Val WorkSchedule, Val DateFrom, Val DaysArray, Val CalculateNextDateFromPrevious = False, RaiseException = True) Export
	...
	If Not ValueIsFilled(WorkSchedule) Then
		If RaiseException Then
			Raise NStr("ru = 'Не указан график работы или производственный календарь.'; en = 'Work schedule or business calendar is not specified.'; pl = 'Nie jest wskazany harmonogram pracy lub kalendarz biznesowy.';de = 'Es ist kein Arbeitszeitplan oder Produktionskalender angegeben.';ro = 'Programul de lucru sau calendarul de afaceri nu este indicat.';tr = 'Çalışma takvimi veya üretim takvimi belirtilmedi.'; es_ES = 'No se ha indicado el horario o el calendario laboral.'");
		EndIf;
		Return Undefined;
	EndIf;
	...
	While Selection.Next() Do
		If Selection.DateByCalendar = Undefined Then
			ErrorMessage = NStr("ru = 'Производственный календарь «%1» не заполнен с даты %2 на указанное количество рабочих дней.'; en = 'Cannot determine the date because business calendar ""%1"" is not filled for the specified number of workdays after %2.'; pl = 'Kalendarz biznesowy «%1» nie jest wypełniony od daty %2 nie jest wskazana ilość dni roboczych.';de = 'Der Produktionskalender ""%1"" wird nicht ab dem Datum %2 der angegebenen Anzahl von Arbeitstagen ausgefüllt.';ro = 'Calendarul de afaceri ""%1"" nu este completat de la data %2 pentru numărul specificat de zile lucrătoare.';tr = '""%1"" İş takvimi belirtilen iş günü için belirtilen %2 itibaren doldurulmamıştır.'; es_ES = 'El ""%1"" calendario laboral no está rellenado desde la fecha %2 para el número especificado de los días laborales.'");
			If RaiseException Then
...
Function DateDiffByCalendar(Val WorkSchedule, Val StartDate, Val EndDate, RaiseException = True) Export
	...
	If Result.IsEmpty() Then
		If RaiseException Then
			ErrorMessage = NStr("ru = 'График работы «%1» не заполнен на период %2.'; en = 'The ""%1"" work schedule is blank for period: %2.'; pl = 'Harmonogram pracy «%1» nie jest wypełniony na okres %2.';de = 'Der Arbeitszeitplan ""%1"" ist für den Zeitraum %2 nicht ausgefüllt.';ro = 'Programul de lucru ""%1"" nu este completat pentru perioada %2.';tr = '""%1"" çalışma takvimi %2 dönem için doldurulmamıştır.'; es_ES = 'El ""%1"" horario de trabajo no está rellenado para el período %2.'");
...
Function ClosestWorkdaysDates(Schedule, InitialDates, GetPrevious = False, RaiseException = True, 
	...
	While Selection.Next() Do
		...
		Else 
			...
			EndIf;
			If RaiseException Then
				MessageText = NStr("ru = 'Невозможно определить ближайшую рабочую дату для даты %1, возможно, график работы не заполнен.'; en = 'Cannot determine the workday nearest to %1. The work schedule might be blank.'; pl = 'Nie można określić najbliższego dnia pracy dla daty %1, możliwe że harmonogram pracy nie został wypełniony.';de = 'Kann den nächsten Werktag für das Datum nicht ermitteln%1, der Arbeitszeitplan ist möglicherweise nicht ausgefüllt.';ro = 'Nu se poate determina cea mai apropiată zi de lucru pentru data %1, posibil, programul de lucru nu este completat.';tr = '%1Tarih için en yakın iş günü belirlenemiyor, çalışma takvimi doldurulmamış olabilir.'; es_ES = 'No de puede determinar el día laboral más cercando para la fecha %1, el horario de trabajo puede no estar poblado.'");
...
Function WorkSchedulesForPeriod(Schedules, StartDate, EndDate) Export
	...
	Raise NStr("ru = 'Подсистема «Графики работы» не обнаружена.'; en = 'The ""Work schedules"" subsystem is not found.'; pl = 'Podsystem «Harmonogramy pracy» nie został znaleziony';de = 'Das Subsystem ""Arbeitszeitpläne"" wurde nicht gefunden.';ro = 'Subsistemul ""Programele de lucru"" nu a fost găsit.';tr = 'Alt sistem ""İş grafikleri"" bulunamadı.'; es_ES = 'Subsistema ""Horarios de trabajo"" no encontrado.'");
...
//
// Parameters:
//	Form - ManagedForm - a form, in which the attribute is to be filled in.
...
Procedure OnAddClassifiers(Classifiers) Export
	...
	Specifier.Description = NStr("ru = 'Производственные календари'; en = 'Business calendars'; pl = 'Kalendarze biznesowe';de = 'Geschäftskalender';ro = 'Calendare de afaceri';tr = 'İş takvimleri'; es_ES = 'Calendarios de los días laborales'");
...
// Returns main business calendar.
//
// Returns:
//   CatalogRef.BusinessCalendars, Undefined - a main business calendar.
//
Function RussianFederationBusinessCalendar() Export
		
	Return MainBusinessCalendar();
...
Function ClassifierData() Export
	...
	If FilesData = Undefined Then
		MessageText = NStr("ru = 'Не удалось получить данные календаря.
                               ...
                               |The ""Classifiers"" subsystem is not supported or is missing.'; 
                               ...
                               |ro = 'Eșec la obținerea datelor calendarului.
                               |Lucrul cu clasificatoarele nu este susținut sau lipsește.';
	...
	If Not IsBlankString(FilesData.ErrorCode) Then
		EventName = NStr("ru = 'Календарные графики.Получение файла классификатора'; en = 'Calendar schedules.Get classifier file'; pl = 'Harmonogramy kalendarzowe.Pobieranie pliku klasyfikatora';de = 'Kalendergrafiken. Erhalten einer Klassifizierungsdatei';ro = 'Programe calendaristice. Obținerea fișierului clasificatorului';tr = 'Takvim grafikleri.Sınıflandırıcı dosyasını alma'; es_ES = 'Horarios. Recepción del archivo del clasificador'", Common.DefaultLanguageCode());
		WriteLogEvent(
			EventName, 
			EventLogLevel.Error,,, 
	...
	If FoundRows.Count() = 0 Then
		MessageText = NStr("ru = 'Не удалось получить данные календаря.
                               ...
                               |ro = 'Eșec la obținerea datelor calendarului.
                               |Clasificatoarele obținute nu conțin calendare.';
                               |tr = 'Takvim verileri alınamadı. 
	...
	If FileInfo.Version < CalendarsVersion() Then
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать полученные данные календаря из-за конфликта версий.
                  ...
                  |ro = 'Eșec la procesarea datelor obținute ale calendarului din cauza conflictului de versiuni.
                  |Versiunea calendarelor
                  |-clasificatorului %1 obținut, 
                  |-%2 incorporate în configurație, 
                  |- clasificatorului %3 încărcat anterior.';
	...
	Except
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не удалось обработать полученные данные календаря.
                  ...
                  |%1.';
                  |ro = 'Eșec la procesarea datelor obținute ale calendarului.
                  |%1.';
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	If Not Common.DataSeparationEnabled() Then
		...
		Handler.ObjectsToBeRead = "InformationRegister.BusinessCalendarData";
		...
		Handler.Comment = NStr("ru = 'Обновление графиков работы и др. данных, зависимых от производственных календарей.'; en = 'Update work schedules and other data that depends on business calendars.'; pl = 'Aktualizacja harmonogramów pracy i in. danych, zależących od kalendarzy produkcyjnych.';de = 'Aktualisierung von Arbeitszeitplänen und anderen Daten in Abhängigkeit von Produktionskalendern.';ro = 'Actualizarea programelor de lucru și altor date, dependente de calendarele de afaceri.';tr = 'Üretim takvimlerine bağlı çalışma çizelgeleri vb. verileri güncelleme.'; es_ES = 'La actualización de los horarios y otros datos que dependen de los calendarios laborales.'");
...
Procedure CheckItemStart(Val XMLReader, Val Name)
	...
	If XMLReader.NodeType <> XMLNodeType.StartElement Or XMLReader.Name <> Name Then
		EventName = NStr("ru = 'Календарные графики.Обработка файла классификатора'; en = 'Calendar schedules.Process classifier file'; pl = 'Harmonogramy kalendarzowe.Przetwarzanie pliku klasyfikatora';de = 'Kalendergrafiken. Verarbeitung der Klassifikatorendatei';ro = 'Programe calendaristice.Procesarea fișierului clasificatorului';tr = 'Takvim grafikleri.Sınıflandırıcı dosyasını işleme'; es_ES = 'Horarios.Procesamiento del archivo del clasificador'", Common.DefaultLanguageCode());
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неверный формат файла данных. Ожидается начало элемента %1'; en = 'Invalid data file format. Beginning of the ""%1"" element is expected.'; pl = 'Nieprawidłowy format pliku danych. Oczekiwany jest początek elementu %1';de = 'Falsches Format der Datendatei. Der Start des Artikels%1 wird erwartet';ro = 'Format incorect al fișierului de date. Se așteaptă începutul elementului %1';tr = 'Veri dosyasının yanlış formatı. %1Öğenin başlangıcı bekleniyor'; es_ES = 'Formato incorrecto del archivo de datos. Inicio del artículo %1 está esperado'"), 
			Name);
...
Procedure CheckItemEnd(Val XMLReader, Val Name)
	...
	If XMLReader.NodeType <> XMLNodeType.EndElement Or XMLReader.Name <> Name Then
		EventName = NStr("ru = 'Календарные графики.Обработка файла классификатора'; en = 'Calendar schedules.Process classifier file'; pl = 'Harmonogramy kalendarzowe.Przetwarzanie pliku klasyfikatora';de = 'Kalendergrafiken. Verarbeitung der Klassifikatorendatei';ro = 'Programe calendaristice.Procesarea fișierului clasificatorului';tr = 'Takvim grafikleri.Sınıflandırıcı dosyasını işleme'; es_ES = 'Horarios.Procesamiento del archivo del clasificador'", Common.DefaultLanguageCode());
		MessageText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неверный формат файла данных. Ожидается конец элемента %1'; en = 'Invalid data file format. End of the ""%1"" element is expected.'; pl = 'Nieprawidłowy format pliku danych. Oczekiwany jest koniec elementu %1';de = 'Falsches Format der Datendatei. Ende des Artikels %1 wird erwartet';ro = 'Format incorect al fișierului de date. Se așteaptă sfârșitul elementului %1';tr = 'Veri dosyasının formatı yanlış. %1Öğenin sonu bekleniyor'; es_ES = 'Formato incorrecto del archivo de datos. Final del artículo %1 está esperado'"), 
			Name);
		WriteLogEvent(EventName, EventLogLevel.Error, , , MessageText);
		Raise MessageText;
	EndIf;
