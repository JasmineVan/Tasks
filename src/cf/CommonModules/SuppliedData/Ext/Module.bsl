...
// retrieved by search by attributes or by specifying a UUID, which one was passed to the Descriptor.
// FileGUID field. If the base already has data with the same data kind and set of key 
...
Procedure SaveSuppliedDataInCache(Val Descriptor, Val PathToFile) Export
	...
	Try
		...
		If PathToOldFile <> Undefined Then
			...
			TempFile = New File(PathToOldFile);
			If TempFile.Exist() Then
				...
				Except
					WriteLogEvent(
						NStr("ru = 'Поставляемые данные.Удаление файлов в томе'; en = 'Supplied data.File deletion in volume'; pl = 'Dostarczone dane.Usuwanie plików z woluminu';de = 'Gelieferte Daten. Dateien im Volumen löschen';ro = 'Datele furnizate.Ștergerea fișierelor în volum';tr = 'Sağlanan veriler. Ciltteki dosyaların silinmesi'; es_ES = 'Datos suministrados.Eliminación de los archivos en el volumen'",
...
Function SuppliedDataFromCache(Val RefOrID) Export
	...
	Else
		...
		Except
			...
			WriteLogEvent(
				NStr("ru = 'Поставляемые данные.Получение файла из тома'; en = 'SuppliedData.Receive the file from the volume'; pl = 'Dostarczone dane. Pobieranie pliku z woluminu';de = 'Gelieferte Daten. Datei vom Volumen empfangen';ro = 'Datele furnizate.Primirea fișierului din volum';tr = 'Sağlanan veri.  Dosyanın birimden alınması'; es_ES = 'Datos proporcionados.Recibiendo el archivo del volumen'", 
				Common.DefaultLanguageCode()),
				EventLogLevel.Error,
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка открытия файла: файл не найден на сервере.
				           ...
				           |
				           |Datei: ""%1.%2"".';
				           |ro = 'Eroare de deschidere a fișierului: fișierul nu a fost găsit pe server.
...
Function GetDataDescription(Val Descriptor) Export
	...
	If TypeOf(Descriptor) = Type("XDTODataObject") Then
		...
		Details = Details + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = ', добавлен: %1 (%2), рекомендовано загрузить: %3 (%2)'; en = ', added: %1 (%2), recommended to import: %3 (%2)'; pl = ', dodano: %1 (%2), zaleca się zaimportować: %3 (%2)';de = ', hinzugefügt: %1 (%2), es wird empfohlen zu importieren: %3 (%2)';ro = ', adăugat: %1 (%2), recomandat pentru import: %3 (%2)';tr = ', eklendi: %1(%2), içe aktarılması önerilir: %3(%2)'; es_ES = ', añadido: %1 (%2), se recomienda importar: %3 (%2)'"), 
	...
	Else
		...
		Details = Details + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = ', добавлен: %1 (%2)'; en = ', added: %1 (%2)'; pl = ', dodany: %1 (%2)';de = ', hinzugefügt: %1 (%2)';ro = ', adăugat: %1 (%2)';tr = ', eklendi: %1 (%2)'; es_ES = ', añadido: %1 (%2)'"), 
...
Procedure MessageChannelHandlersOnDefine(Handlers) Export
	...
	SuppliedDataMessagesMessageHandler.GetMessagesChannelsHandlers(Handlers);
