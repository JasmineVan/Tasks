...
//
//    * ArbitraryObject - Boolean - if True, a table with additional attribute details is created in 
//            the form, the Object parameter is ignored, and additional attributes are not created and not filled in.
//
...
//            To save changes, call TransferValuesFromFormAttributesToObject(). To update the set of 
//            attributes, call UpdateAdditionalAttributesItems().
//
...
//            If the parameter is not specified, but the Object parameter is specified and the Ref 
//            property is not filled in, the initial value is set to True, otherwise, False.
//            When calling the BeforeWriteAtServer procedure in the hide deleted mode, deleted 
//            values are cleared (not transferred back to the object), and the HideDeleted mode is set to False.
//
...
//
// Parameters:
//  Form - ManagedForm - already set in the OnCreateAtServer procedure.
...
Procedure FillCheckProcessing(Form, Cancel, AttributesToCheck, Object = Undefined) Export
	...
	For each Row In Form.Properties_AdditionalAttributeDetails Do
		If Row.RequiredToFill AND NOT Row.Deleted Then
			If Not AttributeIsAvailableByFunctionalOptions(Row) Then
				Continue;
			EndIf;
			Result = True;
			...
			If NOT ValueIsFilled(Form[Row.ValueAttributeName]) Then
				Common.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Поле ""%1"" не заполнено.'; en = 'The ""%1"" field is required.'; pl = 'Pole ""%1"" nie jest wypełnione.';de = 'Das ""%1"" Feld ist nicht ausgefüllt.';ro = 'Câmpul ""%1"" nu este completat.';tr = '""%1"" alanı doldurulmadı.'; es_ES = 'El ""%1"" campo no está rellenado.'"), Row.Description),
					,
...
//
// Parameters:
//  ObjectKind                - Object - for example, a product kind before writing.
//  ObjectWithPropertiesName - String - for example, Products.
//  PropertiesSetAttributeName - String - used when there are several property sets or an attribute 
//                              name of the default set that differs from PropertiesSet is used.
//
Procedure BeforeWriteObjectKind(ObjectKind,
	...
	|	Properties.PropertiesSet.Description AS SetDescription,
	|	Properties.PropertiesSet.DeletionMark AS SetDeletionMark
	|FROM
	|	ChartOfCharacteristicTypes.AdditionalAttributesAndInfo AS Properties
	|WHERE
	|	Properties.PropertiesSet = &PropertiesSet
	|	AND CASE
	|			WHEN Properties.Description <> Properties.Title + "" ("" + Properties.PropertiesSet.Description + "")""
	|				THEN TRUE
	|			WHEN Properties.DeletionMark <> Properties.PropertiesSet.DeletionMark
	|				THEN TRUE
	|			ELSE FALSE
	|		END";
...
//                    deleted values are cleared (not transferred back to the object), and the HideDeleted mode is set to False.
//
...
// Used for sets specified in the procedure
// PropertyManagerOverridable.OnCreatePredefinedPropertiesSets.
//
// Parameters:
...
//  Undefined - if a predefined set is not found.
//
// Example:
...
Function PropertiesSetByName(SetName) Export
	...
	Set = PredefinedPropertiesSets.Get(SetName);
...
Procedure FillAdditionalAttributesInForm(Form, Object = Undefined, LabelsFields = False, HideDeleted = Undefined) Export
	...
	For Each PropertyDetails In PropertiesDetails Do
		...
		For Each TableRow In PropertyDetails.AdditionalAttributesDependencies Do
			...
			// Convert the old condition for backward compatibility.
		...
		If PropertyDetails.RefTypeString Then
			...
			Else
				Value = NStr("ru = 'не задано'; en = 'not set'; pl = 'nie określono';de = 'nicht gesetzt';ro = 'nu este specificat';tr = 'belirlenmedi'; es_ES = 'no definido'");
				EditLink = "NotDefined";
				StringValue = New FormattedString(Value,, StyleColors.EmptyHyperlinkColor,, EditLink);
			EndIf;
			Form[PropertyDetails.ReferenceAttributeNameValue] = StringValue;
		EndIf;
		...
		If ObjectPropertySets.Count() > 1 Then
			...
			Else
				...
				If SetDetails = Undefined Then
					...
					SetDetails.Title = NStr("ru = 'Удаленные реквизиты'; en = 'Deleted attributes'; pl = 'Usunięte atrybuty';de = 'Gelöschte Attribute';ro = 'Atribute șterse';tr = 'Silinen özellikler'; es_ES = 'Atributos eliminados'")
				EndIf;
...
//
// Parameters:
//  PropertiesOwner      - Reference - for example, CatalogRef.Products, DocumentRef.SalesOrder, ...
//                       - Object - for example, CatalogObject.Products, DocumentObject.SalesOrder, ...
...
//  GetAddlAttributes - Boolean - include additional attributes to the result. Default value is True.
//  GetAddlInfo - Boolean - include additional info to the result. Default value is True.
...
Procedure WriteObjectProperties(PropertiesOwner, PropertyAndValueTable) Export
	...
	AdditionalInfoTable = AddlAttributesTable.CopyColumns();
	...
	Try
		If HasAddlAttributes Then
			...
			For Each AdditionalAttribute In AddlAttributesTable Do
				...
				EndIf;
				FillPropertyValues(PropertyRow, AdditionalAttribute, "Property,Value,TextString");
			EndDo;
...
// 1. Updates descriptions of predefined property sets if they differ from the current presentations 
// of matching metadata objects with properties.
// 
// 2. Updates descriptions of not common properties if their adjustment is different from their set 
// description.
// 3. Sets a deletion mark for not common properties if their sets are marked for deletion.
// 
//
	...
	If OptionUseProperties Then
		...
		If CreateAdditionalAttributesDetails Then
			...
			If AccessRight("Update", Metadata.Catalogs.AdditionalAttributesAndInfoSets) Then
				...
				Command.Title = NStr("ru = 'Изменить состав дополнительных реквизитов'; en = 'Change set of additional attributes'; pl = 'Zmiana zestawu atrybutów dodatkowych';de = 'Ändern Sie den Satz zusätzlicher Attribute';ro = 'Modificați setul de atribute suplimentare';tr = 'Ek özelliklerin kümesini değiştir'; es_ES = 'Cambiar el conjunto de atributos adicionales'");
				Command.Action = "Attachable_PropertiesExecuteCommand";
				Command.ToolTip = NStr("ru = 'Изменить состав дополнительных реквизитов'; en = 'Change set of additional attributes'; pl = 'Zmiana zestawu atrybutów dodatkowych';de = 'Ändern Sie den Satz zusätzlicher Attribute';ro = 'Modificați setul de atribute suplimentare';tr = 'Ek özelliklerin kümesini değiştir'; es_ES = 'Cambiar el conjunto de atributos adicionales'");
			...
			Command.Title   = NStr("ru = 'Начать/закончить редактирование'; en = 'Start/end editing'; pl = 'Rozpocząć/zakończyć edycję';de = 'Bearbeitung starten/beenden';ro = 'Începe/ finalizează editarea';tr = 'Düzenlemeyi başla/bitir'; es_ES = 'Empezar/ terminar la edición'");
			Command.Action    = "Attachable_PropertiesExecuteCommand";
			Command.ToolTip   = NStr("ru = 'Начать/закончить редактирование'; en = 'Start/end editing'; pl = 'Rozpocząć/zakończyć edycję';de = 'Bearbeitung starten/beenden';ro = 'Începe/ finalizează editarea';tr = 'Düzenlemeyi başla/bitir'; es_ES = 'Empezar/ terminar la edición'");
...
Procedure PrepareFormForDeferredInitialization(Form, Context, ItemForPlacementName, Index)
	...
	If Parent <> Undefined
		...
		PlacementWarning = NStr("ru = 'Для отображения дополнительных реквизитов необходимо разместить группу ""%1"" не первым элементом (после любой другой группы) в группе ""%2"" (меню Еще - Изменить форму).'; en = 'To show additional attributes, place the ""%1"" group not as the first item (after any other group) in the ""%2"" group (menu More actions - Change form).'; pl = 'Aby wyświetlić więcej danych należy umieścić grupę ""%1"" nie pierwszym elementem (po każdej innej grupie) w grupie ""%2"" (Menu Więcej - Zmień formularz).';de = 'Um zusätzliche Details anzuzeigen, ist es notwendig, die Gruppe ""%1"" nicht das erste Element (nach einer anderen Gruppe) in der Gruppe ""%2"" (Menü Mehr - Formular ändern) zu platzieren.';ro = 'Pentru afișarea atributelor suplimentare trebuie să amplasați grupul ""%1"" nu ca primul element (după orice alt grup) în grupul ""%2"" (meniul Mai multe - Modifică forma).';tr = 'Ek özellikleri görüntülemek için, ""%1""grubu ""%2"" grubunda (başka bir gruptan sonra) ilk öğe şeklinde yerleştirmemeniz gerekir (menü Daha fazla-Formu değiştirin).'; es_ES = 'Para visualizar los requisitos adicionales es necesario colocar en el grupo ""%1"" no como el primer elemento (después del cualquier otro grupo) en el grupo ""%2"" (menú Más - Cambiar el formulario).'");
...
Procedure SetDependenceCondition(DependenciesStructure, AttributePath, TableRow, ConditionCode, Condition)
	If DependenciesStructure = Undefined Then
		...
		EndIf;
		ParametersValues.Insert(AttributePath, Value);
		DependenciesStructure = New Structure;
		DependenciesStructure.Insert("ConditionCode", ConditionCode);
		DependenciesStructure.Insert("ParameterValues", ParametersValues);
...
Function PropertiesUsed(Form, AdditionalParameters)
	...
	PropertiesSet = PropertiesSetByName(ItemName);
	If PropertiesSet = Undefined Then
		PropertiesSet = Catalogs.AdditionalAttributesAndInfoSets[ItemName];
	EndIf;
