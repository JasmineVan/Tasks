...
#EndIf
	...
	Except
		...
		EventLogClient.AddMessageForEventLog(EventLogEvent(), "Error",
			DetailErrorDescription(ErrorInfo()),,True);
		Raise(NStr("ru = 'Ошибка при открытии файла шаблона.'; en = 'Error occurred opening template file.'; pl = 'Podczas otwierania pliku z szablonu wystąpił błąd.';de = 'Beim Öffnen einer Vorlagendatei ist ein Fehler aufgetreten.';ro = 'Eroare la deschiderea fișierului de șablon.';tr = 'Değişim dosyası açılırken bir hata oluştu'; es_ES = 'Ha ocurrido un error al abrir un archivo de modelo.'") + Chars.LF 
...
Function EventLogEvent()
	Return NStr("ru = 'Печать'; en = 'Print'; pl = 'Wydruki';de = 'Drucken';ro = 'Forme de listare';tr = 'Yazdır'; es_ES = 'Impresión'", CommonClient.DefaultLanguageCode());
EndFunction
...
#Else		
	ClarificationText = NStr("ru = 'Для вывода печатных форм в формате Microsoft Word требуется, чтобы на компьютере был установлен пакет Microsoft Office.'; en = 'To output print forms in MS Word formats, Microsoft Office must be installed.'; pl = 'Do wyświetlenia formularzy wydruku w formacie Microsoft Word, wymagane jest zainstalowanie pakietu Microsoft Office na komputerze.';de = 'Um Druckformulare im Microsoft Word-Format auszugeben, muss das Microsoft Office-Paket auf dem Computer installiert sein.';ro = 'Pentru imprimarea formelor de tipar în formatul Microsoft Word este necesar ca pe computer să fie instalat pachetul Microsoft Office.';tr = 'Yazdırma formlarını Microsoft Word biçiminde almak için, Microsoft Office paketinin bilgisayara yüklenmesi gerekir.'; es_ES = 'Para mostrar los formularios de impresión en el formato Microsoft Word se requiere que en el ordenador esté instalado el paquete de Microsoft Office.'");
#EndIf
...
#EndIf
			...
			If GetFiles(FilesDetails, , TempDirectory, False) Then
