...
//   Catalog - CatalogManager, an event subscription as this parameter value
//  sets a catalog manager to be used for the job.
//   StandardDataProcessor - boolean, an event subscription as this parameter value
//  sets a flag of standard data processor execution (the DataAreasJobsQueue catalog is selected as 
//  a catalog).
//
...
Procedure UpdateQueueJobsByTemplates(Parameters = Undefined) Export
	...
	Try
		...
		If TemplateChanges.DeletedItems.Count() > 0
			...
			Query.SetParameter("DeletedTemplates", TemplateChanges.DeletedItems);
			...
			|	ISNULL(Queue.LastRunStartDate, DATETIME(1, 1, 1)) AS LastRunStartDate,
			...
			While Selection.Next() Do
				...
				If TemplateRow = Undefined Then
					MessageTemplate = NStr("ru = 'При обновлении не найден шаблон задания %1'; en = 'Job template %1 not found when updating'; pl = 'Podczas aktualizacji nie znaleziono szablonu zadania %1';de = 'Die Jobvorlage %1 wurde beim Aktualisieren nicht gefunden';ro = 'Șablonul sarcinii %1 nu a fost găsit în timpul actualizării';tr = 'Güncelleme sırasında iş şablonu %1 bulunamadı'; es_ES = 'Modelo de la tarea %1 no se ha encontrado al actualizar'");
...
Function UpdateQueueJobTemplates(Parameters)
	...
	If NOT SaaS.DataSeparationEnabled() Then
		Return New Structure("Added, DeletedItems", New Array, New Array);
	EndIf;
	...
	Return New Structure("AddedEdited, DeletedItems", AddedChangedTemplates, DeletedTemplates);
