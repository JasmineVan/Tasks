...
//  Owner - ManagedForm - the form used to open the event log.
//
...
Procedure OpenDataForViewing(CurrentData) Export
	...
	If CurrentData = Undefined Or CurrentData.Data = Undefined Then
		ShowMessageBox(, NStr("ru = 'Эта запись журнала регистрации не связана с данными (см. колонку ""Данные"")'; en = 'The event log record is not linked to data (see the Data column)'; pl = 'Ten wpis dziennika zdarzeń  nie jest związany z danymi (zob. kolumna """"Dane"""")';de = 'Dieser Ereignisprotokolleintrag bezieht sich nicht auf Daten (siehe Spalte ""Daten"")';ro = 'Această înregistrare în registrul logare nu are legătură cu datele (vezi coloana ""Datele"")';tr = 'Bu olay günlüğü girişi verilerle ilgili değil (""Veri"" sütununa bakın)'; es_ES = 'Esta entrada del registro de eventos no está relacionada con los datos (ver la columna ""Datos"")'"));
		Return;
	EndIf;
	...
	Except
		WarningText = NStr("ru = 'Эта запись журнала регистрации связана с данными, но отобразить их невозможно.
									...
									|%1'; 
									|pl = 'Ten wpis dziennika zdarzeń jest połączony z danymi, ale nie można ich wyświetlić.
									|%1';
									|de = 'Dieser Ereignisprotokoll-Überwachungsdatensatz ist mit den Daten verbunden, sie können jedoch nicht angezeigt werden. 
									|%1';
									|ro = 'Această înregistrare a registrului logare are legătură cu date, dar acestea nu pot fi afișate. 
									|%1';
									|tr = 'Bu olay günlüğü izleme kaydı veriye bağlanır, ancak görüntülenemezler.
									|%1'; 
		...
		If CurrentData.Event = "_$Data$_.Delete" Then 
			...
			WarningText =
					StringFunctionsClientServer.SubstituteParametersToString(WarningText, NStr("ru = 'Данные удалены из информационной базы'; en = 'The data was deleted from the infobase'; pl = 'Dane są usunięte z informacyjnej bazy';de = 'Daten werden aus der Infobase gelöscht';ro = 'Datele sunt șterse din baza de informații';tr = 'Veri veritabanından silindi'; es_ES = 'Datos se han borrado de la infobase'"));
		Else
			WarningText =
				StringFunctionsClientServer.SubstituteParametersToString(WarningText, NStr("ru = 'Возможно, данные удалены из информационной базы'; en = 'Perhaps the data was deleted from the infobase'; pl = 'Być może, dane są usunięte z bazy informacyjnej';de = 'Vielleicht werden die Daten aus der Infobase gelöscht.';ro = 'Posibil, datele sunt șterse din baza de informații';tr = 'Veri veritabanından silinmiş olabilir.'; es_ES = 'Probablemente los datos se hayan borrado de la infobase.'"));
		EndIf;
