...
Procedure CheckDataImportRestrictionDates(Source,
	...
	WriteLogEvent(
		?(ImportRestrictionCheckNode <> Undefined,
			NStr("ru = 'Даты запрета изменения.Найдены запреты загрузки'; en = 'Period-end closing dates. Data import restrictions are found'; pl = 'Zmień datę zamknięcia. Znaleziono zakazy importu';de = 'Ändern Sie das Abschlussdatum. Importverbote werden gefunden';ro = 'Datele de interdicție a modificării.Sunt găsite interdicții de import';tr = 'Kapanış tarihini değiştir. İçe aktarma yasakları bulundu'; es_ES = 'Cambiar la fecha de cierre.Prohibiciones de importación encontradas'",
			     Common.DefaultLanguageCode()),
			NStr("ru = 'Даты запрета изменения.Найдены запреты изменения'; en = 'Period-end closing dates. Data modification restrictions are found'; pl = 'Zmień datę zamknięcia. Znaleziono zakazy zmian.';de = 'Ändern Sie das Abschlussdatum. Änderungsverbote werden gefunden';ro = 'Datele de interdicție a modificării.Sunt găsite interdicții de modificare';tr = 'Kapanış tarihini değiştir. Değişiklik yasakları bulundu'; es_ES = 'Cambiar la fecha de cierre.Prohibiciones de cambios encontradas'",
			     Common.DefaultLanguageCode())),
		EventLogLevel.Error,
		,
		,
...
Function DataChangesDenied(Data, DataID, ErrorDescription, ImportRestrictionCheckNode) Export
	...
	ClosingDatesByObjectsNotSpecified = ?(ImportRestrictionCheckNode = Undefined,
	...
	If DataID <> Undefined Then
		...
		DataToCheck = DataToCheckFromDatabase(Data,
			DataID, EffectiveDates, ClosingDatesByObjectsNotSpecified);
	...
	If Not PeriodEndFound AND TypeOf(Data) <> Type("String") Then
		...
		DataToCheck = DataForCheckFromObject(Data,
			EffectiveDates, ClosingDatesByObjectsNotSpecified);
...
Function ErrorTextImportRestrictionDatesNotImplemented() Export
	...
	Return NStr("ru = 'Даты запрета загрузки данных прошлых периодов из других программ
	                   ...
	                   |ro = 'Datele de interdicție a importului de date din perioadele precedente din alte programe
	                   |nu sunt prevăzute pentru nici un plan de schimb.';
			...
			If PredefinedItemsNames.Find(Selection.PredefinedDataName) <> Undefined Then
				...
				Else
					...
					ElsIf Selection.Ref <> SectionProperties.Ref Then
						...
						Object.Description = "(" + NStr("ru = 'не используется'; en = 'not used'; pl = 'nie używane';de = 'wird nicht benutzt';ro = 'nu se utilizează';tr = 'kullanılmaz'; es_ES = 'no se usa'") + ") " + SectionProperties.Presentation;
					EndIf;
				EndIf;
...
Function DataToCheckFromDatabase(Data, DataID, EffectiveDates, ClosingDatesByObjectsNotSpecified)
	...
	Query = New Query;
	If ClosingDatesByObjectsNotSpecified Then
...
// For the ChangeOrImportRestricted function.
Function DataForCheckFromObject(Data, EffectiveDates, ClosingDatesByObjectsNotSpecified)
	...
	DataToCheck = PeriodClosingDates.DataToCheckTemplate();
	NoObject = ClosingDatesByObjectsNotSpecified;
	 ...
	 Or DataSources.Count() = 0 Then
		...
		ErrorText = StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для таблицы ""%1""
			           ...
			           |en = 'No data source for period-end closing check is specified for table %1. Location:
			           |Common module PeriodEndClosingDatesOverridable,
			           |procedure FillDataSourcesForPeriodClosingCheck.'; 
			           |pl = 'W tabeli ""%1""
			           |w procedurze ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
			           |ogólnego modułu ДатыЗапретаИзмененияПереопределяемый
			           |nie są podane źródła danych, które są wymagane do sprawdzenia dat zakazu.';
			           |de = 'Für die Tabelle ""%1""
			           | bei der Vorgehensweise AusfüllenVonDatenquellenZurÜberprüfungDesÄnderungsverbot
			           |des allgemeinen Moduls DatenÄnderungsverbotNeudefinierbar
			           |sind die Datenquellen, die zur Überprüfung der Sperrdaten erforderlich sind, nicht angegeben..';
			           |ro = 'Pentru tabelul ""%1""
			           |în procedura ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
			           |a modulului general ДатыЗапретаИзмененияПереопределяемый
			           |nu sunt indicate sursele de date necesare pentru verificarea datelor de interdicție.';
			           |tr = 'Prosedürdeki ""%1""
			           | tablosu için Yasağı Kontrol Etmek İçin Veri Kaynaklarını Doldurun Yasak tarihlerini
			           | kontrol etmek için Yeniden Tanımlanabilir Değişiklik Yasağı Değişimi Yasaklanan Tarihin
			           | toplam modülünün değiştirilmesi gerekli değildir.'; 
			           |es_ES = 'Para la tabla ""%1""
			           |en el procedimiento ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
			           |del módulo común ДатыЗапретаИзмененияПереопределяемый
			           |no están indicadas las fuentes de datos necesarias para comprobar las fechas de prohibición.'"),
			Table);
		Raise ErrorText;
	EndIf;
...
Function CurrentDataSourceForPeriodClosingCheck(SectionsProperties)
	...
	ErrorTitle =
		NStr("ru = 'Ошибка в процедуре ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
		           |общего модуля ДатыЗапретаИзмененияПереопределяемый.'; 
		           |en = 'Error occurred. Location: common module PeriodClosingDatesOverridable,
		           |procedure FillDataSourcesForPeriodClosingCheck.'; 
		           |pl = 'Błąd w procedurze ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
		           |ogólnego modułu ДатыЗапретаИзмененияПереопределяемый.';
		           |de = 'Fehler bei der Vorgehensweise AusfüllenVonDatenquellenZurÜberprüfungDesÄnderungsverbot
		           |des allgemeinen Moduls ""DatenÄnderungsverbotNeudefinierbar.';
		           |ro = 'Eroare în procedura ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
		           |a modulului comun ДатыЗапретаИзмененияПереопределяемый.';
	...
	For Each Row In Tables Do
		...
		Try
			InRegNameTable = Upper(Row.Table);
			If StrStartsWith(InRegNameTable, Upper("InformationRegister"))
				Or StrStartsWith(InRegNameTable, Upper("AccumulationRegister"))
				Or StrStartsWith(InRegNameTable, Upper("AccountingRegister"))
				Or StrStartsWith(InRegNameTable, Upper("CalculationRegister")) Then
		...
		If MetadataObject = Undefined Then
			Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения в источнике данных неверно указана таблица
				           ...
				           |""%1"" falsch angegeben.';
				           |ro = 'Pentru verificarea interdicției de modificare în sursa de date este indicat incorect tabelul
				           |""%1"".';
			...
			If SectionProperties = Undefined Then
				Raise ErrorTitle + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Для проверки запрета изменения в источнике данных указан несуществующий
					           ...
					           |en = 'Period-end closing date check error. Data source contains invalid section for table %2:
					           |%1.'; 
					           ...
					           |ro = 'Pentru verificarea interdicției de modificare în sursa de date este indicat compartimentul ""%1"" inexistent
					           |pentru tabelul ""%2"".';
					           |tr = 'Değişiklik yasağı kontrolü için veri kaynağında "
" tablosu için mevcut olmayan %1"" bölüm %2 belirtilmiştir.'; 
		...
		If FieldKind = "DateField" Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |.';
				           |ro = 'Pentru verificarea interdicției de modificare 
				           |în sursa de date pentru tabelul ""%1""
				           |nu este specificat câmpul datei.';
				           |tr = 'Tablo ""
				           |
				           |"" için veri kaynağındaki değişiklik yasağını kontrol etmek için veri alanı %1belirtilmemiş.'; 
		...
		If FieldKind = "DateField" Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |ro = 'Pentru verificarea interdicției de modificare 
				           |în sursa de date pentru tabelul ""%1""
				           |este specificat incorect câmpul datei: ""%2""';
				           |tr = 'Tablo ""
				           |
				           |"" için veri kaynağındaki değişiklik yasağını kontrol etmek için veri alanı %1yanlış belirtilmiştir: ""%2"".'; 
				           |es_ES = 'Para comprobar la restricción de cambio 
				           |en la fuente de datos para la tabla ""%1""
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |ro = 'Pentru verificarea interdicției de modificare 
				           |în sursa de date pentru tabelul ""%1""
				           |este specificat incorect câmpul obiectului: ""%2""';
				           |tr = 'Tablo ""
				           |
				           |"" için veri kaynağındaki değişiklik yasağını kontrol etmek için nesne alanı %1yanlış belirtilmiştir: ""%2""'; 
				           |es_ES = 'Para comprobar la restricción de cambio 
				           |en la fuente de datos para la tabla ""%1""
		...
		If FieldKind = "DateField" Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |ro = 'Pentru verificarea interdicției de modificare 
				           |în sursa de date pentru tabelul ""%1""
				           |este specificat incorect câmpul datei:
				           |nu este setat câmpul secțiunii tabelare specificate ""%2"".';
				           |tr = 'Tablo ""
				           |
				           |"" için veri kaynağındaki değişiklik yasağını kontrol etmek için tarih alanı %1yanlış belirtilmiştir: belirlenen tablo kısmının ""%2"" alanı belirtilmedi 
				           |.'; 
				           |es_ES = 'Para comprobar la restricción de cambio 
				           |en la fuente de datos para la tabla ""%1""
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |ro = 'Pentru verificarea interdicției de modificare 
				           |în sursa de date pentru tabelul ""%1""
				           |este specificat incorect câmpul obiectului:
				           |nu este setat câmpul secțiunii tabelare specificate ""%2"".';
				           |tr = 'Tablo ""
				           |
				           |"" için veri kaynağındaki değişiklik yasağını kontrol etmek için nesne alanı %1yanlış belirtilmiştir: belirlenen tablo kısmının ""%2"" alanı belirtilmedi 
				           |.'; 
				           |es_ES = 'Para comprobar la restricción de cambio 
				           |en la fuente de datos para la tabla ""%1""
		...
		If FieldKind = "DateField" Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |în sursa de date pentru tabelul ""%1""
				           |este specificat incorect câmpul datei:
				           |este setat incorect câmpul secțiunii tabelare specificate ""%2"".';
				           |tr = 'Tablo ""
				           |
				           |"" için veri kaynağındaki değişiklik yasağını kontrol etmek için tarih alanı %1yanlış belirtilmiştir: belirlenen tablo kısmının ""%2"" alanı yanlış belirtildi 
				           |.'; 
				           |es_ES = 'Para comprobar la restricción de cambio 
				           |en la fuente de datos para la tabla ""%1""
				           |no está establecido el campo de la fecha:
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Для проверки запрета изменения 
				           ...
				           |tr = 'Tablo ""
				           |
				           |"" için veri kaynağındaki değişiklik yasağını kontrol etmek için nesne alanı %1yanlış belirtilmiştir: belirlenen tablo kısmının ""%2"" alanı yanlış belirtildi 
				           |.'; 
				           |es_ES = 'Para comprobar la restricción de cambio 
				           |en la fuente de datos para la tabla ""%1""
				           |no está establecido correctamente el campo del objeto:
	...
	If Not DataItem.Filter.User.Use Then
		Raise
			NStr("ru = 'Выгрузка или загрузка записей регистра сведений ""Даты запрета изменения""
			           ...
			           |ro = 'Exportul sau importul înregistrărilor registrului de date ""Date de interdicție a modificării""
			           |se admite numai în secțiunea dimensiunii Utilizator.';
