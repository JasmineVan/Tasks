...
Function SeparatedMetadataObjects(Val Separator) Export
	...
	If CommonAttributeMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Общий реквизит %1 не обнаружен в конфигурации.'; en = 'Common attribute %1 was not found in the configuration.'; pl = 'Wspólny rekwizyt %1 nie wykryto w konfiguracji.';de = 'Die gemeinsamen Requisiten %1 sind in der Konfiguration nicht enthalten.';ro = 'Atributul comun %1 nu a fost găsit în configurație.';tr = 'Ortak özellik %1 yapılandırmada bulunamadı.'; es_ES = 'Atributo común %1 no se ha encontrado en la configuración.'"), Separator);
	EndIf;
	...
	If CommonAttributeMetadata.DataSeparation = Metadata.ObjectProperties.CommonAttributeDataSeparation.Separate Then
		...
		For Each CompositionItem In CommonAttributeComposition Do
			...
			If (CommonAttributeAutoUsage AND CompositionItem.Use = AutoUseCommonAttribute)
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(NStr("ru = 'Для общего реквизита %1 не используется разделение данных.'; en = 'Data separation is used for the %1 common attribute.'; pl = 'Dla wspólnego rekwizytu %1 nie stosuje się podział danych.';de = 'Für das gemeinsame Requisit %1 wird keine Datentrennung verwendet.';ro = 'Separarea datelor nu este utilizată pentru atributul comun %1.';tr = 'Ortak özellik için %1 veri ayırma kullanılmaz.'; es_ES = 'Separación de datos %1 no está utilizada para el atributo común.'"), Separator);
		
	EndIf;
	
	// II. In case of sequences and document journals, determining whether they are separated by included documents.
	
	// 1) Sequences. Going over sequences and checking the first included document in each of them. If a sequence includes no documents, it is considered as separated.
	For Each SequenceMetadata In Metadata.Sequences Do
		...
		If SequenceMetadata.Documents.Count() = 0 Then
			...
			MessageTemplate = NStr("ru = 'В последовательность %1 не включено ни одного документа.'; en = 'The %1 sequence contains no documents.'; pl = 'Sekwencja %1 nie zawiera żadnych dokumentów.';de = 'Die Sequenz %1 enthält keine Dokumente.';ro = 'Secvența %1 nu include niciun document.';tr = 'Sıra %1 herhangi  bir belge içermez.'; es_ES = 'Secuencia %1 no incluye ningún documento.'");
			MessageText = StringFunctionsClientServer.SubstituteParametersToString(MessageTemplate, SequenceMetadata.Name);
			WriteLogEvent(NStr("ru = 'Получение разделенных объектов метаданных'; en = 'Getting separated metadata objects'; pl = 'Odbieraj osobne obiekty metadanych';de = 'Empfange separate Metadatenobjekte';ro = 'Primiți obiecte metadate separate';tr = 'Ayrı meta veri nesneleri al'; es_ES = 'Recibir los objetos de metadatos separados'", 
	...
	// 2) Going over sequences and checking the first included document in each of them. If the journal contains no documents, it is considered to be separated.
	For Each DocumentJournalMetadata In Metadata.DocumentJournals Do
		...
		If DocumentJournalMetadata.RegisteredDocuments.Count() = 0 Then
			...
			MessageTemplate = NStr("ru = 'В журнал %1 не включено ни одного документа.'; en = 'The %1 journal contains no documents.'; pl = 'Dziennik %1 nie zawiera żadnych dokumentów.';de = 'Journal %1 enthält keine Dokumente.';ro = 'Jurnalul %1 nu conține niciun document.';tr = 'Günlük%1 herhangi bir belge içermiyor.'; es_ES = 'Registro %1 no contiene ningún documento.'");
