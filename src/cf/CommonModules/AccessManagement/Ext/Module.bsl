			...
			If Upper(Row.AccessKind) = Upper("EditRight") Then
				Raise StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
					           ...
					           |EditingRight dla tabeli z identyfikatorem ""%1"".
					           |W kontrolach ograniczeń rola (jako dodatkowe prawo)
					           |może zależeć tylko od prawa odczytu.';
					           |de = 'In der Funktion HatRolle des Moduls ZuriffsManagement ist ein Fehler aufgetreten. 
					           |In den eingestellten Zugriffswerten ist die Zugriffsart BearbeitungsRecht
					           ...
					           |ro = 'Eroare în funcția ЕстьРоль a modulului УправлениеДоступом.
					           |În setul valorilor de acces este indicat tipul de acces ПравоИзменения
					           |a tabelului cu ID ""%1"".
					           |În restricția de verificare a rolului (ca drept suplimentar)
					           |poate exista dependența doar de dreptul de Citire.';
		...
		ElsIf AccessKindsNames.Get(Row.AccessKind) <> Undefined
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
				           ...
				           |pl = 'Wystąpił błąd w funkcji JestRola modułu ZarządzanieDostępem.
				           ...
				           |""PrawoOdczytu"", ""PrawoZmiany"", jeżeli są używane.';
				           |de = 'In der Funktion HatRolle des Moduls ZugriffsManagement ist ein Fehler aufgetreten. 
				           |Zugriffswerte enthalten die bekannte Zugriffsart ""%1"",
				           | die Sie nicht angeben sollten. 
				           |
				           |Spezifizieren Sie nur spezielle Zugriffs
				           |arten ""LeseRecht"", ""ÄnderungsRecht"", wenn sie verwendet werden.';
				           |ro = 'Eroare în funcția ЕстьРоль a modulului УправлениеДоступом.
				           |Setul de valori de acces conține tipul de acces cunoscut ""%1"",
				           | pe care nu trebuie să-l specificați.
				           |
				           |Specificați doar tipurile de acces speciale
				           |""ПравоЧтения"", ""ПравоИзменения"", dacă sunt utilizate.';
				           |tr = 'AccessManagement modülünün HasRole işlevinde bir hata oluştu. 
				           |Erişim değerleri kümesi belirlemeniz gereken "
"  bilinen erişim türünü içerir.  %1
				           | Kullanıldıkları zaman sadece ""ReadRight"", ""UpdateRight"" özel erişim türlerini belirtin.
				           |
				           |'; 
				           |es_ES = 'Ha ocurrido un error en la función HasRole del módulo AccessManagement.
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка в функции ЕстьРоль модуля УправлениеДоступом.
				           ...
				           |ro = 'Eroare în funcția ЕстьРоль a modulului УправлениеДоступом.
				           |Setul valorilor de acces conține tipul de acces necunoscut ""%1"".';
				           |tr = 'AccessManagement modülünün HasRole işlevinde bir hata oluştu.
...
Function HasRight(Right, ObjectRef, User = Undefined) Export
	...
	If RightsDetails = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не найдено описание возможных прав для таблицы ""%1""'; en = 'Description of available rights for table %1 is not found.'; pl = 'Opis możliwych uprawnień dla tabeli ""%1"" nie został znaleziony';de = 'Beschreibung der möglichen Rechte für die Tabelle ""%1"" wurde nicht gefunden';ro = 'Nu s-a găsit descrierea drepturilor posibile pentru tabelul ""%1""';tr = '""%1"" tablosu için olası hakların açıklanması bulunmadı'; es_ES = 'Descripción de los posibles derechos para la tabla ""%1"" no se ha encontrado'"),
	...
	If RightDetails = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Не найдено описание права ""%1"" для таблицы ""%2""'; en = 'Description of right %1 for table %2 is not found'; pl = 'Prawidłowy opis ""%1"" dla tabeli ""%2"" nie został znaleziony';de = 'Beschreibung der Rechte ""%1"" für Tabelle ""%2"" wurde nicht gefunden';ro = 'Descrierea dreptului ""%1"" pentru tabelul ""%2"" nu a fost găsită';tr = '""%1"" tablosu için ""%2"" hakkın açıklaması bulunmadı'; es_ES = 'Descripción del derecho ""%1"" para la tabla ""%2"" no se ha encontrado'"),
...
// Checks if editing of an existing object in the database is allowed at the rights level and record 
// level for the current user.
//
// Important: if the subsystem operates in the standard restriction mode and not in the universal 
// restriction mode, then the
// Edit right to the table is checked, and the Read right is checked on the record level.
// 
// Parameters:
//  DataDetails - CatalogRef,
//                   DocumentRef,
...
// Checks if reading of an existing object from the database is allowed at the rights level and 
// record level for the current user. If it is not allowed, an exception is raised.
//
// Parameters:
//  DataDetails - CatalogRef,
//                   DocumentRef,
...
// Checks if editing of an existing object in the database is allowed at the rights level and record 
// level for the current user. If it is not allowed, an exception is raised.
//
// Important: if the subsystem operates in the standard restriction mode and not in the universal 
// restriction mode, then the
// Edit right to the table is checked, and the Read right is checked on the record level.
...
//  Profile      - CatalogRef.AccessGroupsProfiles - a profile that requires finding or creating a 
//                   personal access group and include a user into it.
...
//
// Parameters:
//  Form - ManagedForm - a form of an access value that uses groups to select allowed values.
//                   
//
//  Attribute - Undefined - it is the name of the Object.AccessGroup form attribute.
//                 - String - name of a form attribute that contains the access group.
//
...
Procedure OnCreateAccessValueForm(Form,
	...
	ErrorTitle =
		NStr("ru = 'Ошибка в процедуре ПриСозданииФормыЗначенияДоступа
		           ...
		           |de = 'In der Prozedur AufZugriffsFormularErstellen des 
		           |allgemeinen Moduls ZugriffsManagement ist ein Fehler aufgetreten.';
		           |ro = 'Eroare în procedura ПриСозданииФормыЗначенияДоступа
		           |a modulului general УправлениеДоступом.';
	...
	If TypeOf(CreateNewAccessValue) <> Type("Boolean") Then
		...
		Except
			...
			Raise ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Параметр СозданиеНового не указан, а автоматическое заполнение
				           ...
				           |%1'; 
				           |pl = 'Parametr TworzenieNowego nie jest określony, a automatyczne wypełnienie
				           | z rekwizytów formy ""Obiekt.Łącze"" niedostępne z powodu:
				           |%1';
				           |de = 'Der Parameter NeuErstellen ist nicht angegeben, und das automatische Ausfüllen
				           |aus den Formulardetails ""Objekt.Referenz"" ist aus diesem Grund nicht möglich:
				           |%1';
				           |ro = 'Parametrul СозданиеНового nu este indicat, iar completarea automată
				           |din atributul formei ""Объект.Ссылка"" nu este accesibilă din motivul:
				           |%1';
				           |tr = 'YenisiniOluştur parameteresi belirtilmemiştir, ""Nesne.Referans"" formun %1özelliğinden otomatik doldurma aşağıdaki nedenle yapılamaz: 
				           |
				           |'; 
	...
	If TypeOf(ValueType) <> Type("Type") Then
		...
		Except
			ErrorInformation = ErrorInfo();
			Raise ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Параметр ТипЗначения не указан, а автоматическое заполнение
				           |из реквизита формы ""Объект.Ссылка"" недоступно по причине:
				           |%1'; 
				           |en = 'The ValueType parameter is not specified and automatic filling
				           |from the Object.Ref form attribute is not available due to:
				           |%1'; 
				           |pl = 'Parametr TypZnaczenia nie jest określony, a automatyczne wypełnienie
				           | z rekwizytów formy ""Obiekt.Łącze"" niedostępne z powodu:
				           |%1';
				           |de = 'Der Parameter WerteTyp ist nicht angegeben, und das automatische Ausfüllen
				           |aus den Formulardetails ""Objekt.Referenz"" ist aus diesem Grund nicht möglich:
				           |%1';
				           |ro = 'Parametrul ТипЗначения nu este indicat, iar completarea automată
				           |din atributul formei ""Объект.Ссылка"" nu este accesibilă din motivul:
				           |%1';
				           |tr = 'DeğerTürü parameteresi belirtilmemiştir, ""Nesne.Referans"" formun %1özelliğinden otomatik doldurma aşağıdaki nedenle yapılamaz: 
				           |
				           |'; 
		...
		Except
			...
			Raise ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Параметр Реквизит не указан, а автоматическое заполнение
				           ...
				           | z rekwizytów formy ""Obiekt.Łącze"" niedostępne z powodu:
				           |%1';
				           |de = 'Der Parameter Attribut ist nicht angegeben, und das automatische Ausfüllen
				           |aus den Formulardetails ""Objekt.ZugriffsGruppe"" ist aus diesem Grund nicht möglich:
				           |%1';
				           |ro = 'Parametrul Реквизит nu este indicat, iar completarea automată
				           |din atributul formei ""Объект.ГруппаДоступа"" nu este accesibilă din motivul:
				           |%1';
				           |tr = 'Özellik parameteresi belirtilmemiştir, ""Nesne.ErişimGrubu"" formun %1özelliğinden otomatik doldurma aşağıdaki nedenle yapılamaz: 
				           |
				           |'; 
			...
			Except
				...
				Raise ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
					NStr("ru = 'Не удалось получить значение реквизита формы ""%1"", указанного параметре Реквизит по причине:
					           ...
					           |%2'; 
					           |pl = 'Nie udało się uzyskać wartość rekwizytów formy ""%1"", określonego parametru Rekwizyty z powodu:
					           |%2';
					           ...
					           |%2';
					           |ro = 'Eșec la obținerea valorii atributului formei ""%1"", indicate în parametrul Atribut din motivul:
					           |%2';
	...
	If TypeOf(AccessValuesGroup) <> GroupsProperties.Type Then
		Raise ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для значений доступа типа ""%1""
			           ...
			           |dostępu w rekwizytach GrupaDostępu.';
			           ...
			           |wertes beim Attribut ZugriffsGruppe überein.';
			           |ro = 'Pentru valorile de acces de tipul ""%1""
			           |se utilizează tipul de acces ""%2"" cu tipul de valori ""%3""
			           | specificat în modulul supradeterminat.
			           |Dar acest tip nu coincide cu tipul ""%4"" în forma valorii
			           |de acces la atributul ГруппаДоступа.';
...
" değerleriyle kullanılır. 
	...
	If ValuesGroupsForChange.Count() = 0
	   AND CreateNewAccessValue Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для добавления требуются разрешенные ""%1"".'; en = 'Allowed %1 are required to add items.'; pl = 'Do dodania wymagane są dopuszczalne ""%1"".';de = 'Zulässige ""%1"" sind zum Hinzufügen erforderlich.';ro = 'Pentru adăugare sunt necesare ""%1"" permise.';tr = 'Eklemek için izin verilen ""%1"" gereklidir.'; es_ES = 'Permitidos ""%1"" se requieren para añadir.'"),
...
Function AccessValuesGroupsAllowingAccessValuesChange(AccessValuesType, ReturnAll = False) Export
	...
	ErrorTitle =
		NStr("ru = 'Ошибка в процедуре ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа
		           ...
		           |of the AccessManagement common module.'; 
		           |pl = 'Wystąpił błąd w procedurze AccessValueGroupsAllowingAccessValuesChange 
		           |modułu wspólnego AccessManagement.';
		           |de = 'In der Prozedur ZugriffsgruppenDieDieÄnderungDerZugriffswerte zulassen 
		           |des allgemeinen ZugriffsManagementmoduls ist ein Fehler aufgetreten.';
		           |ro = 'Eroare în procedura ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа
		           |a modulului comun УправлениеДоступом.';
		           |tr = 'ErişimYönetimi ortak modülünün ErişimDeğerleriGrubuErişimiİzinVerilenDeğerlerDeğişimi prosedüründe 
		           | bir hata oluştu.'; 
...
Procedure SetDynamicListFilters(List, FiltersDetails) Export
	...
	If Not LimitAccessAtRecordLevel()
	 Or AccessManagementInternal.LimitAccessAtRecordLevelUniversally(, True)
	 Or Users.IsFullUser(,, False) Then
		Return;
	EndIf;
	...
	If TypeOf(List) <> Type("DynamicList") Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
			           ...
			           |des gemeinsamen Moduls AccessManagement ist ein Fehler aufgetreten. Der Wert ""%1"" des List-Parameters ist keine dynamische Liste.';
			           |ro = 'A apărut o eroare la apelarea procedurii SetDynamicListFilters
			           | a modulului comun AccessManagement. Valoarea ""%1"" a parametrului Listă nu este o listă dinamică.';
			           |tr = 'ErişimKontrolü genel modülün DinamikListesininSeçiminiAyarla prosedürünün çağrı hatası. 
			           ...
			           |El valor del parámetro Lista ""%1"" no es lista dinámica.'"), String(List));
	EndIf;
	...
	If Not ValueIsFilled(List.MainTable) Then
		Raise
			NStr("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
			           ...
			           |pl = 'Wywołanie procedury nie powiodło się НастроитьОтборыДинамическогоСписка wspólnego modułu УправлениеДоступом.
			           |Przeniesiona lista dynamiczna nie ma głównej tabeli.';
			           |de = 'Fehler beim Aufruf der Prozedur KonfigurierenVonDynamischenListenAuswahlmöglichkeiten des Moduls ZugriffsKontrolle.
			           |Die übertragene dynamische Liste hat keine Haupttabelle.';
			           |ro = 'Eroare de apelare a procedurii НастроитьОтборыДинамическогоСписка a modulului general УправлениеДоступом.
			           |La lista dinamică transmisă nu este indicat tabelul principal.';
			           |tr = 'ErişimKontrolü genel modülün DinamikListesininSeçiminiAyarla prosedürünün çağrı hatası. 
	...
	If Not List.CustomQuery Then
		Raise
			NStr("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
			           ...
			           |ro = 'A apărut o eroare la apelarea procedurii SetDynamicListFilters a modulului comun AccessManagement.
			           |Lista dinamică transmisă procedurii nu conține caseta de selectare ""Interogare personalizată"".';
			           |tr = 'ErişimKontrolü genel modülün DinamikListesininSeçiminiAyarla prosedürünün çağrı hatası. 
		...
		If Not ValueIsFilled(Alias) Then
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка вызова процедуры НастроитьОтборыДинамическогоСписка общего модуля УправлениеДоступом.
				           ...
				           |in der an die Prozedur übergebenen dynamischen Liste kann nicht gefunden werden.';
				           |ro = 'A apărut o eroare la apelarea procedurii SetDynamicListFilters a modulului comun AccessManagement.
				           |Nu se poate găsi alias-ul ""%1"" tabel principal
				           |din lista dinamică transmisă procedurii.';
				           |tr = 'ErişimKontrolü genel modülün DinamikListesininSeçiminiAyarla prosedürünün çağrı hatası. 
...
//    * AccessKind - String - optional, except for special ones: ReadRight and UpdateRight.
//    * AccessValue - DefinedType.AccessValue - an access value type specified for an access kind in 
	...
	If NOT SetsFilled Then
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Неверные параметры.
			           ...
			           |Typ obiektu ""%1""
			           |nie występuje w przypadku subskrypcji zdarzeń
			           ...
			           |Tipul obiectului ""%1""
			           |nu a fost găsit în niciunul dintre subscrierile la evenimentele
			           |""Înregistrare seturile valorilor de acces"",
			           |""Înregistare seturile dependente ale valorilor de acces"".';
	...
	Except
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 ""%2""
			           ...
			           |%3';
			           |ro = '%1 ""%2""
			           |nu a generat setul valorilor de acces din motivul:
			           |%3';
			           |tr = '%1""%2""
	...
	If Table.Count() = 0 Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = '%1 ""%2""
			           ...
			           |de = '%1 ""%2""
			           |hat einen leeren Satz von Zugriffswerten gebildet.';
			           |ro = '%1 ""%2""
			           |a generat setul gol al valorilor de acces.';
			           |tr = '%1""%2""
...
//   * Name           - String - PredefinedDataName - it is used to link the supplied data to the 
//                       predefined item.
//   * ID - String - a UUID string of the supplied profile used to search in the database.
//                       
//                       To get the ID, create a profile in 1C:Enterprise mode and get the reference 
//                       UUID. Do not specify IDs received using an arbitrary method as this may 
//                       violate the uniqueness of the references.
...
//                        If it is blank, the assignment is for users.
...
//	ProfileDetails.Roles.Add("ViewApplicationChangeHistory");
...
//    * Key     - String - a name of the role to be replaced, for example, ReadBasicMasterData. If 
//                          the role is deleted, add the prefix "?" to the name, for example, "? ReadBasicMasterData".
//
...
// Returns:
//  ValueTable - a table with the following columns:
//    * OwnersType - Ref - a blank reference of the right owner type from description of the
...
Procedure ReplaceRightsInObjectsRightsSettings(RenamedTable) Export
	...
	Try
		...
		If RepeatedNewNames.Count() > 0 Then
			...
			EndDo;
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Ошибка в параметрах процедуры ЗаменитьПраваВНастройкахПравОбъектов
				           |общего модуля УправлениеДоступом.
				           |
				           ...
				           |%1.'; 
				           |pl = 'Błąd w parametrach procedury ЗаменитьПраваВНастройкахПравОбъектов
				           |wspólnego modułu УправлениеДоступом.
				           |
				           |Po aktualizacji będą powtarzały się ustawienia następnych nowych imion praw:
				           |%1.';
				           |de = 'Fehler in den Parametern der Prozedur ErsetzenDerRechteInDenObjektRechteEinstellungen des
				           |allgemeinen Moduls ZugriffsKontrolle.
				           |
				           |Nach dem Update werden die Einstellungen für die folgenden neuen Rechte wiederholt:
				           |%1.';
				           |ro = 'Eroare în parametrii procedurii ЗаменитьПраваВНастройкахПравОбъектов
				           |din modulul general УправлениеДоступом.
				           |
				           |După actualizare se vor repeta setările următoarelor nume de drepturi noi:
				           |%1.';
				           |tr = 'ErişimYönetimi genel modülünün
				           | NesneHaklarınAyarlarındaHakkınıDeğiştir işlem parametrelerinde bir hata oluştu. 
				           |
				           |Güncellemeden sonra hakların aşağıdaki yeni isimlerin ayarları tekrarlanacaktır:%1
				           |.'; 
...
Procedure DisableAccessKeysUpdate(Disable, ScheduleUpdate = True) Export
	...
	If Disable AND Not Users.IsFullUser() Then
		ErrorText =
			NStr("ru = 'Некорректный вызов процедуры ОтключитьОбновлениеКлючейДоступа общего модуля УправлениеДоступом.
			           ...
			           |pl = 'Nieprawidłowe wywołanie procedury ОтключитьОбновлениеКлючейДоступа wspólnego modułu УправлениеДоступом.
			           |Wyłączenie aktualizacji kluczy dostępu jest możliwe tylko dla trybu pełnego użytkownika lub
			           |w trybie uprzywilejowanym.';
			           |de = 'Ein falscher Aufruf der Prozedur AktualisierungVonZugriffsSchlüsselnDeaktivieren des allgemeinen Moduls ZugriffsKontrolle.
			           ...
			           |ro = 'Apelare incorectă a procedurii ОтключитьОбновлениеКлючейДоступа a modulului general УправлениеДоступом.
			           |Dezactivarea actualizării cheilor de acces este posibilă numai pentru utilizatorul cu drepturi depline sau
			           |în regim privilegiat.';
			           |tr = 'ErişimKontrolü genel modülün ErişimAnahtarlarınınGüncellenmesiniDevreDışıBırak prosedürün çağrısı yanlıştır. 
	...
	If Not Disable AND ScheduleUpdate Then
		...
		If EditedLists.Count() > 0 Then
			If AccessManagementInternal.LimitAccessAtRecordLevelUniversally() Then
				...
				EndDo;
				AccessManagementInternal.AddDependentLists(Lists, AddedLists);
				PlanningParameters = AccessManagementInternal.AccessUpdatePlanningParameters();
				PlanningParameters.AllowedAccessKeys = False;
				PlanningParameters.Details = "DisableAccessKeysUpdateOnFinishDisabling";
...
// Adds a deferred update handler that enables universal access restriction (enables the 
// LimitAccessAtRecordLevelUniversally constant).
// Use it only in final typical solutions, not in library supplies.
//
// Parameters:
...
//
// Example:
//	The OnAddUpdateHandlers(Handlers) Export procedure
...
Procedure AddUpdateHandlerToEnableUniversalRestriction(Version, Handlers) Export
	...
	Handler.Comment = NStr("ru = 'Включение универсального ограничения доступа на уровне записей.'; en = 'Enabling universal access restriction at the record level.'; pl = 'Włączenie uniwersalnego ograniczenia dostępu na poziomie zapisów.';de = 'Aktivieren Sie die universelle Zugriffsbeschränkung auf Schreibebene.';ro = 'Activarea restricției de acces universale la nivel de înregistrări.';tr = 'Kayıt düzeyinde evrensel erişim kısıtlamalarını etkinleştirin.'; es_ES = 'Activación de restricción universal de acceso en nivel de registros.'");
...
// Returns:
//  Structure - with the following properties:
//   * ErrorsDescription - Structure - with the following properties:
		...
		ElsIf AccessKindsNames.Get(Row.AccessKind) <> Undefined
			...
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Объект ""%1"" сформировал набор значений доступа,
				           ...
				           |""ReadingRight"", ""ChangingRight"", jeśli są używane.';
				           |de = 'Objekt ""%1"" generierte Zugriffs
				           |werte, die eine bekannte Zugriffsart ""%2"" enthalten, die nicht angegeben werden sollte. 
				           |
				           |Spezifizieren Sie nur spezielle Zugriffs
				           |arten ""LeseRecht"", ""RechtÄndern"", wenn sie verwendet werden.';
				           |ro = 'Obiectul ""%1"" a generat setul de valori de acces
				           |care conține tipul de acces cunoscut ""%2"", care nu trebuie specificat.
				           |
				           |Specificați doar tipurile speciale de acces
				           |""ПравоЧтения"", ""ПравоИзменения"", dacă sunt utilizate.';
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersToString(
				NStr("ru = 'Объект ""%1"" сформировал набор значений доступа,
				           ...
				           |de = 'Objekt ""%1"" erzeugte Zugriffs
				           |werte, die die unbekannte Zugriffsart ""%2"" enthalten.';
				           |ro = 'Obiectul ""%1"" a generat setul valorilor de acces
				           |care conține tipul de acces necunoscut ""%2"".';
...
Procedure FillDestinationByResultSets(Destination, ResultSets)
	...
	If AccessKindProperties = Undefined Then
		Raise ErrorTitle + Chars.LF + Chars.LF + StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Для значений доступа типа ""%1""
			           ...
			           |ro = 'Pentru valorile de acces ale tipului ""%1"",
			           |grupurile valorilor de acces nu sunt utilizate.';
			           |tr = '""%1"" tipi erişim
	...
	If Not AccessManagementInternal.SimplifiedAccessRightsSetupInterface() Then
		Raise
			NStr("ru = 'Данная операция возможна только для упрощенного
			           ...
			           |ro = 'Această operație este posibilă numai pentru interfața
			           |simplificată de setare a drepturilor de acces.';
	   ...
	   AND TypeOf(User) <> Type("CatalogRef.ExternalUsers") Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersToString(
			NStr("ru = 'Недопустимое значение параметра %1 в %2.
			           ...
			           |Așteptat: %3; valoarea trimisă: %4 (tip %5).';
			...
			ExpectedTypes, 
			?(ParameterValue <> Undefined, ParameterValue, NStr("ru = 'Неопределено'; en = 'Undefined'; pl = 'Nieokreślone';de = 'Nicht definiert';ro = 'Nedefinit';tr = 'Tanımlanmamış'; es_ES = 'No definido'")),
					...
					If ValueIsFilled(ErrorDescription) Then
						Raise
							NStr("ru = 'Профиль Администратор должен быть хотя бы у одного пользователя,
							           ...
							           |ro = 'Cel puțin un utilizator trebuie să aibă profilul Administrator
							           |cu permisiunea de a intra în program.';
