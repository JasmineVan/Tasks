...
Procedure OnWrite(Cancel)
	...
	EmailManagement.SetDeletionMarkForEmailAttachments(Ref, DeletionMark);
...
Procedure BeforeWrite(Cancel, WriteMode, PostingMode)
	...
	If IsNew() Then
		AdditionalProperties.Insert("DeletionMark", False);
	Else
		AdditionalProperties.Insert("DeletionMark",
			Common.ObjectAttributeValue(Ref, "DeletionMark"));
	EndIf;
...
Procedure Filling(FillingData, FillingText, StandardProcessing)
	...
	ElsIf (TypeOf(FillingData) = Type("Structure")) AND (FillingData.Property("Base")) 
		 AND (TypeOf(FillingData.Base) = Type("DocumentRef.IncomingEmail") 
		 OR TypeOf(FillingData.Base) = Type("DocumentRef.OutgoingEmail")) Then
		
		Interactions.FillDefaultAttributes(ThisObject, Undefined);
		FillBasedOnEmail(FillingData.Base, FillingData.Command);
...
Procedure FillBasedOnEmail(Base, ReplyType)
	...
	FillParametersFromEmail(Base, MoveSender, MoveAllRecipients,
