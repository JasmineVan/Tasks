...
Procedure BeforeClose(Cancel, Exit, WarningText, StandardProcessing)
	...
	If Not Exit
		...
		If HasFilesToEdit Then
			...
			QuestionText = NStr("ru = 'Данные были изменены, Записать?'; en = 'Data was changed. Write?'; pl = 'Data was changed. Write?';de = 'Data was changed. Write?';ro = 'Data was changed. Write?';tr = 'Data was changed. Write?'; es_ES = 'Data was changed. Write?'");
			AdditionalParameters = New Structure;
...
Procedure OnWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If MessageFormat = Enums.EmailEditingMethods.HTML Then
		...
		For each Attachment In AttachmentsNamesToIDsMapsTable Do
			...
			AttachedFile = EmailManagement.WriteEmailAttachmentFromTempStorage(
			                     Email,
			                     PictureAddressInTempStorage,
			                     "_" + StrReplace(Attachment.FileIDForHTML, "-", "_"),
			                     BinaryPictureData.Size());
			
			If AttachedFile <> Undefined Then
				AttachedFileObject = AttachedFile.GetObject();
				AttachedFileObject.EmailFileID = Attachment.FileIDForHTML;
	...
	If BaseEmailProcessingRequired() Then
		...
		For Each Attachment In BaseEmailAttachments Do
			...
			PictureAddressInTempStorage = PutToTempStorage(BinaryPictureData, UUID);
			AttachedFile = EmailManagement.WriteEmailAttachmentFromTempStorage(
			Email,PictureAddressInTempStorage,Attachment.Description, Attachment.Size);
			If AttachedFile <> Undefined Then
				AttachedFileObject = AttachedFile.GetObject();
				AttachedFileObject.EmailFileID = Attachment.EmailFileID;
				AttachedFileObject.Write();
			EndIf;
	...
	For Each AttachmentsTableRow In Attachments Do
		...
		If AttachmentsTableRow.Placement = 4 Then
			...
			EmailManagement.WriteEmailAttachmentFromTempStorage(
				Email, AttachmentsTableRow.FileNameOnComputer, FileName, Size);
...
Procedure AfterWriteAtServer(CurrentObject, WriteParameters)
	...
	If Object.EmailStatus <> CurrentEmailStatus Then
		...
		InteractionsServerCall.SetEmailFolder(Object.Ref,
			Interactions.DefineDefaultFolderForEmail(Object.Ref, True));
...
Procedure RecipientsListPresentationStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If Object.EmailStatus <> PredefinedValue("Enum.OutgoingEmailStatuses.Sent") Then
		...
		If SendingOption  = "SendTo:" Then
			SelectionGroup = NStr("ru = 'Кому'; en = 'SendTo'; pl = 'SendTo';de = 'SendTo';ro = 'SendTo';tr = 'SendTo'; es_ES = 'SendTo'");
		ElsIf SendingOption  = "Copy:" Then
			SelectionGroup = NStr("ru = 'Копии'; en = 'Cc'; pl = 'Cc';de = 'Cc';ro = 'Cc';tr = 'Cc'; es_ES = 'Cc'");
		ElsIf SendingOption = "BCC:" Then
			SelectionGroup = NStr("ru = 'Скрытые'; en = 'Hidden'; pl = 'Hidden';de = 'Hidden';ro = 'Hidden';tr = 'Hidden'; es_ES = 'Hidden'");
		EndIf;
...
Procedure RecipientsListBeforeEditEnd(Item, NewRow, CancelEdit, Cancel)
	...
	If StrFind(Address, "@") = 0 OR StrFind(Address, ".") = 0 Then
		ShowMessageBox(, NStr("ru = 'Необходимо ввести адрес электронной почты'; en = 'Enter an email address'; pl = 'Enter an email address';de = 'Enter an email address';ro = 'Enter an email address';tr = 'Enter an email address'; es_ES = 'Enter an email address'"));
		Cancel = True;
		Return;
	EndIf;
	...
	If FoundRows.Count() > 1 Then
		ErrorTextTemplate = NStr("ru = 'Адрес %1 уже есть в списке.'; en = 'Address %1 already exists in the list.'; pl = 'Address %1 already exists in the list.';de = 'Address %1 already exists in the list.';ro = 'Address %1 already exists in the list.';tr = 'Address %1 already exists in the list.'; es_ES = 'Address %1 already exists in the list.'");
...
Procedure RecipientsListPresentationAutoComplete(Item, Text, ChoiceData, DataGetParameters, Waiting, StandardProcessing)
	...
	ChoiceData = ChoiceDataForEmail(Text, 0);
	...
	Else
		...
		If RecipientsList.Count() = 0 Then
			...
			CommonClient.MessageToUser(
				NStr("ru='Необходимо указать хотя бы одного получателя письма.'; en = 'Specify at least one email recipient.'; pl = 'Specify at least one email recipient.';de = 'Specify at least one email recipient.';ro = 'Specify at least one email recipient.';tr = 'Specify at least one email recipient.'; es_ES = 'Specify at least one email recipient.'"),, "RecipientsList");
			Return;
...
Procedure SpecifyContacts(Command)
	...
	If Object.EmailStatus <> PredefinedValue("Enum.OutgoingEmailStatuses.Sent") Then
		...
		EditRecipientsList(True, NStr("ru = 'Копии'; en = 'Cc'; pl = 'Cc';de = 'Cc';ro = 'Cc';tr = 'Cc'; es_ES = 'Cc'"));
	...
	For each AddressString In RecipientsList Do
		...
		For Each AddressStructure In Result Do
			...
			If Not IsBlankString(AddressStructure.ErrorDescription) Then
				Cancel = True;
				CommonClient.MessageToUser(AddressStructure.ErrorDescription, , "RecipientsList[" + String(RecipientsList.IndexOf(AddressString)) + "].Presentation");
			EndIf;
...
Procedure FillAttachments(PassedParameters = Undefined)
	...
	If Object.Ref.IsEmpty() AND PassedParameters <> Undefined Then
		If PassedParameters.Property("Base") 
			...
			If  PassedParameters.Basis.Command = "Forward" Then
				...
				AttachmentTab = EmailManagement.GetEmailAttachments(PassedParameters.Basis.Base, True);
				...
				DataStoredInAttachmentsEmailsDatabase = Interactions.DataStoredInAttachmentsEmailsDatabase(PassedParameters.Basis.Base);
			...
			ElsIf PassedParameters.Basis.Command = "ForwardAsAttachment"
				AND Parameters.Basis.Property("Base")  Then

				AddEmailAttachment(Parameters.Basis.Base);
...
Procedure AttachIncomingBaseEmailAsAttachmentIfNecessary(CurrentObject)
	...
	If CurrentObject.EmailStatus = Enums.OutgoingEmailStatuses.Outgoing 
		...
		FileParameters.Insert("FilesOwner", CurrentObject.Ref);
		FileParameters.Insert("Author", Undefined);
		FileParameters.Insert("BaseName", NStr("ru = 'Пересылаемое сообщение'; en = 'Forwarded message'; pl = 'Forwarded message';de = 'Forwarded message';ro = 'Forwarded message';tr = 'Forwarded message'; es_ES = 'Forwarded message'"));
		FileParameters.Insert("ExtensionWithoutPoint", "html");
...
&AtClient
Procedure NewFileAfterInitialization(File, AdditionalParameters) Export
	
	NotifyDescription = New NotifyDescription("GetSizeAfterCompletion", ThisObject, AdditionalParameters);
	File.BeginGettingSize(NotifyDescription);
	
EndProcedure

&AtClient
Procedure GetSizeAfterCompletion(Size, AdditionalParameters) Export
...
Procedure FileSelectionDialogAfterChoice(SelectedFiles, AdditionalParameters) Export
	...
	For Each SelectedFile In SelectedFiles Do
		...
		NotifyDescription = New NotifyDescription("NewFileAfterInitialization", ThisObject, AdditionalParameters);
		File = New File();
		File.BeginInitialization(NotifyDescription, SelectedFile);
...
Procedure IsFileAfterCompletionCheck(IsFile, AdditionalParameters) Export
	...
	NotifyDescription              = New NotifyDescription("NewFileAfterInitialization", ThisObject, AdditionalParameters);
	File = New File();
	File.BeginInitialization(NotifyDescription, FullName);
...
Function GenerateOutgoingMessagePlainText(SelectionIncomingEmailData, CurrentObject)
	...
	StringHeader = NStr("ru = '-----Пересылаемое сообщение-----'; en = '-----Forwarded message-----'; pl = '-----Forwarded message-----';de = '-----Forwarded message-----';ro = '-----Forwarded message-----';tr = '-----Forwarded message-----'; es_ES = '-----Forwarded message-----'");
	
	StringHeader = StringHeader + Chars.LF+ NStr("ru = 'От'; en = 'From'; pl = 'From';de = 'From';ro = 'From';tr = 'From'; es_ES = 'From'") + ": "+ SelectionIncomingEmailData.SenderPresentation
		          + ?(SelectionIncomingEmailData.MetadataObjectName = "IncomingEmail",
		          "[" + SelectionIncomingEmailData.SenderAddress +"]",
		          "");
		
	StringHeader = StringHeader + Chars.LF+ NStr("ru = 'Отправлено'; en = 'Sent'; pl = 'Sent';de = 'Sent';ro = 'Sent';tr = 'Sent'; es_ES = 'Sent'") + ": " 
	              + Format(SelectionIncomingEmailData.Date,"DLF=DT");
	
	StringHeader = StringHeader + Chars.LF+ NStr("ru = 'Кому'; en = 'SendTo'; pl = 'SendTo';de = 'SendTo';ro = 'SendTo';tr = 'SendTo'; es_ES = 'SendTo'") + ": " 
	...
	If CCRecipientsTable.Count() > 0 Then
		StringHeader = StringHeader + Chars.LF+ NStr("ru = 'Копии'; en = 'Cc'; pl = 'Cc';de = 'Cc';ro = 'Cc';tr = 'Cc'; es_ES = 'Cc'") + ": "
		+ Interactions.GetIncomingEmailRecipientsPresentations(CCRecipientsTable);
	EndIf;
	
	StringHeader = StringHeader + Chars.LF+ NStr("ru = 'Тема'; en = 'Subject'; pl = 'Subject';de = 'Subject';ro = 'Subject';tr = 'Subject'; es_ES = 'Subject'") + ": " + SelectionIncomingEmailData.Subject;
	...
	If PassedParameters.Property("Recipient") AND PassedParameters.Recipient <> Undefined Then
		...
		ElsIf TypeOf(PassedParameters.Recipient) = Type("ValueList") Then
			...
			For Each ListItem In PassedParameters.Recipient Do
				...
				NewRow.SendingOption = NStr("ru = 'Кому:'; en = 'To:'; pl = 'To:';de = 'To:';ro = 'To:';tr = 'To:'; es_ES = 'To:'");
				NewRow.Address = ListItem.Value;
				NewRow.Presentation = ProcessedAddresseePresentation(ListItem.Presentation);
			EndDo;
...
Procedure SendExecute()
	...
	If FoundRows.Count() = 0 Then
		CommonClient.MessageToUser(
			NStr("ru ='Выбранная учетная запись не доступна для отправки писем'; en = 'The selected account is not available for sending emails'; pl = 'The selected account is not available for sending emails';de = 'The selected account is not available for sending emails';ro = 'The selected account is not available for sending emails';tr = 'The selected account is not available for sending emails'; es_ES = 'The selected account is not available for sending emails'"),, "SenderPresentation", "Object");
		Return;
	EndIf;
	...
	If FoundRows[0].DeleteAfterSend Then
			...
			ButtonsList.Add(DialogReturnCode.Yes, NStr("ru = 'Отправить'; en = 'Send'; pl = 'Send';de = 'Send';ro = 'Send';tr = 'Send'; es_ES = 'Send'"));
			ButtonsList.Add(DialogReturnCode.No, NStr("ru = 'Отправить и сохранить'; en = 'Send and save'; pl = 'Send and save';de = 'Send and save';ro = 'Send and save';tr = 'Send and save'; es_ES = 'Send and save'"));
			ButtonsList.Add(DialogReturnCode.Cancel, NStr("ru = 'Отмена'; en = 'Cancel'; pl = 'Cancel';de = 'Cancel';ro = 'Cancel';tr = 'Cancel'; es_ES = 'Cancel'"));
			...
			QuestionText = NStr("ru = 'Для данной учетной записи не предусмотрено сохранение отправленных писем в системе.
			                    ...
			                    |Continue?'; 
			                    |pl = 'It is not required to save the sent emails for this account.
			                    |Continue?';
			                    |de = 'It is not required to save the sent emails for this account.
			                    |Continue?';
			                    |ro = 'It is not required to save the sent emails for this account.
			                    |Continue?';
			                    |tr = 'It is not required to save the sent emails for this account.
			                    |Continue?'; 
			                    |es_ES = 'It is not required to save the sent emails for this account.
			                    |Continue?'");
			
			CloseNotificationHandler = New NotifyDescription("PromptForNotSavingSentEmail", ThisObject);
			ShowQueryBox(CloseNotificationHandler,QuestionText, ButtonsList,, DialogReturnCode.Yes, NStr("ru = 'Отправка письма'; en = 'Sending email'; pl = 'Sending email';de = 'Sending email';ro = 'Sending email';tr = 'Sending email'; es_ES = 'Sending email'"));
...
Procedure ForwardExecute()
	...
	Base = New Structure("Base,Command", Object.Ref, "Forward");
...
Procedure SetButtonTitleByDefault()
	...
	If Object.EmailStatus = Enums.OutgoingEmailStatuses.Sent Then
		Items.Send.Title = NStr("ru = 'Переслать'; en = 'Forward'; pl = 'Forward';de = 'Forward';ro = 'Forward';tr = 'Forward'; es_ES = 'Forward'");
	ElsIf Object.EmailStatus = Enums.OutgoingEmailStatuses.Outgoing Then
		Items.Send.Title = NStr("ru = 'Отправить сейчас'; en = 'Send now'; pl = 'Send now';de = 'Send now';ro = 'Send now';tr = 'Send now'; es_ES = 'Send now'");
	ElsIf Object.EmailStatus = Enums.OutgoingEmailStatuses.Draft Then
		...
		Items.Send.Title = NStr("ru = 'Отправить'; en = 'Send'; pl = 'Send';de = 'Send';ro = 'Send';tr = 'Send'; es_ES = 'Send'");
...
&AtServerNoContext
Function ChoiceDataForEmail(Text, IndexInRecipientsList)
	
	ListToReturn = New ValueList;
	ContactsTypesDetailsArray = InteractionsClientServer.ContactsDetails();
	
	QueryText = 
	"SELECT ALLOWED DISTINCT
	|	CatalogContact.Ref AS Contact,
	|	PRESENTATION(CatalogContact.Ref) AS Presentation,
	|	ContactInformationTable.EMAddress AS EMAddress 
	|FROM
	...
	|WHERE
	|	(CatalogContact.Description LIKE &EnteredString
	...
	For each DetailsArrayElement In ContactsTypesDetailsArray Do
		...
		Else
			...
			ObjectMetadata = Metadata.FindByType(DetailsArrayElement.Type);
			...
			For Each Field In FieldsList Do
				If ObjectMetadata.StandardAttributes[Field.Name].Type.Types()[0] = Type("String") Then
					InputFieldsConditionByString = InputFieldsConditionByString + " OR CatalogContact." + Field.Name + " LIKE &EnteredString";
				EndIf;
			EndDo;
			...
			|SELECT
			|	CatalogContact.Ref                                                           AS Contact,
			|	CatalogContact." + DetailsArrayElement.ContactPresentationAttributeName + " AS Presentation,
			|	ContactInformationTable.EMAddress                                                AS EMAddress
			|FROM
			|	Catalog." + DetailsArrayElement.Name + " AS CatalogContact
			|		INNER JOIN Catalog." + DetailsArrayElement.Name + ".ContactInformation AS ContactInformationTable
			|		ON (ContactInformationTable.Ref = CatalogContact.Ref)
			|			AND (ContactInformationTable.Type = VALUE(Enum.ContactInformationTypes.EmailAddress) AND (ContactInformationTable.EMAddress <> """"))
			|WHERE
			|	(ContactInformationTable.EMAddress LIKE &EnteredString
			|			OR ContactInformationTable.ServerDomainName LIKE &EnteredString
			|			OR ContactInformationTable.Presentation LIKE &EnteredString 
			|" + InputFieldsConditionByString + ") 
			|";
	...
	Query.SetParameter("EnteredString", Text + "%");
	...
	While Selection.Next() Do
		...
		ChoiceValue.Insert("Address", Selection.EMAddress);
		ChoiceValue.Insert("Presentation", Selection.Presentation);
		ChoiceValue.Insert("IndexInRecipientsList", IndexInRecipientsList);
		ListToReturn.Add(ChoiceValue, 
			InteractionsClientServer.GetAddresseePresentation(Selection.Presentation, Selection.EMAddress, ""));
...
Procedure EditRecipientsList(ToSelect, SelectionGroup = "")
	...
	For each Recipient In RecipientsList Do
		If Recipient.SendingOption = NStr("ru = 'Обратный адрес:'; en = 'Return address:'; pl = 'Return address:';de = 'Return address:';ro = 'Return address:';tr = 'Return address:'; es_ES = 'Return address:'") Then
			NewRow = Object.ReplyRecipients.Add();
		ElsIf Recipient.SendingOption = NStr("ru = 'Копия:'; en = 'Copy:'; pl = 'Copy:';de = 'Copy:';ro = 'Copy:';tr = 'Copy:'; es_ES = 'Copy:'") Then
			NewRow = Object.CCRecipients.Add();
		ElsIf Recipient.SendingOption = NStr("ru = 'Скрытая копия:'; en = 'BCC:'; pl = 'BCC:';de = 'BCC:';ro = 'BCC:';tr = 'BCC:'; es_ES = 'BCC:'") Then
	...
	OpeningParameters.Insert("DefaultGroup",?(IsBlankString(SelectionGroup),NStr("ru = 'Кому'; en = 'SendTo'; pl = 'SendTo';de = 'SendTo';ro = 'SendTo';tr = 'SendTo'; es_ES = 'SendTo'"), SelectionGroup));
...
Procedure GenerateEmailRecipientsLists()
	...
	AddAddressToRecipientsList(RecipientsList, Object.EmailRecipients, NStr("ru = 'Кому:'; en = 'To:'; pl = 'To:';de = 'To:';ro = 'To:';tr = 'To:'; es_ES = 'To:'"));
	AddAddressToRecipientsList(RecipientsList, Object.CCRecipients, NStr("ru = 'Копия:'; en = 'Copy:'; pl = 'Copy:';de = 'Copy:';ro = 'Copy:';tr = 'Copy:'; es_ES = 'Copy:'"));
	AddAddressToRecipientsList(RecipientsList, Object.BccRecipients, NStr("ru = 'Скрытая копия:'; en = 'BCC:'; pl = 'BCC:';de = 'BCC:';ro = 'BCC:';tr = 'BCC:'; es_ES = 'BCC:'"));
	AddAddressToRecipientsList(RecipientsList, Object.ReplyRecipients, NStr("ru = 'Обратный адрес:'; en = 'Return address:'; pl = 'Return address:';de = 'Return address:';ro = 'Return address:';tr = 'Return address:'; es_ES = 'Return address:'"));
...
Function ExecuteSendingAtServer()
	...
	Except
		...
		If IsIssueOfEmailAddressesServerRejection 
			...
			Result.MessageText = StringFunctionsClientServer.SubstituteParametersToString(
				...
				|pl = 'The following email addressees are not accepted by the mail server:
				|%1. Email is sent to other addressees.';
				|de = 'The following email addressees are not accepted by the mail server:
				|%1. Email is sent to other addressees.';
				|ro = 'The following email addressees are not accepted by the mail server:
				|%1. Email is sent to other addressees.';
				|tr = 'The following email addressees are not accepted by the mail server:
				|%1. Email is sent to other addressees.'; 
				|es_ES = 'The following email addressees are not accepted by the mail server:
				|%1. Email is sent to other addressees.'", Common.DefaultLanguageCode()),
	...
	If NOT EmailObject.DeleteAfterSend Then
		...
		InteractionsServerCall.SetEmailFolder(
			Object.Ref, Interactions.DefineFolderForEmail(Object.Ref));
		CurrentEmailStatus = Object.EmailStatus;
...
Procedure DeleteOldSignatureItems(EmailTextFormattedDocument, OldSignature)
	...
	EmailTextFormattedDocument.GetHTML(HTMLTextOldSignature, AttachmentsOldSignature);
...
Procedure AddFormattedDocumentToFormattedDocument(DocumentRecipient, DocumentToAdd)
	...
	For Ind = 0 To DocumentToAdd.Items.Count() -1 Do
		...
		Else
			If TypeOf(ItemToAdd) = Type("FormattedDocumentText") Then
...
Procedure DeterminePossibilityToFillEmailByTemplate()
	...
	If Object.EmailStatus = Enums.OutgoingEmailStatuses.Draft
		...
		If ModuleMessagesTemplatesInternal.MessagesTemplatesUsed() Then
			MessagesTemplatesUsed = ModuleMessagesTemplatesInternal.HasAvailableTemplates("Email");
		EndIf;
	EndIf;
